...
var SettingsFormComponent = class _SettingsFormComponent extends AsyncHandler {
  ...
  ngOnChanges(changes) {
    if (changes.merge) {
      this.encryption_level.set(this.merge() ? He.NeverDisplay + 1 : He.None);
    }
  ...
  save(level) {
    ...
    lastValueFrom(settings[level].id ? Zc(settings[level].id, details) : Xc(details)).then((new_settings) => {
      this.saving.update((s) => {
        s[level] = false;
        return s;
      });
  ...
  saveAll() {
    ...
    const promises = [];
    for (let i = 0; i < He.NeverDisplay + 1; i++) {
      const settings = this.settings();
      if (settings[i] && !this.saving()[i]) {
        ...
        promises.push(settings[i].id ? Zc(settings[i].id, details) : Xc(details));
      }
    }
    if (promises.length) {
  ...
  _processSettings(settings) {
    ...
    for (let i = 0; i < He.NeverDisplay + 1; i++) {
      ...
      if (!setting) {
        processed_settings.push(this._processSetting(new Oe({ encryption_level: i })));
        continue;
      }
  ...
  _processSetting(setting) {
    if (setting.encryption_level === He.Admin && !this.is_admin || setting.encryption_level === He.Support && !this.is_support || setting.encryption_level === He.NeverDisplay) {
      const obj = {};
      for (const key of setting.keys) {
        obj[key] = `<${i18n("COMMON.SETTINGS_MASKED")}>`;
      }
      const settings_string = (setting.keys || []).length ? dump(obj) : "";
      return new Oe(__spreadProps(__spreadValues({}, setting), {
        parent_id: this.id(),
        settings_string
      }));
    }
    return new Oe(__spreadProps(__spreadValues({}, setting), { parent_id: this.id() }));
  }
...
//# sourceMappingURL=chunk-4TTJT75G.js.map
