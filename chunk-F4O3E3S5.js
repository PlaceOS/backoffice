...
var RepositoryFormComponent = class _RepositoryFormComponent extends AsyncHandler {
  ...
  is_interface = computed(
    () => this.formModel().repo_type === Ms.Interface,
  ...
  ngOnInit() {
    ...
    this.repo_types.set([
      { id: Ms.Driver, name: i18n("REPOS.TYPE_DRIVER") },
      {
        id: Ms.Interface,
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (form_item.id ? Dc(form_item.id, form_item) : Hc(form_item));
  ...
  async _loadDefaultBranch() {
    const { id, uri, username, password } = this.formModel();
    return (id ? Kc(id) : isValidUrl(uri) && uri.startsWith("http") ? Lc({
      repository_url: uri,
      username,
      password
  ...
  async _loadCommits() {
    ...
    try {
      const list = await (id ? Wc(id, { branch }) : Gc({
        repository_url: uri,
        username,
        password,
        branch
...
//# sourceMappingURL=chunk-F4O3E3S5.js.map
