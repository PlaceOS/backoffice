...
import {
  SystemStateService
} from "./chunk-XB3VKKB2.js";
import "./chunk-6H2QAZNA.js";
import {
  BindingDirective
} from "./chunk-MCO4TBLM.js";
...
import {
  ...
  MatMenuModule,
  MatMenuTrigger
} from "./chunk-KVIAPY72.js";
import {
  ModuleRuntimeErrorsModalComponent
} from "./chunk-EXE7PQKU.js";
import {
  MatProgressBar,
  MatProgressBarModule
} from "./chunk-NU74PY2O.js";
import "./chunk-FYYQIRW2.js";
import {
  SettingsFieldComponent
} from "./chunk-HFVGZQWU.js";
...
var ViewModuleStateModalComponent = class _ViewModuleStateModalComponent extends AsyncHandler {
  ...
  async updateState() {
    ...
    try {
      const state = await va(this.system.id, class_parts.slice(0, class_parts.length - 1).join("_"), num);
...
var SystemModulesComponent = class _SystemModulesComponent extends AsyncHandler {
  ...
  query_fn = (_) => ic({ q: _, no_logic: true }).then((resp) => resp.data.map((mod) => __spreadProps(__spreadValues({}, mod), {
    extra: mod.driver?.name
  })));
  /** Function for excluding modules already within this system */
  exclude_fn = (item, __) => item.control_system_id === this.item.id || item.role === Ct.Logic;
  ...
  async loadModule(device) {
    await pc(device.id).then(() => notifySuccess(`Successfully loaded module "${device.name || device.id}"`), (err) => notifyError(`Error loading module. Error: ${JSON.stringify(err.response || err.message || err)}`));
  }
  /**
...
//# sourceMappingURL=chunk-XXPUSTMW.js.map
