...
var APIKeyModalComponent = class _APIKeyModalComponent extends AsyncHandler {
  ...
  users = computed(
    () => {
      ...
      const list = [...this.user_list()].sort((a, b2) => (a.name || "").localeCompare(b2.name || ""));
  ...
  async loadSelectedUser() {
    ...
    if (!user_id || selected_user)
      return;
    const user = await Za(user_id).catch(() => null);
    if (!user)
      return;
  ...
  loadUsers() {
    this.timeout("load_users", async () => {
      ...
      try {
        const users = this.domain() ? (await Ka({
...
var APIKeyService = class _APIKeyService {
  ...
  _available_scopes = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_available_scopes" } : (
    /* istanbul ignore next */
    {}
  )), { loader: async () => await f("/api/engine/v2/scopes") }));
  ...
  _users = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_users" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!domain)
        return [];
      return (await Ka({ authority_id: domain.id, q: q2 })).data;
    }
  }));
  users = computed(
    () => this._users.value() || [],
  ...
  async newKey() {
    ...
    const key = await x({
      ...
      fn: (d) => new PlaceAPIKeyDetails(d),
      path: "api_keys",
      form_data: __spreadProps(__spreadValues({}, api_key), {
        expires_at: api_key.expires_at ?? api_key.ttl ? getUnixTime(Date.now() + api_key.ttl) : void 0,
  ...
  async _currentUser() {
    ...
    if (cached_user?.id)
      return cached_user;
    return Za("current").catch(() => null);
  }
  async _scopes() {
    const scopes = this.available_scopes();
    if (scopes.length)
      return scopes;
    return f("/api/engine/v2/scopes").catch(() => []);
  }
  _userLabel(user) {
...
function AdminAPIKeysComponent_ng_template_39_Conditional_1_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", ctx_r2.isExpired(data_r9) ? "Expired" : \u0275\u0275pipeBind1(2, 3, +data_r9 * 1e3), " ");
  }
}
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminAPIKeysComponent, { className: "AdminAPIKeysComponent", filePath: "src/app/admin/api-keys/api-keys.component.ts", lineNumber: 237 });
})();
export {
  AdminAPIKeysComponent
};
//# sourceMappingURL=chunk-GNGJJQZD.js.map
