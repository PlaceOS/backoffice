...
var ResourceImportsComponent = class _ResourceImportsComponent {
  ...
  async ngOnInit() {
    const domain = Ut();
    const domain_list = await lastValueFrom(xu({ limit: 100 }).pipe(map((_) => _.data)));
  ...
  async importResource(resource, notify = true) {
    ...
    if (!domain)
      return;
    const system = await lastValueFrom(Xc({
  ...
  async loadResourceList() {
    ...
    const result = await lastValueFrom(O({
      path: "place",
    ...
    const { data } = await lastValueFrom(Jc({
...
//# sourceMappingURL=chunk-ASBAEIWN.js.map
