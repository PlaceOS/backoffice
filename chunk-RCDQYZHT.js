...
function generateBasicHandlers(endpoint, data, filter = (_, _q) => true) {
  ...
  Iu(action);
  Iu(__spreadProps(__spreadValues({}, action), { method: "PUT" }));
  Iu({
    path: `${endpoint}/:id`,
    metadata: data,
    method: "DELETE",
...
Iu({
  ...
  metadata: REPO_DATA,
  method: "GET",
  callback: (_) => REPO_DATA.filter((repo) => repo.type === yi.Interface).map((repo) => repo.name)
});
...
//# sourceMappingURL=chunk-RCDQYZHT.js.map
