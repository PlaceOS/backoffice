...
var BindingDirective = class _BindingDirective extends AsyncHandler {
  ...
  onEvent = input("", ...ngDevMode ? [{ debugName: "onEvent" }] : []);
  ...
  _old_model = null;
  ngOnInit() {
    $u()?.pipe(first((_) => _)).subscribe((_) => this.bindVariable());
  }
  ngOnChanges(changes) {
  ...
  bindVariable() {
    if (Dt() && this.bind() && this.sys() && this.mod() && !this._binding) {
      this.timeout("bind", () => {
        const module = vp(this.sys(), this.mod(), this.index());
  ...
  execute() {
    if (Dt() && this.exec() && this.sys() && this.mod() && !this._timers["execute"]) {
...
//# sourceMappingURL=chunk-CIPLR5DU.js.map
