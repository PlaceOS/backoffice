...
var PlaceDetailsComponent = class _PlaceDetailsComponent extends AsyncHandler {
  ...
  async loadApiDetails() {
    const details = await lastValueFrom(v(`${h()}/cluster/versions`)).catch((err) => notifyError(i18n("ADMIN.BACKEND_SERVICES_ERROR", {
      error: JSON.stringify(err.response || err.message || err)
    })));
    this.api_details.set(details || []);
  }
  async loadPlatformDetails() {
    const { changelog, version } = await lastValueFrom(v(`${h()}/platform`)).catch((err) => {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(PlaceDetailsComponent, { className: "PlaceDetailsComponent", filePath: "src/app/admin/details.component.ts", lineNumber: 200 });
})();
export {
  PlaceDetailsComponent
};
//# sourceMappingURL=chunk-4QIYDX5K.js.map
