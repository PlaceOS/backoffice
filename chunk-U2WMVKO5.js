...
var EdgeModalComponent = class _EdgeModalComponent {
  ...
  async save() {
    ...
    const method = edge.id ? Lu(edge.id, edge) : Bu(edge);
...
var PlaceEdgeComponent = class _PlaceEdgeComponent {
  ...
  async token(edge) {
    const details = await lastValueFrom(Qu(edge.id));
  ...
  async remove(i) {
    ...
    const err = await lastValueFrom(Vu(i.id)).catch((_) => _);
    details.close();
    if (err)
  ...
  async loadEdges() {
    ...
    const { data } = await lastValueFrom(Hu());
...
//# sourceMappingURL=chunk-U2WMVKO5.js.map
