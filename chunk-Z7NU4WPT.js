...
function generateBasicHandlers(endpoint, data, filter = (_, _q) => true) {
  ...
  to(action);
  to(__spreadProps(__spreadValues({}, action), { method: "PUT" }));
  to({
    path: `${endpoint}/:id`,
    metadata: data,
    method: "DELETE",
...
to({
  ...
  metadata: REPO_DATA,
  method: "GET",
  callback: (_) => REPO_DATA.filter((repo) => repo.type === Us.Interface).map((repo) => repo.name)
});
...
//# sourceMappingURL=chunk-Z7NU4WPT.js.map
