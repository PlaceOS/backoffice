...
var ResourceImportsComponent = class _ResourceImportsComponent {
  ...
  async ngOnInit() {
    const domain = kt();
    const domain_list = await lastValueFrom(ru({ limit: 100 }).pipe(map((_) => _.data)));
  ...
  async importResource(resource, notify = true) {
    ...
    if (!domain)
      return;
    const system = await lastValueFrom(Ec({
  ...
  async loadResourceList() {
    ...
    const { data } = await lastValueFrom(Ac({
...
//# sourceMappingURL=chunk-NUPQC3ZG.js.map
