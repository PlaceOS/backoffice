...
var ZonesStateService = class _ZonesStateService {
    ...
    loader: async ({ params }) => {
      ...
      try {
        const details = await Promise.all([
          ...
          (isSubsystemUser() ? Promise.resolve({ data: [], total: 0 }) : oh(item.id)).then((d) => d.total).catch(() => 0),
  ...
  _systems = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_systems" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Yt))
        return [];
      const response = await querySupportSystems({ zone_id: item.id }).catch(() => ({ data: [] }));
  ...
  _triggers = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_triggers" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Yt))
        return [];
      const response = await (isSubsystemUser() ? Promise.resolve({ data: [], total: 0 }) : oh(item.id)).catch(() => ({
        data: []
      }));
      return response.data;
    }
  ...
  _children = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_children" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Yt))
        return [];
      const response = await querySupportZones({ parent_id: item.id }).catch(() => ({ data: [] }));
      return response.data;
    }
  }));
  children = computed(
...
//# sourceMappingURL=chunk-YLOSXOTA.js.map
