...
var SettingsFormComponent = class _SettingsFormComponent extends AsyncHandler {
  ...
  constructor() {
    super();
    effect(() => {
      this.encryption_level.set(this.merge() ? ze.NeverDisplay + 1 : ze.None);
    });
    effect(() => {
  ...
  save(level) {
    ...
    (settings[level].id ? oa(settings[level].id, details) : ua(details)).then((new_settings) => {
      this.saving.update((s) => {
        s[level] = false;
        return s;
      });
  ...
  saveAll() {
    ...
    const promises = [];
    for (let i = 0; i < ze.NeverDisplay + 1; i++) {
      const settings = this.settings();
      if (settings[i] && !this.saving()[i]) {
        ...
        promises.push(settings[i].id ? oa(settings[i].id, details) : ua(details));
      }
    }
    if (promises.length) {
  ...
  _processSetting(setting) {
    if (setting.encryption_level === ze.Admin && !this.is_admin || setting.encryption_level === ze.Support && !this.is_support || setting.encryption_level === ze.NeverDisplay) {
      const obj = {};
...
//# sourceMappingURL=chunk-APUZ6JVU.js.map
