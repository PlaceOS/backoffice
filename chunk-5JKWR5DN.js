...
function generateBasicHandlers(endpoint, data, filter = (_, q) => true) {
  ...
  Is(action);
  Is(__spreadProps(__spreadValues({}, action), { method: "PUT" }));
  Is({
    path: `${endpoint}/:id`,
    metadata: data,
    method: "DELETE",
...
Is({
  ...
  metadata: REPO_DATA,
  method: "GET",
  callback: (_) => REPO_DATA.filter((repo) => repo.type === Rr.Interface).map((repo) => repo.name)
});
...
var PARENT_TYPES = ["systems", "zones", "drivers", "users"];
...
Is({
  ...
  callback: (event) => {
    ...
    if (index >= 0) {
      OAUTH_SOURCES[index] = __spreadProps(__spreadValues(__spreadValues({}, OAUTH_SOURCES[index]), event.body), { updated_at: Date.now() / 1e3 });
      saveToSession("oauth_sources", OAUTH_SOURCES);
...
Is({
  ...
  callback: (event) => {
    ...
    if (index >= 0) {
      SAML_SOURCES[index] = __spreadProps(__spreadValues(__spreadValues({}, SAML_SOURCES[index]), event.body), { updated_at: Date.now() / 1e3 });
      saveToSession("saml_sources", SAML_SOURCES);
...
Is({
  ...
  callback: (event) => {
    ...
    if (index >= 0) {
      LDAP_SOURCES[index] = __spreadProps(__spreadValues(__spreadValues({}, LDAP_SOURCES[index]), event.body), { updated_at: Date.now() / 1e3 });
      saveToSession("ldap_sources", LDAP_SOURCES);
...
Is({
  ...
  metadata: [],
  method: "GET",
  callback: (event) => createFilter2(API_KEYS)(event.query_params)
});
Is({
...
Is({
  ...
  metadata: [],
  method: "GET",
  callback: (event) => createFilter2(BROKERS)(event.query_params)
});
Is({
  path: `${API}/brokers/:id`,
  metadata: [],
...
Is({
  ...
  callback: (event) => {
    ...
    if (index >= 0) {
      BROKERS[index] = __spreadProps(__spreadValues(__spreadValues({}, BROKERS[index]), event.body), { updated_at: Date.now() / 1e3 });
      saveToSession("brokers", BROKERS);
...
Is({
  ...
  metadata: [],
  method: "GET",
  callback: (event) => createFilter2(EDGES)(event.query_params)
});
Is({
  path: `${API}/edges/:id`,
  metadata: [],
...
Is({
  ...
  callback: (event) => {
    ...
    if (index >= 0) {
      EDGES[index] = __spreadProps(__spreadValues(__spreadValues({}, EDGES[index]), event.body), { updated_at: Date.now() / 1e3 });
      saveToSession("edges", EDGES);
      return EDGES[index];
    }
    throw { status: 404, message: "Edge not found" };
...
Is({
  ...
  metadata: [],
  method: "GET",
  callback: (event) => createFilter2(APPLICATIONS)(event.query_params)
});
Is({
...
Is({
  ...
  callback: (event) => {
    ...
    if (index >= 0) {
      APPLICATIONS[index] = __spreadProps(__spreadValues(__spreadValues({}, APPLICATIONS[index]), event.body), { updated_at: Date.now() / 1e3 });
      saveToSession("applications", APPLICATIONS);
...
Is({
  ...
  metadata: [],
  method: "GET",
  callback: (event) => createFilter2(UPLOADS)(event.query_params)
});
Is({
  path: `${API}/uploads/:id`,
  metadata: [],
...
Is({
  ...
  metadata: [],
  method: "GET",
  callback: (event) => createFilter2(STORAGE_PROVIDERS)(event.query_params)
});
Is({
...
Is({
  ...
  callback: (event) => {
    ...
    if (index >= 0) {
      STORAGE_PROVIDERS[index] = __spreadProps(__spreadValues(__spreadValues({}, STORAGE_PROVIDERS[index]), event.body), { updated_at: Date.now() / 1e3 });
      saveToSession("storage_providers", STORAGE_PROVIDERS);
...
Is({
  ...
  callback: (event) => {
    ...
    if (index >= 0) {
      TENANTS[index] = __spreadProps(__spreadValues(__spreadValues({}, TENANTS[index]), event.body), { updated_at: Date.now() / 1e3 });
      saveToSession("tenants", TENANTS);
...
var PARENT_TYPES2 = ["systems", "zones", "drivers", "modules", "users", "triggers", "repositories"];
...
//# sourceMappingURL=chunk-5JKWR5DN.js.map
