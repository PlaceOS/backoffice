...
var AuthSourceModalComponent = class _AuthSourceModalComponent extends AsyncHandler {
  ...
  samlFormModel = signal(
    generateSAMLSourceForm(new sr({ authority_id: this._data.domain.id })),
  ...
  ldapFormModel = signal(
    generateLDAPSourceForm(new Vi({ authority_id: this._data.domain.id })),
  ...
  type = computed(
    () => this.item() instanceof sr ? "saml" : this.item() instanceof Vi ? "ldap" : "oauth",
  ...
  setType(type) {
    ...
    switch (type) {
      case "saml":
        ...
        break;
      case "ldap":
        this.item.set(new Vi(data));
        break;
      default:
        this.item.set(new tr(data));
        break;
    }
    this.initialiseForm();
  }
  updateMethod(item) {
    switch (this.type()) {
      case "saml":
        return item.id ? ta(item.id, item) : na(item);
      case "ldap":
        return item.id ? Qu(item.id, item) : Ku(item);
    }
    return item.id ? bc(item.id, item) : $c(item);
  }
  /**
...
var ApplicationFormComponent = class _ApplicationFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (save_item.id ? yo(save_item.id, save_item) : bo(save_item));
...
var DomainStateService = class _DomainStateService {
  ...
  _domain_id = computed(
    () => {
      const item = this.item();
      return item instanceof Qi ? item.id : void 0;
    },
  ...
  _applications = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_applications" } : (
    ...
    loader: async ({ params }) => {
      const response = await mo(__spreadValues({
        authority_id: params.id
  ...
  async update(domain) {
    const item = await Lo(domain.id, domain);
  ...
  async performAzureIntegration() {
    ...
    if (!(item instanceof Qi))
      return;
    const result = await p(`/api/engine/v2/admin_consent/${encodeURIComponent(item.id)}`).catch((error) => {
  ...
  async editApplication(item) {
    item = item || new Hi({ owner_id: this.active_item.id });
  ...
  async deleteApplication(item) {
    ...
    const err = await $o(item.id).catch((_) => _);
    details.close();
    if (err)
  ...
  async deleteAuthSource(item) {
    ...
    const method = item instanceof sr ? sa : item instanceof tr ? vc : Zu;
...
//# sourceMappingURL=chunk-XRDYN7ZC.js.map
