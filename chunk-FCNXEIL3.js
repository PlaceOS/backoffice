...
var BackofficeUsersService = class _BackofficeUsersService extends AsyncHandler {
  ...
  get dark_mode() {
    ...
    const theme = localStorage.getItem("BACKOFFICE.theme") ?? (this._user() || {}).ui_theme;
  ...
  constructor() {
    super();
    const unsubscribe = wr().subscribe((online) => {
      if (online)
        this.load();
    });
  ...
  async query(query_params) {
    return (await Ia(query_params)).data;
  }
  load() {
...
//# sourceMappingURL=chunk-FCNXEIL3.js.map
