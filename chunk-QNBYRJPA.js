...
function querySupportSystems(options = {}) {
  ...
  return $a(__spreadValues(__spreadValues(__spreadValues({}, options), isSubsystemUser() ? { subsystem: "support" } : {}), selected_id() ? { group_id: selected_id() } : {}));
}
...
async function querySupportModules(options = {}) {
  ...
  for (let index = 0; index < systems.length; index += 8) {
    ...
    const pages = await Promise.all(systems.slice(index, index + 8).map((system) => collectPages(dc({ control_system_id: system.id })).catch((error) => {
...
//# sourceMappingURL=chunk-QNBYRJPA.js.map
