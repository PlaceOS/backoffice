...
var SystemStateService = class _SystemStateService extends AsyncHandler {
    ...
    loader: async ({ params }) => {
      ...
      try {
        const [triggers, metadata] = await Promise.all([
          _a(item.id).then((response) => response.total).catch(() => 0),
          Hu(item.id).then((response) => response.length).catch(() => 0)
        ]);
        return {
  ...
  _zones = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_zones" } : (
    ...
    loader: async ({ params }) => {
      ...
      try {
        const response = await pa(item.id).catch(() => ({
          data: []
        }));
  ...
  _triggers = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_triggers" } : (
    ...
    loader: async ({ params }) => {
      ...
      try {
        const response = await _a(item.id).catch(() => ({
          data: []
        }));
  ...
  async startSystem() {
    ...
    const error = await ua(this.active_item.id).then(() => null).catch((err) => {
  ...
  async stopSystem() {
    ...
    const error = await ca(this.active_item.id).then(() => null).catch((err) => {
  ...
  async newModule() {
    const mod = await this._state.edit(new ks({
  ...
  async addModuleToSystem(device) {
    ...
    const ref = this._dialog.open(SelectItemModalComponent, {
      data: {
        ...
        query_fn: (_) => Xc({ q: _ }).then((resp) => resp.data.filter((_2) => _2.id !== item.id))
      }
    });
    ...
    if (!system)
      return ref.close();
    await ra(system.id, device.id).catch((_e) => {
      ref.close();
  ...
  async addTrigger(trigger) {
    const t = await ma(this.active_item.id, {
      control_system_id: this.active_item.id,
      enabled: true,
      important: false,
  ...
  async removeTrigger(trigger) {
    ...
    if (!details?.reason)
      return;
    await ga(this.active_item.id, trigger.id).catch((err) => {
      details.close();
  ...
  async reorderModules(fst, snd) {
    ...
    const resp = await na(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      modules: list
    })).catch((err) => {
  ...
  async addZones(zone_list) {
    ...
    const system = await na(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      zones
    })).catch((err) => {
  ...
  async toggleModulePower(device) {
    const method = device.running ? Xu : Vu;
...
//# sourceMappingURL=chunk-CZJL7P5H.js.map
