import {
  UserPipe
} from "./chunk-BRRQL4FE.js";
import {
  SettingsFieldComponent
} from "./chunk-EXD2IQAI.js";
import {
  DiffViewerComponent
} from "./chunk-BQMD5MQB.js";
import {
  validateJSONString
...
var MetadataHistoryModalComponent = class _MetadataHistoryModalComponent {
  ...
  second_details = "";
  async ngOnInit() {
    const history = await ka(this._data.id, {
      name: this._data.name,
      limit: 5e3
    }).toPromise();
...
var MetadataDisplayComponent = class _MetadataDisplayComponent extends AsyncHandler {
  ...
  async deleteMetadata(field) {
    ...
    if (result.reason !== "done")
      return;
    await lastValueFrom(wa(this.item().id, { name: field })).catch((err) => {
    ...
    xa(this.item().id, __spreadProps(__spreadValues({}, value), {
      ...
      next: (item) => {
        ...
        if (field.name !== value.name) {
          lastValueFrom(wa(this.item().id, { name: field.name })).catch((err) => notifyError(`Error removing old "${field.name}" metadata. Error: ${JSON.stringify(err.response || err.message || err)}`));
        }
        if (index >= 0) {
          this.metadata.update((l) => {
...
//# sourceMappingURL=chunk-EHSFINS6.js.map
