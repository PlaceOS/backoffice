...
var SystemStateService = class _SystemStateService extends AsyncHandler {
  ...
  counts = combineLatest([
    ...
    const details = await Promise.all([
      lastValueFrom(la(item.id).pipe(map((d) => d.total))).catch(() => 0),
      lastValueFrom(Ku(item.id).pipe(map((d) => d.length))).catch(() => 0)
    ]);
  ...
  zones = this._state.item.pipe(debounceTime(200), switchMap((item) => {
    ...
    return aa(item.id).pipe(map((i) => [item, i.data]), startWith([item, []]));
  }), map(([item, zones]) => {
  ...
  triggers = combineLatest([
    ...
    return la(item.id).pipe(map((i) => i.data), startWith([]));
  ...
  async startSystem() {
    ...
    if (details && details.reason) {
      ...
      const resp = await lastValueFrom(ra(this.active_item.id)).catch((err) => {
  ...
  async stopSystem() {
    ...
    const resp = await lastValueFrom(ia(this.active_item.id)).catch((err) => {
  ...
  async addModuleToSystem(device) {
    ...
    const ref = this._dialog.open(SelectItemModalComponent, {
      data: {
        ...
        query_fn: (_) => Gc({ q: _ }).pipe(map((resp) => resp.data.filter((_2) => _2.id !== item.id)))
      }
    });
    ...
    if (!system)
      return;
    await lastValueFrom(ta(system.id, device.id)).catch((_e) => {
      ref.close();
  ...
  async addTrigger(trigger) {
    const t = await lastValueFrom(ha(this.active_item.id, {
      control_system_id: this.active_item.id,
      enabled: true,
      important: false,
  ...
  async removeTrigger(trigger) {
    ...
    if (!details || !details.reason)
      return;
    await lastValueFrom(fa(this.active_item.id, trigger.id)).catch((err) => {
      details.close();
  ...
  async reorderModules(fst, snd) {
    ...
    const resp = await lastValueFrom(Zc(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      modules: list
    }))).catch((err) => {
   ...
   */
  async joinModule(id) {
    await lastValueFrom(ta(this.active_item.id, id)).catch((err) => {
      notifyError(`Error adding module ${id} to system. Error: ${err.statusText || err.message || err}`);
    });
    this.timeout("join", async () => {
      const system = await lastValueFrom(Kc(this.active_item.id));
      if (system)
  ...
  async removeModule(device) {
    ...
    if (!details || !details.reason)
      return;
    const system = await lastValueFrom(na(this.active_item.id, device.id)).catch((err) => {
  ...
  async addZones(zone_list) {
    ...
    const system = await lastValueFrom(Zc(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      zones
    }))).catch((err) => {
  ...
  async toggleModulePower(device) {
    const method = device.running ? lc : ac;
...
//# sourceMappingURL=chunk-ASXQIFPJ.js.map
