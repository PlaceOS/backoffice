...
var SettingsFormComponent = class _SettingsFormComponent extends AsyncHandler {
  ...
  save(level) {
    ...
    lastValueFrom(settings[level].id ? Yc(settings[level].id, details) : Gc(details)).then((new_settings) => {
      this.saving.update((s) => {
        s[level] = false;
        return s;
      });
  ...
  saveAll() {
    ...
    for (let i = 0; i < He.NeverDisplay + 1; i++) {
      ...
      if (settings[i] && !this.saving()[i]) {
        ...
        promises.push(settings[i].id ? Yc(settings[i].id, details) : Gc(details));
      }
    }
    if (promises.length) {
  ...
  _processSettings(settings) {
    ...
    for (let i = 0; i < He.NeverDisplay + 1; i++) {
      ...
      if (!setting) {
        processed_settings.push(this._processSetting(new Ee({ encryption_level: i })));
        continue;
      }
  ...
  _processSetting(setting) {
    if (setting.encryption_level === He.Admin && !this.is_admin || setting.encryption_level === He.Support && !this.is_support || setting.encryption_level === He.NeverDisplay) {
      ...
      return new Ee(__spreadProps(__spreadValues({}, setting), {
        parent_id: this.id(),
        settings_string
      }));
    }
    return new Ee(__spreadProps(__spreadValues({}, setting), { parent_id: this.id() }));
  }
...
//# sourceMappingURL=chunk-NPZDXYW4.js.map
