...
setTimeout(() => {
  ...
  qa("current").pipe(delay(1e3), retry(10)).subscribe((user) => _current_user.next(user));
}, 300);
...
var SettingsService = class _SettingsService extends AsyncHandler {
  ...
  async init() {
    ...
    const data = await oc(user.id, "settings").toPromise();
...
//# sourceMappingURL=chunk-DUTIV43C.js.map
