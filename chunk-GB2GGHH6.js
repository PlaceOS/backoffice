...
var SystemStateService = class _SystemStateService extends AsyncHandler {
  ...
  counts = combineLatest([
    ...
    const details = await Promise.all([
      lastValueFrom(jc(item.id).pipe(map((d) => d.total))).catch((_2) => 0),
      lastValueFrom(Eu(item.id).pipe(map((d) => d.length))).catch((_2) => 0)
    ]);
...
//# sourceMappingURL=chunk-GB2GGHH6.js.map
