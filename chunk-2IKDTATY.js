...
var StaffTenantModalComponent = class _StaffTenantModalComponent {
  ...
  async save() {
    ...
    const data = ni(__spreadValues(__spreadValues({}, this.tenant || {}), value), ["", null, void 0]);
    const call = this.tenant?.id ? ae(`/api/staff/v1/tenants/${this.tenant.id}`, data) : S("/api/staff/v1/tenants", data);
...
var PlaceStaffAPIComponent = class _PlaceStaffAPIComponent {
  ...
  async removeTenant(tenant) {
    ...
    const system = await ee(`/api/staff/v1/tenants/${tenant.id}`).catch((err) => {
  ...
  async loadTenants() {
    ...
    const tenants = await p("/api/staff/v1/tenants").catch(() => []);
...
//# sourceMappingURL=chunk-2IKDTATY.js.map
