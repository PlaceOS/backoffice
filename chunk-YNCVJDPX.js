...
var DriverUpdateListModalComponent = class _DriverUpdateListModalComponent {
  ...
  async updateDrivers() {
    ...
    await Promise.all(selected.map((driver) => driver.commit !== driver.update_info.commit ? qu(driver.id, __spreadProps(__spreadValues({}, driver), {
...
var DriverStateService = class _DriverStateService {
  ...
  updates_available = this._poll.pipe(switchMap(() => Tu({ update_available: true, limit: 1 }).pipe(catchError(() => of({ data: [], total: 0 })))), map((d) => d.total > 1), shareReplay(1));
  modules = this.item.pipe(switchMap(async (item) => {
    if (!item)
      return { data: [] };
    this._loading.next(true);
    const details = await ic({
      driver_id: item.id
    }).toPromise();
  ...
  docs = this.item.pipe(debounceTime(100), switchMap(async (item) => {
    if (!item)
      return "";
    this._loading.next(true);
    const docs = await lastValueFrom(Nu(item.id)).catch(() => "");
    this._loading.next(false);
    return docs;
  ...
  async updateDriver() {
    ...
    const success = await qu(item.id, __spreadProps(__spreadValues({}, item), {
  ...
  async recompileDriver() {
    ...
    const success = await Mu(item.id).toPromise().catch(() => false);
    if (success === false) {
  ...
  async reloadDriver() {
    ...
    const success = await Cu(item.id).toPromise().catch(() => false);
    if (success === false) {
  ...
  async removeModule(device) {
    ...
    if (!details || !details.reason)
      return;
    const system = await na(this.active_item.id, device.id).toPromise().catch((err) => {
...
//# sourceMappingURL=chunk-YNCVJDPX.js.map
