...
@Component({
    ...
    template: `
        ...
        <header class="flex items-center my-4">
            <button mat-icon-button [disabled]="!template_system" (click)="editCondition()">
                <app-icon className="backoffice-plus"></app-icon>
            </button>
            <div class="font-medium text-lg" i18n="@@triggerConditionsHeader">Conditions</div>
        </header>
        <section
            role="table"
            *ngIf="comparisons.length || time_dependents.length; else no_conditions"
        >
            <div table-head i18n="@@triggerComparisonLabel" *ngIf="comparisons.length">
            ...
            <div table-body>
                <div table-row *ngFor="let comparison of comparisons">
                    ...
                    <div class="w-24 flex items-center justify-center">
                        ...
                        <button mat-icon-button (click)="removeCondition(comparison)">
                            <app-icon [icon]="{ class: 'backoffice-trash' }"></app-icon>
                        </button>
                    </div>
                </div>
            </div>
            <div table-head i18n="@@triggerTimeLabel" *ngIf="time_dependents.length">
        ...
        <header class="flex items-center space-x-2 my-4">
            <button mat-icon-button [disabled]="!template_system" (click)="editAction()">
                <app-icon className="backoffice-plus"></app-icon>
            </button>
            <div class="font-medium text-lg" i18n="@@triggerActionsHeader">Actions</div>
        </header>
        <section role="table" *ngIf="functions.length || mailers.length; else no_actions">
            <div table-head i18n="@@triggerFunctionsLabel" *ngIf="functions.length">
                <div class="flex-1 p-2">Function Call Actions</div>
            </div>
            <div table-body cdkDropList (cdkDropListDropped)="confirmReorder('function', $event)">
                <div table-row *ngFor="let action of functions" cdkDrag>
                    <div class="w-12 flex items-center justify-center" cdkDragHandle>
            ...
            <div table-body cdkDropList (cdkDropListDropped)="confirmReorder('mailer', $event)">
                <div table-row *ngFor="let action of mailers" cdkDrag>
                    ...
                    <div class="flex-1 p-2">
                        <span [matTooltip]="action.emails | formatList" i18n="@@emailCountDisplay"
                            >{{ action.emails.length }} { action.emails.length, plural, =1 { Address
        ...
        <ng-template #no_conditions>
            <div class="flex flex-col items-center p-8" i18n="@@triggerConditionsEmpty">
                No condtions for trigger
            </div>
        </ng-template>
        <ng-template #no_actions>
            <div class="flex flex-col items-center p-8" i18n="@@triggerActionsEmpty">
...
export class TriggerAboutComponent extends BaseClass {
    ...
    public readonly editCondition = (c?) => this._service.editCondition(c, this.template_system);
    public readonly removeCondition = (c) => this._service.removeCondition(c);
    public readonly editAction = (a?) => this._service.editAction(a, this.template_system);
    ...
    public confirmReorder(type: 'function' | 'mailer', event: CdkDragDrop<any[]>): void {
        if (event && event.previousIndex !== event.currentIndex) {
            this._service.reorderAction(type, event.previousIndex, event.currentIndex);
        }
    }
}
