...
import { copyToClipboard, unique } from 'src/app/common/general';
import { notifyInfo } from 'src/app/common/notifications';
import { UploadDetails, uploadFile } from 'src/app/common/uploads';
...
@Component({
    ...
    template: `
        ...
        >
            ...
            >
                <app-icon class="text-4xl" className="backoffice-plus"></app-icon>
                <p>Upload Image(s)</p>
                <input
                    type="file"
            ...
            >
                <div overlay class="absolute inset-0 hover:bg-black hover:bg-opacity-50 text-white">
                    <div
                        actions
            ...
            >
                ...
                >
                    <app-icon class="text-3xl opacity-0" className="backoffice-retweet"></app-icon>
                </div>
            </div>
            <button
                mat-icon-button
            ...
            if (files.length) {
                for (let i = 0; i < files.length; i++) {
                    uploadFile(files[i]).subscribe(
                        (details) =>
                            (this.upload_list = [
                                ...this.upload_list.filter((_) => _.id !== details.id),
                                details,
                            ]),
                        (details) =>
                            (this.upload_list = [
