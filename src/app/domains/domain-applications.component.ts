...
@Component({
    ...
    template: `
        <button mat-button class="w-full sm:w-40 mb-4" (click)="newApplication()" i18n="@@newAction">New Application</button>
        <div role="table" class="overflow-x-auto" *ngIf="(applications | async)?.length; else empty_state">
            <div table-head>
                <div class="w-40 p-2" i18n="@@applicationTableName">Name</div>
                <div class="flex-1 p-2" i18n="@@applicationTableRedirect">Redirect</div>
                <div class="w-48 p-2" i18n="@@applicationTableClientId">Client ID</div>
                <div class="w-48 p-2" i18n="@@applicationTableClientSecret">Secret</div>
                <div class="w-24 p-2" i18n="@@applicationTableScope">Scope</div>
                <div class="w-24"></div>
            </div>
            <div table-body>
                <div table-row *ngFor="let item of applications | async">
                    <div class="w-40 p-2" [matTooltip]="item.id">{{ item.name }}</div>
                    ...
                    <div class="w-24 flex items-center justify-center">
                        ...
                        <button mat-icon-button (click)="deleteApplication(item)">
...
export class DomainApplicationsComponent {
    ...
    public readonly editApplication = (item) => this._service.editApplication(item);
    public readonly deleteApplication = (item) => this._service.deleteApplication(item);
