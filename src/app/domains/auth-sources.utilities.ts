 ...
 */
export function generateOAuthSourceForm(auth_source?: PlaceOAuthSource) {
    const fields = {
        ...
        auth_scheme: new FormControl(
            auth_source?.auth_scheme || 'request_body',
        ),
...
export function generateSAMLSourceForm(
    ...
    const fields: HashMap<FormControl> = {
        ...
        idp_sso_target_url: new FormControl(
            auth_source.idp_sso_target_url || '',
            [Validators.required],
        ),
        name_identifier_format: new FormControl(
            auth_source.name_identifier_format || '',
        ),
        assertion_consumer_service_url: new FormControl(
            auth_source.assertion_consumer_service_url || '',
            [Validators.required],
        ),
        request_attributes: new FormControl(
            auth_source.request_attributes || [],
        ),
        ...
        idp_cert_fingerprint: new FormControl(
            auth_source.idp_cert_fingerprint || '',
        ),
        attribute_service_name: new FormControl(
            auth_source.attribute_service_name || '',
        ),
        attribute_statements: new FormControl(
            auth_source.attribute_statements || {},
        ),
        idp_slo_target_url: new FormControl(
            auth_source.idp_slo_target_url || '',
        ),
        slo_default_relay_state: new FormControl(
            auth_source.slo_default_relay_state || '',
        ),
    };
    return new FormGroup(fields);
}
