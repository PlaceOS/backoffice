    ...
    template: `
        <div role="table" *ngIf="logs?.length; else empty_state">
            <div table-head>
                ...
                <div class="w-1/3" i18n="@@logEventAction">Systems Accessed</div>
            </div>
            <div table-body>
                <div table-row *ngFor="let item of logs">
                    <div class="w-1/3">{{ item.start | date: 'MMM d, y, h:mm a' }}</div>
                    <div class="w-1/3">{{ item.end | date: 'MMM d, y, h:mm a' }}</div>
                    <div class="w-1/3">
        ...
        <ng-template #empty_state>
            <div class="p-8 text-center" i18n="@@logTableEmpty">No logs found</div>
        </ng-template>
    `,
    styles: [`
        :host {
