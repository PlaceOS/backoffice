...
var TriggerActionModalComponent = class _TriggerActionModalComponent extends AsyncHandler {
  ...
  save() {
    ...
    Pa(this.trigger.id, __spreadProps(__spreadValues({}, this.trigger), {
...
var TriggerConditionComparisonFormComponent = class _TriggerConditionComparisonFormComponent {
  ...
  loadSystemStatusVariables(mod_name, side) {
    ...
    if (!name[0]?.length)
      return;
    pa(this.system().id, name.length > 1 ? name.slice(0, name.length - 1).join("_") : name[0], +name[name.length - 1] || 1).subscribe((var_map) => {
  ...
  loadSystemModules() {
    ...
    hc({ control_system_id: system.id }).pipe(map((resp) => resp.data)).subscribe((module_list) => {
...
var TriggerConditionTimeFormComponent = class _TriggerConditionTimeFormComponent extends AsyncHandler {
  ...
  toggleCRON(is_cron) {
    ...
    this.form().controls.time_type.setValue(is_cron ? lo.CRON : lo.AT);
...
var TriggerConditionModalComponent = class _TriggerConditionModalComponent extends AsyncHandler {
  ...
  async save() {
    ...
    const item = await Pa(this.trigger.id, __spreadProps(__spreadValues({}, this.trigger), {
...
var TriggerStateService = class _TriggerStateService {
  ...
  async reorderAction(type, fst, snd) {
    ...
    const resp = await Pa(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item.toJSON()), {
      actions
    })).toPromise().catch((_) => _);
    if (!(resp instanceof me)) {
      const error = resp;
  ...
  async removeCondition(condition) {
    ...
    list.splice(index, 1);
    const resp = await Pa(item.id, __spreadProps(__spreadValues({}, item.toJSON()), {
      conditions
  ...
  async removeAction(action) {
    ...
    const resp = await Pa(item.id, __spreadProps(__spreadValues({}, item.toJSON()), { actions })).toPromise().catch((err) => err);
    details.close();
    if (!(resp instanceof me)) {
      const error = resp;
...
//# sourceMappingURL=chunk-YY5UV4F3.js.map
