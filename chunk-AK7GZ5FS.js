...
var BulkItemModalComponent = class _BulkItemModalComponent {
  ...
  generateTemplate() {
    switch (this._data.constr) {
      ...
      case uo:
        return [new uo(USER_TEMPLATE).toJSON()];
      case Un:
        return [new Un(ZONE_TEMPLATE).toJSON()];
    }
  }
...
var domains = {
  ...
  show: (_) => Au(_),
  save: (item) => item.id ? wu(item.id, item) : Ou(item),
  remove: (item) => Iu(item.id),
  itemConstructor: Js,
  delete_message: ``,
  name: "DOMAINS"
};
var drivers = {
  ...
  show: (_) => Eu(_),
  save: (item) => item.id ? qu(item.id, item) : Uu(item),
  remove: (item) => Ru(item.id),
  itemConstructor: Hr,
  delete_message: ``,
  delete_extra: async (_) => {
    ...
    const count = await ic(query).pipe(map(({ total }) => total)).toPromise().catch((_err) => 0);
    return count ? [
      "error",
...
var repositories = {
  ...
  show: (_) => $c(_),
  save: (item) => item.id ? xc(item.id, item) : Ac(item),
  remove: (item) => wc(item.id),
  itemConstructor: no,
  delete_message: `'`,
  name: "REPOS"
};
var systems = {
  ...
  show: (_) => Kc(_),
  save: (item) => item.id ? Zc(item.id, __spreadProps(__spreadValues({}, item), {
    support_url: processURL(item, item.support_url)
  })) : Xc(__spreadProps(__spreadValues({}, item), {
    support_url: processURL(item, item.support_url)
  })),
...
var triggers = {
  ...
  show: (_) => ba(_),
  save: (item) => item.id ? va(item.id, item) : Sa(item),
...
var users = {
  ...
  show: (_) => Aa(_),
  save: (item) => item.id ? Oa(item.id, item) : Ia(item),
...
var zones = {
  ...
  show: (_) => ja(_),
  save: (item) => item.id ? Fa(item.id, item) : za(item),
  remove: (item) => Ha(item.id),
  itemConstructor: Un,
  delete_message: ``,
  name: "ZONES"
};
...
var ActiveItemService = class _ActiveItemService extends AsyncHandler {
  ...
  async updateSettings() {
    ...
    if (item && item.settings) {
      let settings = await lastValueFrom(Wc({ parent_id: item.id }).pipe(map((resp) => resp.data)));
      settings = new Array(5).fill(0).map((_, idx) => settings.find((_2) => _2.encryption_level === idx) || new Ie({
...
//# sourceMappingURL=chunk-AK7GZ5FS.js.map
