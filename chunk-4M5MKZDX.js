  ...
  _processItems(list) {
    for (const item of list) {
      if (item instanceof ks) {
        ...
        const detail = item.role === Tt.Service ? item.uri : item.role === Tt.Logic ? name ? `${name} | ${item.control_system_id} ` : item.control_system_id : item.ip;
        item.display_name = item.custom_name || item.name || "<Unnamed>";
        item.extra = detail;
      } else if (item instanceof Zi) {
        item.display_name = item.name || "<Unnamed>";
        item.extra = item.repo_type;
      } else if (item instanceof vs) {
        item.display_name = item.display_name || item.name || "<Unnamed>";
        item.zone_issues = (item.email || item.map_id) && item.zones.length < 3 ? "system" : "";
      } else if (item instanceof Qt) {
        item.display_name = item.display_name || item.name || "<Unnamed>";
...
//# sourceMappingURL=chunk-4M5MKZDX.js.map
