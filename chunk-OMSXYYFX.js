...
function generateBasicHandlers(endpoint, data, filter = (_, _q) => true) {
  ...
  tu(action);
  tu(__spreadProps(__spreadValues({}, action), { method: "PUT" }));
  tu({
    path: `${endpoint}/:id`,
    metadata: data,
    method: "DELETE",
...
tu({
  ...
  metadata: REPO_DATA,
  method: "GET",
  callback: (_) => REPO_DATA.filter((repo) => repo.type === Yr.Interface).map((repo) => repo.name)
});
...
//# sourceMappingURL=chunk-OMSXYYFX.js.map
