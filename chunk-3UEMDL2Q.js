...
function generateBasicHandlers(endpoint, data, filter = (_, _q) => true) {
  ...
  uo(action);
  uo(__spreadProps(__spreadValues({}, action), { method: "PUT" }));
  uo({
    path: `${endpoint}/:id`,
    metadata: data,
    method: "DELETE",
...
uo({
  ...
  metadata: REPO_DATA,
  method: "GET",
  callback: (_) => REPO_DATA.filter((repo) => repo.type === Ms.Interface).map((repo) => repo.name)
});
...
//# sourceMappingURL=chunk-3UEMDL2Q.js.map
