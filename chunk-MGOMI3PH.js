...
var BulkItemModalComponent = class _BulkItemModalComponent {
  ...
  generateTemplate() {
    switch (this._data.constr) {
      case qs:
        return [new qs(SYSTEM_TEMPLATE).toJSON()];
      case Ps:
        return [new Ps(MODULE_TEMPLATE).toJSON()];
      case vs:
        return [new vs(DRIVER_TEMPLATE).toJSON()];
      case Mn:
        return [new Mn(USER_TEMPLATE).toJSON()];
      case Yt:
        return [new Yt(ZONE_TEMPLATE).toJSON()];
    }
  }
  getRequiredFields() {
    switch (this._data.constr) {
  ...
  findDuplicates() {
    ...
    const dedup_field = this._data.constr === Mn ? "email" : "name";
    this.item_list.forEach((item, index) => {
...
var DriverFormComponent = class _DriverFormComponent extends AsyncHandler {
  ...
  _repo_list = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_repo_list" } : (
    ...
    try {
      const { data } = await Rc({ limit: 1e3 });
      return data.filter((repo) => repo.type === Us.Driver);
    } finally {
  ...
  async applyDriverCommit(commit) {
    ...
    const details = await Lc(repo.id, {
      driver: `${driver.id}`,
      commit: `${commit.id}`
    }).catch(() => null);
  ...
  async _loadDetailsFromForm() {
    ...
    if (!id)
      return;
    this.loading.set("DRIVERS.DETAILS_LOADING");
    const repo = await Uc(repository_id);
    const driver = {
      id: file_name,
...
var applyModuleFormSchema = (path) => {
  ...
  min(path.port, 1, {
    when({ valueOf }) {
      ...
      return role === Ct.Device || role === Ct.SSH;
    }
  });
  max(path.port, 65535, {
    when({ valueOf }) {
  ...
  required(path.uri, {
    when({ valueOf }) {
      const role = valueOf(path.role);
      return role === Ct.Service || role === Ct.Websocket;
    }
  });
  required(path.ip, {
    when({ valueOf }) {
  ...
  required(path.system, {
    when({ valueOf }) {
      return valueOf(path.role) === Ct.Logic;
    }
  });
};
...
var ModuleFormComponent = class _ModuleFormComponent extends AsyncHandler {
  ...
  driver_query_fn = (_) => Wo({ q: _ }).then((resp) => resp.data);
  system_query_fn = (_) => ha({ q: _ }).then((resp) => resp.data);
  edge_query_fn = (_) => tu({ q: _ }).then((resp) => resp.data);
  ...
  ngOnInit() {
    ...
    if (control_system_id && !system) {
      da(control_system_id).then((sys) => this.formModel.update((v) => __spreadProps(__spreadValues({}, v), { system: sys })), () => null);
    }
  }
  constructor() {
    super();
    ...
    effect(() => {
      ...
      if (!driver?.id)
        return;
      const role = driver.role || Ct.Logic;
      const udp = driver.role === Ct.Service || driver.role === Ct.Websocket ? false : model2.udp;
      const system = driver.role === Ct.Logic ? model2.system : void 0;
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      if (form_value.role !== Ct.Logic) {
        delete form_value.control_system_id;
      }
      const form_item = item.id ? ni(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? oc(form_item.id, form_item) : uc(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ModuleFormComponent, { className: "ModuleFormComponent", filePath: "src/app/modules/module-form.component.ts", lineNumber: 340 });
})();
...
var SystemFormComponent = class _SystemFormComponent extends AsyncHandler {
  ...
  query_fn = (_) => eh({ q: _ }).then((resp) => resp.data);
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      this._dialog_ref.disableClose = true;
      const item_json = item.toJSON ? item.toJSON() : item;
      const _a2 = this.formModel(), { zone: _zone } = _a2, form_value = __objRest(_a2, ["zone"]);
      const form_item = item.id ? ni(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      const processed_item = __spreadProps(__spreadValues({}, form_item), {
        support_url: this.processURL(form_item, form_item.support_url || "")
      });
      try {
        const _item = await (processed_item.id ? pa(processed_item.id, processed_item) : fa(processed_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
var ZoneFormComponent = class _ZoneFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      this._dialog_ref.disableClose = true;
      const item_json = item.toJSON ? item.toJSON() : item;
      const _a2 = this.formModel(), { parent_zone: _parent_zone } = _a2, form_value = __objRest(_a2, ["parent_zone"]);
      const form_item = item.id ? ni(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? sh(form_item.id, form_item) : ih(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
async function zoneSubtreeIds(zone_id) {
  ...
  while (level.length) {
    ...
    const children = await collectPages(eh({ parent_id: level.join(","), limit: PAGE_SIZE }));
    level = [];
...
async function splitZoneSystems(zone_ids) {
  ...
  const pages = await mapLimit(zone_ids, READ_CONCURRENCY, (zone_id) => collectPages(ha({ zone_id, limit: PAGE_SIZE })));
  ...
  const confirmed = await mapLimit(candidates, READ_CONCURRENCY, (system) => da(system.id).catch(() => null));
  const orphaned = [];
...
async function domainTenants(domain) {
  if (!domain)
    return [];
  const tenants = await p("/api/staff/v1/tenants");
...
async function planDomainCascade(domain) {
  ...
  const [applications, tenants, all_domains] = await Promise.all([
    collectPages(mo({ authority_id: domain.id, limit: PAGE_SIZE })),
    domainTenants(domain.domain),
    org_zone_id ? collectPages(zo({ limit: PAGE_SIZE })) : Promise.resolve([])
  ]);
  ...
  if (tenants.length) {
    ...
    plan.steps.push(...tenants.map((tenant) => ({
      ...
      run: () => ee(`/api/staff/v1/tenants/${tenant.id}`)
    })));
  }
  if (!org_zone_id) {
  ...
  const org_zone = await nh(org_zone_id).catch(() => null);
  if (!org_zone) {
...
var domains = {
  ...
  show: (_) => Fo(_),
  save: (item) => item.id ? Lo(item.id, item) : jo(item),
  remove: (item) => Go(item.id),
  itemConstructor: Qi,
...
var drivers = {
  ...
  delete_extra: async (_) => {
    ...
    const count = await ic(query).then(({ total }) => total).catch((_err) => 0);
    return count ? [
      "error",
...
var groups = {
  ...
  show: (_) => Su(_),
  save: (item) => item.id ? qu(item.id, item) : Au(item),
  remove: (item) => Pu(item.id),
  itemConstructor: On,
...
var modules = {
  ...
  show: (_) => rc(_),
  save: (item) => item.id ? oc(item.id, item) : uc(item),
  remove: (item) => cc(item.id),
  itemConstructor: Ps,
...
var repositories = {
  ...
  show: (_) => Uc(_),
  save: (item) => item.id ? Ic(item.id, item) : Ec(item),
  remove: (item) => Mc(item.id),
  itemConstructor: nr,
...
async function saveSystem(item) {
  const _a2 = item, { modules: modules2, start_modules } = _a2, system_data = __objRest(_a2, ["modules", "start_modules"]);
  ...
  if (item.id)
    return pa(item.id, form_data);
  const system = await fa(form_data);
...
async function addLogicDriverModules(system, driver_ids) {
  ...
  await Promise.all([...new Set(driver_ids)].map(async (driver_id) => {
    const driver = await Qo(driver_id);
    if (driver.role !== Ct.Logic)
      return;
    await uc({
      driver_id: driver.id,
...
var triggers = {
  ...
  show: (_) => Ca(_),
  save: (item) => item.id ? wa(item.id, item) : Na(item),
  remove: (item) => Da(item.id),
  itemConstructor: de,
...
var users = {
  ...
  show: (_) => La(_),
  save: (item) => item.id ? Ga(item.id, item) : Ba(item),
  remove: (item) => Wa(item.id),
  itemConstructor: Mn,
...
var zones = {
  ...
  show: (_) => nh(_),
  save: (item) => item.id ? sh(item.id, item) : ih(item),
  remove: (item) => rh(item.id),
  itemConstructor: Yt,
...
var ActiveItemService = class _ActiveItemService extends AsyncHandler {
  ...
  async updateSettings() {
    ...
    if (item && item.settings) {
      let settings = (await ia({ parent_id: item.id })).data;
...
//# sourceMappingURL=chunk-MGOMI3PH.js.map
