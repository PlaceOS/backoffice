...
var DriverUpdateListModalComponent = class _DriverUpdateListModalComponent {
  ...
  async updateDrivers() {
    ...
    await Promise.all(selected.map((driver) => driver.commit !== driver.update_info.commit ? Co(driver.id, __spreadProps(__spreadValues({}, driver), {
...
var DriverStateService = class _DriverStateService {
  ...
  async updateDriver() {
    ...
    const success = await Co(item.id, __spreadProps(__spreadValues({}, item), {
  ...
  async recompileDriver() {
    ...
    await Mo(item.id).catch(async (e) => {
      console.log("Error:", e);
  ...
  async reloadDriver() {
    ...
    const success = await Do(item.id).catch(() => false);
    if (success === false) {
...
//# sourceMappingURL=chunk-EAXPA4HE.js.map
