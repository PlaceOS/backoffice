import {
  UserPipe
} from "./chunk-7YOPAQU7.js";
import {
  SettingsFieldComponent
} from "./chunk-HFVGZQWU.js";
import {
  DiffViewerComponent
} from "./chunk-PPSBIOLW.js";
import {
  validateJSONString
...
var MetadataHistoryModalComponent = class _MetadataHistoryModalComponent {
  ...
  async ngOnInit() {
    const history = await Yu(this._data.id, {
      name: this._data.name,
      limit: 5e3
    });
...
var MetadataDisplayComponent = class _MetadataDisplayComponent extends AsyncHandler {
  ...
  async deleteMetadata(field) {
    ...
    if (result.reason !== "done")
      return;
    await tc(this.item().id, { name: field }).catch((err) => {
  ...
  saveMetadata(field) {
    ...
    Xu(this.item().id, __spreadProps(__spreadValues({}, value), {
      ...
      if (field.name !== value.name) {
        tc(this.item().id, { name: field.name }).catch((err) => notifyError(`Error removing old "${field.name}" metadata. Error: ${JSON.stringify(err.response || err.message || err)}`));
      }
      if (index >= 0) {
        this.metadata.update((l) => {
  ...
  async loadMetadata() {
    ...
    await Ju(this.item().id).then((map) => {
...
//# sourceMappingURL=chunk-UVA3QA25.js.map
