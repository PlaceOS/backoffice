...
var ModuleFormComponent = class _ModuleFormComponent extends AsyncHandler {
  ...
  ngOnInit() {
    ...
    if (control_system_id && !system) {
      va(control_system_id).then((sys) => this.formModel.update((v) => __spreadProps(__spreadValues({}, v), { system: sys })), () => null);
    }
  }
  constructor() {
    super();
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      if (form_item.role !== Nt.Logic) {
        delete form_item.control_system_id;
      }
      try {
        const _item = await (form_item.id ? fc(form_item.id, form_item) : _c(form_item));
...
//# sourceMappingURL=chunk-IJ4I7GGV.js.map
