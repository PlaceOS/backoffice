...
var BulkItemModalComponent = class _BulkItemModalComponent {
  ...
  generateTemplate() {
    switch (this._data.constr) {
      case vs:
        return [new vs(SYSTEM_TEMPLATE).toJSON()];
      case ks:
        return [new ks(MODULE_TEMPLATE).toJSON()];
      case ms:
        return [new ms(DRIVER_TEMPLATE).toJSON()];
      case qn:
        return [new qn(USER_TEMPLATE).toJSON()];
      case Qt:
        return [new Qt(ZONE_TEMPLATE).toJSON()];
    }
  }
  getRequiredFields() {
    switch (this._data.constr) {
  ...
  findDuplicates() {
    ...
    const dedup_field = this._data.constr === qn ? "email" : "name";
    this.item_list.forEach((item, index) => {
...
var DomainFormComponent = class _DomainFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (form_item.id ? To(form_item.id, form_item) : Eo(form_item));
  ...
  async newSettings(item, settings_string) {
    const new_settings = new Ae({
      parent_id: item.id,
      settings_string,
      encryption_level: De.Support
    });
    await Jc(new_settings).catch((err) => {
      this.loading.set(null);
...
var DriverFormComponent = class _DriverFormComponent extends AsyncHandler {
  ...
  role_types = [
    { id: Tt.SSH, name: "DRIVERS.SSH" },
    { id: Tt.Device, name: "DRIVERS.DEVICE" },
    { id: Tt.Service, name: "DRIVERS.SERVICE" },
    { id: Tt.Websocket, name: "DRIVERS.WEBSOCKET" },
    { id: Tt.Logic, name: "DRIVERS.LOGIC" }
  ];
  alert_levels = [
  ...
  _repo_list = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_repo_list" } : (
    ...
    try {
      const { data } = await yc({ limit: 1e3 });
      return data.filter((repo) => repo.type === As.Driver);
    } finally {
  ...
  async applyDriverCommit(commit) {
    ...
    const details = await Tc(repo.id, {
      driver: `${driver.id}`,
      commit: `${commit.id}`
    }).catch(() => null);
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Js(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? wo(form_item.id, form_item) : Do(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({
          reason: "done",
          metadata: { item: _item }
  ...
  _applyDriverDetails(details) {
    ...
    this.formModel.update((value) => __spreadProps(__spreadValues({}, value), {
      ...
      role: port_number ? port_number === 22 ? Tt.SSH : Tt.Device : details_any.uri_base ? details_any.uri_base.startsWith("ws") ? Tt.Websocket : Tt.Service : Tt.Logic,
  ...
  async _loadDetailsFromForm() {
    ...
    if (!id)
      return;
    this.loading.set("DRIVERS.DETAILS_LOADING");
    const repo = await $c(repository_id);
    const driver = {
      id: file_name,
...
function generateGroupFormModel(group) {
  return {
    ...
    authority_id: group?.authority_id || Pt()?.id || "",
...
var GroupFormComponent = class _GroupFormComponent extends AsyncHandler {
  ...
  _domain_list = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_domain_list" } : (
    /* istanbul ignore next */
    {}
  )), { loader: async () => (await Uo({ limit: 1e3 })).data }));
  ...
  query_parent_groups = (_) => lu({ q: _, limit: 20 }).then(({ data }) => data);
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = Js(__spreadProps(__spreadValues(__spreadValues({}, item), form_value), {
        subsystems: form_value.subsystems || []
      }), ["", void 0]);
      try {
        const _item = await (form_item.id ? _u(form_item.id, form_item) : pu(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
  ...
  async loadParentGroup() {
    ...
    if (!parent_id)
      return;
    const parent = await fu(parent_id).catch(() => null);
...
var applyModuleFormSchema = (path) => {
  ...
  required(path.uri, {
    when({ valueOf }) {
      ...
      return role === Tt.Service || role === Tt.Websocket;
    }
  });
  required(path.ip, {
    when({ valueOf }) {
      const role = valueOf(path.role);
      return role === Tt.Device || role === Tt.SSH;
    }
  });
  required(path.port, {
    when({ valueOf }) {
  ...
  required(path.system, {
    when({ valueOf }) {
      return valueOf(path.role) === Tt.Logic;
    }
  });
};
...
var ModuleFormComponent = class _ModuleFormComponent extends AsyncHandler {
  ...
  driver_query_fn = (_) => Mo({ q: _ }).then((resp) => resp.data);
  system_query_fn = (_) => Xc({ q: _ }).then((resp) => resp.data);
  edge_query_fn = (_) => Go({ q: _ }).then((resp) => resp.data);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (!driver?.id)
        return;
      const role = driver.role || Tt.Logic;
      const udp = driver.role === Tt.Service || driver.role === Tt.Websocket ? false : model2.udp;
      const system = driver.role === Tt.Logic ? model2.system : void 0;
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Js(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? Zu(form_item.id, form_item) : Ju(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
...
function generateRepositoryFormModel(repository) {
  return {
    ...
    uri: repository?.uri || "",
    repo_type: repository?.repo_type || As.Driver,
...
var RepositoryFormComponent = class _RepositoryFormComponent extends AsyncHandler {
  ...
  is_interface = computed(
    () => this.formModel().repo_type === As.Interface,
  ...
  ngOnInit() {
    ...
    this.heading.set(i18n(`${this._name}.${edit ? "EDIT" : "NEW"}`));
    this.repo_types.set([
      { id: As.Driver, name: i18n("REPOS.TYPE_DRIVER") },
      {
        id: As.Interface,
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Js(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? bc(form_item.id, form_item) : vc(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
  ...
  async _loadDefaultBranch() {
    const { id, uri, username, password } = this.formModel();
    return (id ? Ic(id) : isValidUrl(uri) && uri.startsWith("http") ? xc({
      repository_url: uri,
      username,
      password
  ...
  async _loadCommits() {
    ...
    try {
      const list = await (id ? Rc(id, { branch }) : qc({
        repository_url: uri,
        username,
        password,
        branch
...
var SystemFormComponent = class _SystemFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (processed_item.id ? na(processed_item.id, processed_item) : sa(processed_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
function generateTriggerConditionFormModel(condition = {}) {
  ...
  return {
    ...
    operator: condition.operator || Yi.EQ,
    right: right || void 0,
...
var TriggerFormComponent = class _TriggerFormComponent extends AsyncHandler {
  ...
  async submit() {
    ...
    try {
      const _item = await (form_item.id ? Aa(form_item.id, form_item) : qa(form_item));
      this._dialog_ref.disableClose = false;
      this.event.emit({ reason: "done", metadata: { item: _item } });
      notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
      this._dialog_ref.close();
...
var UserFormComponent = class _UserFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (form_item.id ? Ca(form_item.id, form_item) : Oa(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
var ZoneFormComponent = class _ZoneFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Js(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? Wa(form_item.id, form_item) : Qa(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
var domains = {
  ...
  show: (_) => Io(_),
  save: (item) => item.id ? To(item.id, item) : Eo(item),
  remove: (item) => Co(item.id),
  itemConstructor: zi,
...
var drivers = {
  ...
  show: (_) => No(_),
  save: (item) => item.id ? wo(item.id, item) : Do(item),
  remove: (item) => Ho(item.id),
  itemConstructor: ms,
  ...
  delete_extra: async (_) => {
    ...
    const count = await Qu(query).then(({ total }) => total).catch((_err) => 0);
    return count ? [
      "error",
...
var groups = {
  ...
  show: (_) => fu(_),
  save: (item) => item.id ? _u(item.id, item) : pu(item),
...
var modules = {
  ...
  show: (_) => Ku(_),
  save: (item) => item.id ? Zu(item.id, item) : Ju(item),
  remove: (item) => Yu(item.id),
  itemConstructor: ks,
...
var repositories = {
  ...
  show: (_) => $c(_),
  save: (item) => item.id ? bc(item.id, item) : vc(item),
  remove: (item) => kc(item.id),
  itemConstructor: Zi,
...
async function saveSystem(item) {
  ...
  if (item.id)
    return na(item.id, form_data);
  const system = await sa(form_data);
...
async function addLogicDriverModules(system, driver_ids) {
  ...
  await Promise.all([...new Set(driver_ids)].map(async (driver_id) => {
    const driver = await No(driver_id);
    if (driver.role !== Tt.Logic)
      return;
    await Ju({
      driver_id: driver.id,
...
var triggers = {
  ...
  show: (_) => xa(_),
  save: (item) => item.id ? Aa(item.id, item) : qa(item),
  remove: (item) => Pa(item.id),
  itemConstructor: he,
...
var users = {
  ...
  show: (_) => Ta(_),
  save: (item) => item.id ? Ca(item.id, item) : Oa(item),
  remove: (item) => Ma(item.id),
  itemConstructor: qn,
...
var zones = {
  ...
  show: (_) => Ba(_),
  save: (item) => item.id ? Wa(item.id, item) : Qa(item),
...
var ActiveItemService = class _ActiveItemService extends AsyncHandler {
  ...
  async updateSettings() {
    ...
    if (item && item.settings) {
      let settings = (await Qc({ parent_id: item.id })).data;
...
//# sourceMappingURL=chunk-2HQXE7CG.js.map
