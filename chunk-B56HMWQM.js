...
var BookingLimitsModalComponent = class _BookingLimitsModalComponent {
  ...
  async save() {
    ...
    const call = Y(`/api/staff/v1/tenants/${this.tenant.id}/limits`, booking_limits);
...
var StaffTenantModalComponent = class _StaffTenantModalComponent {
  ...
  async save() {
    ...
    const data = ds(__spreadProps(__spreadValues(__spreadValues({}, this.tenant || {}), value), {
      booking_limits
    }), ["", null, void 0]);
    const call = this.tenant?.id ? Ne(`/api/staff/v1/tenants/${this.tenant.id}`, data) : Y("/api/staff/v1/tenants", data);
...
var PlaceStaffAPIComponent = class _PlaceStaffAPIComponent {
  ...
  async ngOnInit() {
    ...
    const domain_list = await Tu().pipe(map((r) => r.data)).toPromise();
  ...
  async removeTenant(tenant) {
    ...
    const system = await Pe(`/api/staff/v1/tenants/${tenant.id}`).toPromise().catch((err) => {
  ...
  async loadTenants() {
    ...
    const tenants = await N("/api/staff/v1/tenants").toPromise().catch(() => []);
...
//# sourceMappingURL=chunk-B56HMWQM.js.map
