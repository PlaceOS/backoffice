  ...
  _associated_settings = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_associated_settings" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof qs))
        return [];
      return isSubsystemUser() ? ia({ parent_id: item.id }).then((response) => response.data).catch(() => []) : Ra(item.id).catch(() => []);
    }
  }));
    ...
    loader: async ({ params }) => {
      ...
      try {
        const response = isSubsystemUser() ? {
          data: await Promise.all(item.zones.map((id) => nh(id)))
        } : await Aa(item.id).catch(() => ({
          data: []
        }));
...
//# sourceMappingURL=chunk-K2I63XXT.js.map
