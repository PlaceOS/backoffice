...
} from "./chunk-R44475IX.js";
import {
  ViewResponseModalComponent
} from "./chunk-2QSFLPYH.js";
import {
  PlaceDebugService
} from "./chunk-57NRHP4B.js";
import {
...
function SystemTriggerFormComponent_Conditional_1_Conditional_4_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275repeaterCreate(7, SystemTriggerFormComponent_Conditional_1_Conditional_4_For_8_Template, 4, 2, "mat-chip", 19, _forTrack0);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SystemTriggerFormComponent, { className: "SystemTriggerFormComponent", filePath: "src/app/systems/system-trigger-form.component.ts", lineNumber: 155 });
})();
...
var SystemStateService = class _SystemStateService extends AsyncHandler {
  ...
  counts = combineLatest([
    ...
    const details = await Promise.all([
      lastValueFrom(ya(item.id).pipe(map((d) => d.total))).catch(() => 0),
      lastValueFrom(ic(item.id).pipe(map((d) => d.length))).catch(() => 0)
    ]);
  ...
  zones = this._state.item.pipe(debounceTime(200), switchMap((item) => {
    ...
    return ma(item.id).pipe(map((i) => [item, i.data]), startWith([item, []]));
  }), map(([item, zones]) => {
  ...
  triggers = combineLatest([
    ...
    return ya(item.id).pipe(map((i) => i.data), startWith([]));
  ...
  async startSystem() {
    ...
    if (details && details.reason) {
      ...
      const resp = await lastValueFrom(la(this.active_item.id)).catch((err) => {
  ...
  async stopSystem() {
    ...
    const resp = await lastValueFrom(ha(this.active_item.id)).catch((err) => {
  ...
  async newModule() {
    const mod = await this._state.edit(new Gr({
  ...
  async addModuleToSystem(device) {
    ...
    const ref = this._dialog.open(SelectItemModalComponent, {
      data: {
        ...
        query_fn: (_) => na({ q: _ }).pipe(map((resp) => resp.data.filter((_2) => _2.id !== item.id)))
      }
    });
    ...
    if (!system)
      return;
    await lastValueFrom(ca(system.id, device.id)).catch((_e) => {
      ref.close();
  ...
  async addTrigger(trigger) {
    const t = await lastValueFrom(ga(this.active_item.id, {
      control_system_id: this.active_item.id,
      enabled: true,
      important: false,
  ...
  async removeTrigger(trigger) {
    ...
    if (!details || !details.reason)
      return;
    await lastValueFrom(ba(this.active_item.id, trigger.id)).catch((err) => {
      details.close();
  ...
  async reorderModules(fst, snd) {
    ...
    const resp = await lastValueFrom(sa(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      modules: list
    }))).catch((err) => {
   ...
   */
  async joinModule(id) {
    await lastValueFrom(ca(this.active_item.id, id)).catch((err) => {
      notifyError(`Error adding module ${id} to system. Error: ${err.statusText || err.message || err}`);
    });
    this.timeout("join", async () => {
      const system = await lastValueFrom(ia(this.active_item.id));
      if (system)
  ...
  async removeModule(device) {
    ...
    if (!details || !details.reason)
      return;
    const system = await lastValueFrom(aa(this.active_item.id, device.id)).catch((err) => {
  ...
  async addZones(zone_list) {
    ...
    const system = await lastValueFrom(sa(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      zones
    }))).catch((err) => {
  ...
  async toggleModulePower(device) {
    const method = device.running ? yc : mc;
...
//# sourceMappingURL=chunk-K7NAKAHJ.js.map
