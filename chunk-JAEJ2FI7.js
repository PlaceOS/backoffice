...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(OauthSourceFormComponent, { className: "OauthSourceFormComponent", filePath: "src/app/ui/forms/oauth-source-form.component.ts", lineNumber: 306 });
})();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SamlSourceFormComponent, { className: "SamlSourceFormComponent", filePath: "src/app/ui/forms/saml-source-form.component.ts", lineNumber: 408 });
})();
...
var AuthSourceModalComponent = class _AuthSourceModalComponent extends AsyncHandler {
  ...
  samlFormModel = signal(
    generateSAMLSourceForm(new Wi({ authority_id: this._data.domain.id })),
  ...
  ldapFormModel = signal(
    generateLDAPSourceForm(new Fi({ authority_id: this._data.domain.id })),
  ...
  type = computed(
    () => this.item() instanceof Wi ? "saml" : this.item() instanceof Fi ? "ldap" : "oauth",
  ...
  setType(type) {
    ...
    switch (type) {
      case "saml":
        this.item.set(new Wi(data));
        break;
      case "ldap":
        this.item.set(new Fi(data));
        break;
      default:
  ...
  updateMethod(item) {
    switch (this.type()) {
      case "saml":
        return item.id ? Hc(item.id, item) : zc(item);
      case "ldap":
        return item.id ? Iu(item.id, item) : Cu(item);
    }
    return item.id ? sc(item.id, item) : ic(item);
  }
  /**
...
var ApplicationFormComponent = class _ApplicationFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (save_item.id ? to(save_item.id, save_item) : no(save_item));
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ApplicationFormComponent, { className: "ApplicationFormComponent", filePath: "src/app/domains/application-form.component.ts", lineNumber: 224 });
})();
...
var DomainStateService = class _DomainStateService {
  ...
  _auth_sources = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_auth_sources" } : (
    ...
    loader: async ({ params }) => {
      ...
      const [saml, oauth, ldap] = await Promise.all([
        Nc(q).then((response) => response.data),
        tc(q).then((response) => response.data),
        Uu(q).then((response) => response.data)
      ]).catch(() => [[], [], []]);
  ...
  _counts = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_counts" } : (
    ...
    loader: async ({ params }) => {
      ...
      const details = await Promise.all([
        Xr(q).then((response) => response.total).catch(() => 0),
        Promise.all([
          Nc(q),
          tc(q),
          Uu(q)
        ]).then(([saml, oauth, ldap]) => saml.total + oauth.total + ldap.total).catch(() => 0),
        Aa(q).then((response) => response.total).catch(() => 0)
      ]);
  ...
  async update(domain) {
    const item = await qo(domain.id, domain);
  ...
  async editApplication(item) {
    item = item || new Ui({ owner_id: this.active_item.id });
  ...
  async deleteApplication(item) {
    ...
    const err = await so(item.id).catch((_) => _);
    details.close();
    if (err)
  ...
  async deleteAuthSource(item) {
    ...
    const method = item instanceof Wi ? Fc : item instanceof Bi ? rc : Eu;
...
//# sourceMappingURL=chunk-JAEJ2FI7.js.map
