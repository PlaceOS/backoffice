...
var SettingsFormComponent = class _SettingsFormComponent extends AsyncHandler {
  ...
  constructor() {
    super();
    effect(() => {
      this.encryption_level.set(this.merge() ? Le.NeverDisplay + 1 : Le.None);
    });
    effect(() => {
  ...
  save(level) {
    ...
    (settings[level].id ? _a(settings[level].id, details) : ma(details)).then((new_settings) => {
      this.saving.update((s) => {
        s[level] = false;
        return s;
      });
  ...
  saveAll() {
    ...
    const promises = [];
    for (let i = 0; i < Le.NeverDisplay + 1; i++) {
      const settings = this.settings();
      if (settings[i] && !this.saving()[i]) {
        ...
        promises.push(settings[i].id ? _a(settings[i].id, details) : ma(details));
      }
    }
    if (promises.length) {
  ...
  _processSetting(setting) {
    if (setting.encryption_level === Le.Admin && !this.is_admin || setting.encryption_level === Le.Support && !this.is_support || setting.encryption_level === Le.NeverDisplay) {
      const obj = {};
...
//# sourceMappingURL=chunk-O2CJFV6M.js.map
