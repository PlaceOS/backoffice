...
async function loadSupportAccess(user) {
  ...
  const groups = await (user.sys_admin ? collectPages(ku({ limit: 200 })).then((groups2) => groups2.map((group) => ({ group, permissions: 255 }))) : xu({ subsystem: "support" })).catch(() => []);
...
function querySupportSystems(options = {}) {
  ...
  return ha(__spreadValues(__spreadValues(__spreadValues({}, options), isSubsystemUser() ? { subsystem: "support" } : {}), selected_id() ? { group_id: selected_id() } : {}));
}
...
async function querySupportModules(options = {}) {
  ...
  for (let index = 0; index < systems.length; index += 8) {
    ...
    const pages = await Promise.all(systems.slice(index, index + 8).map((system) => collectPages(ic({ control_system_id: system.id })).catch((error) => {
...
//# sourceMappingURL=chunk-6IWBYUDE.js.map
