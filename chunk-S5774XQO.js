...
var DriverUpdateListModalComponent = class _DriverUpdateListModalComponent {
  ...
  loading = signal("Loading drivers...", ...ngDevMode ? [{ debugName: "loading" }] : []);
  _change = signal(0, ...ngDevMode ? [{ debugName: "_change" }] : []);
  drivers_with_updates = toSignal(toObservable(this._change).pipe(switchMap(() => {
  ...
  selected_drivers = signal([], ...ngDevMode ? [{ debugName: "selected_drivers" }] : []);
  all_selected = computed(() => this.selected_drivers().length === this.drivers_with_updates().total, ...ngDevMode ? [{ debugName: "all_selected" }] : []);
  some_selected = computed(() => this.selected_drivers().length > 0 && !this.all_selected(), ...ngDevMode ? [{ debugName: "some_selected" }] : []);
  ...
  async updateDrivers() {
    ...
    await Promise.all(selected.map((driver) => driver.commit !== driver.update_info.commit ? Fu(driver.id, __spreadProps(__spreadValues({}, driver), {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(DriverUpdateListModalComponent, { className: "DriverUpdateListModalComponent", filePath: "src/app/drivers/driver-update-list-modal.component.ts", lineNumber: 166 });
})();
...
var DriverStateService = class _DriverStateService {
  ...
  updates_available = this._poll.pipe(switchMap(() => Du({ update_available: true, limit: 1 }).pipe(catchError(() => of({ data: [], total: 0 })))), map((d) => d.total > 1), shareReplay(1));
  modules = this.item.pipe(switchMap(async (item) => {
    if (!item)
      return { data: [] };
    this._loading.next(true);
    const details = await hc({
      driver_id: item.id
    }).toPromise();
  ...
  docs = this.item.pipe(debounceTime(100), switchMap(async (item) => {
    if (!item)
      return "";
    this._loading.next(true);
    const docs = await lastValueFrom(Bu(item.id)).catch(() => "");
    this._loading.next(false);
    return docs;
  ...
  async updateDriver() {
    ...
    const success = await Fu(item.id, __spreadProps(__spreadValues({}, item), {
  ...
  async recompileDriver() {
    ...
    await Hu(item.id).toPromise().catch(async (e) => {
      console.log("Error:", e);
  ...
  async reloadDriver() {
    ...
    const success = await Wu(item.id).toPromise().catch(() => false);
    if (success === false) {
  ...
  async removeModule(device) {
    ...
    if (!details || !details.reason)
      return;
    const system = await aa(this.active_item.id, device.id).toPromise().catch((err) => {
...
//# sourceMappingURL=chunk-S5774XQO.js.map
