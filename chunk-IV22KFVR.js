...
var PlaceClusterTaskListComponent = class _PlaceClusterTaskListComponent extends AsyncHandler {
  ...
  killProcess(process) {
    return lastValueFrom(xu(this.cluster().id, { driver: process.id }));
  }
  async loadCluster(id) {
    const clusters = await lastValueFrom(Su({ q: id }).pipe(map((_) => _.data)));
  ...
  async updateProcessList() {
    ...
    const list = await lastValueFrom($u(this.cluster().id, {
      include_status: true
    })).catch(() => []);
...
//# sourceMappingURL=chunk-IV22KFVR.js.map
