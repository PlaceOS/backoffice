...
var ModuleStateService = class _ModuleStateService {
  ...
  _driver = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_driver" } : (
    ...
    loader: async ({ params: item }) => {
      if (!(item instanceof gs))
        return null;
      return Io(item.driver_id).catch(() => null);
    }
  }));
  ...
  _system = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_system" } : (
    ...
    params: () => this.item(),
    loader: async ({ params: item }) => {
      if (!(item instanceof gs) || !item.system_id)
        return null;
      return Jc(item.system_id).catch(() => null);
    }
  }));
  ...
  _edge = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_edge" } : (
    ...
    params: () => this.item(),
    loader: async ({ params: item }) => {
      if (!(item instanceof gs) || !item.edge_id)
        return null;
      return zo(item.edge_id).catch(() => null);
    }
  }));
  edge = computed(
  ...
  async toggleModuleState() {
    const method = this.active_item.running ? Ku : Qu;
...
//# sourceMappingURL=chunk-2QWNDQ43.js.map
