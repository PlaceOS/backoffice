...
test.describe('Zones', () => {
    ...
    test.describe('US-ZON-001: View Zones List', () => {
        test('AC-ZON-001-1: Display Zones List - should show sidebar with all zones', async ({ page }) => {
            ...
            await expect(zonesPage.zonesList.first()).toBeVisible({ timeout: 10000 });
        });

        test('AC-ZON-001-3: Search Zones - should filter zones by search term', async ({ page }) => {
            await zonesPage.goto();
            ...
            const items = page.locator('item-sidebar a p, virtual-scroll a p').filter({ hasText: /Level/i });
            const count = await items.count();
            expect(count).toBeGreaterThan(0);
        });

        test('AC-ZON-001-4: Zone Selection - should display zone details when selected', async ({ page }) => {
            await zonesPage.goto();
    ...
    test.describe('US-ZON-002: View Zone Details', () => {
        test('AC-ZON-002-1: Display Basic Information - should show zone name', async ({ page }) => {
            await zonesPage.goto();
            await page.waitForTimeout(1000);
            ...
            await expect(zonesPage.mainContent).toBeVisible();
            if (zoneName) {
                await expect(zonesPage.mainContent.locator(`text=${zoneName.trim().split('\n')[0]}`).first()).toBeVisible({ timeout: 5000 }).catch(() => {});
            }
        });
    });
    ...
    test.describe('US-ZON-003: Create New Zone', () => {
        test('AC-ZON-003-1: Access Create Form - should open create modal', async ({ page }) => {
            await zonesPage.goto();
        ...
        test('AC-ZON-003-5: Successful Creation - should create zone and show in list', async ({ page }) => {
            await zonesPage.goto();
    ...
    test.describe('US-ZON-004: Edit Zone Details', () => {
        test('AC-ZON-004-1: Access Edit Mode - should open edit modal with current details', async ({ page }) => {
            await zonesPage.goto();
            ...
            const nameField = zonesPage.dialog.getByRole('textbox', { name: 'Name', exact: true });
            const value = await nameField.inputValue();
            expect(value.length).toBeGreaterThan(0);
        });

        test('AC-ZON-004-5: Save Changes - should persist changes', async ({ page }) => {
            await zonesPage.goto();
            ...
            await expect(zonesPage.dialog).not.toBeVisible({ timeout: 5000 }).catch(() => {});
        });
    });
    ...
    test.describe('US-ZON-005: Delete Zone', () => {
        test('AC-ZON-005-1: Delete Confirmation - should show confirmation dialog', async ({ page }) => {
            await zonesPage.goto();
        ...
        test('AC-ZON-005-4: Cancel Deletion - should keep zone when cancelled', async ({ page }) => {
            await zonesPage.goto();
            ...
            if (zoneName) {
                await zonesPage.expectZoneInList(zoneName.trim().split('\n')[0]);
            }
        });
    });
    ...
    test.describe('US-ZON-006: View Child Zones', () => {
        test('AC-ZON-006-1: Display Child Zones - should show children tab', async ({ page }) => {
            await zonesPage.goto();
    ...
    test.describe('US-ZON-007: View Systems in Zone', () => {
        test('AC-ZON-007-1: Display Systems List - should show systems in zone', async ({ page }) => {
            await zonesPage.goto();
    ...
    test.describe('US-ZON-008: Manage Zone Triggers', () => {
        test('AC-ZON-008-1: View Zone Triggers - should display triggers list', async ({ page }) => {
            await zonesPage.goto();
    ...
    test.describe('US-ZON-009: Manage Zone Metadata', () => {
        test('AC-ZON-009-1: View Metadata - should display metadata entries', async ({ page }) => {
            await zonesPage.goto();
    ...
    test.describe('US-ZON-010: View Zone History', () => {
        test('AC-ZON-010-1: Display Change History - should show history list', async ({ page }) => {
            await zonesPage.goto();
