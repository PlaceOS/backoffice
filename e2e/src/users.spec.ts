...
test.describe('Users', () => {
    ...
    test.describe('US-USR-001: View Users List', () => {
        test('AC-USR-001-1: Display Users List - should show sidebar with all users', async ({ page }) => {
            ...
            await expect(usersPage.usersList.first()).toBeVisible({ timeout: 10000 });
        });

        test('AC-USR-001-2: Search Users - should filter users by search term', async ({ page }) => {
            await usersPage.goto();
        ...
        test('AC-USR-001-3: User Selection - should display user details when selected', async ({ page }) => {
            await usersPage.goto();
    ...
    test.describe('US-USR-002: View User Details', () => {
        test('AC-USR-002-1: Display Basic Information - should show name and email', async ({ page }) => {
            await usersPage.goto();
            await page.waitForTimeout(1000);
            ...
            await expect(usersPage.mainContent.locator('text=/@/').first()).toBeVisible({ timeout: 5000 }).catch(() => {
    ...
    test.describe('US-USR-003: Create New User', () => {
        test('AC-USR-003-1: Access Create Form - should open create modal', async ({ page }) => {
            await usersPage.goto();
        ...
        test('AC-USR-003-6: Successful Creation - should create user and show in list', async ({ page }) => {
            await usersPage.goto();
            ...
            try {
                ...
                const validationErrors = page.locator('.mat-mdc-form-field-error, mat-error, [class*="error"]:has-text("required")');
                await expect(validationErrors).toHaveCount(0, { timeout: 2000 });
            }
        });
    });
    ...
    test.describe('US-USR-004: Edit User Details', () => {
        test('AC-USR-004-1: Access Edit Mode - should open edit modal with current details', async ({ page }) => {
            await usersPage.goto();
        ...
        test('AC-USR-004-4: Save Changes - should persist changes', async ({ page }) => {
            await usersPage.goto();
            ...
            const nameField = usersPage.dialog.locator('input[formcontrolname="name"], input').first();
            ...
            await expect(usersPage.dialog).not.toBeVisible({ timeout: 5000 }).catch(() => {});
        });
    });
    ...
    test.describe('US-USR-005: Delete User', () => {
        test('AC-USR-005-1: Delete Confirmation - should show confirmation dialog', async ({ page }) => {
            await usersPage.goto();
        ...
        test('AC-USR-005-3: Cancel Deletion - should keep user when cancelled', async ({ page }) => {
            await usersPage.goto();
            ...
            if (userName) {
                await usersPage.expectUserInList(userName.trim().split('\n')[0]);
            }
        });
    });
    ...
    test.describe('US-USR-007: Enable/Disable User', () => {
        test('AC-USR-007-3: Status Indicator - should display user status', async ({ page }) => {
            await usersPage.goto();
    ...
    test.describe('US-USR-009: Manage User Metadata', () => {
        test('AC-USR-009-1: View Metadata - should display metadata entries', async ({ page }) => {
            await usersPage.goto();
    ...
    test.describe('US-USR-010: View User History', () => {
        test('AC-USR-010-1: Display Change History - should show history list', async ({ page }) => {
            await usersPage.goto();
    ...
    test.describe('US-USR-012: View User Groups', () => {
        test('AC-USR-012-1: Display Groups - should show user groups', async ({ page }) => {
            await usersPage.goto();
