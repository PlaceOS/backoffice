...
var MetadataDisplayComponent = class _MetadataDisplayComponent extends AsyncHandler {
  ...
  async deleteMetadata(field) {
    ...
    if (result.reason !== "done")
      return;
    await lastValueFrom(nc(this.item().id, { name: field })).catch((err) => {
  ...
  saveMetadata(field) {
    ...
    ec(this.item().id, __spreadProps(__spreadValues({}, value), {
      ...
      next: (item) => {
        ...
        if (field.name !== value.name) {
          lastValueFrom(nc(this.item().id, { name: field.name })).catch((err) => notifyError(`Error removing old "${field.name}" metadata. Error: ${JSON.stringify(err.response || err.message || err)}`));
        }
        if (index >= 0) {
          this.metadata.update((l) => {
...
//# sourceMappingURL=chunk-A7GGL63M.js.map
