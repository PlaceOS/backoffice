...
var SettingsFormComponent = class _SettingsFormComponent extends AsyncHandler {
  ...
  constructor() {
    super();
    effect(() => {
      this.encryption_level.set(this.merge() ? De.NeverDisplay + 1 : De.None);
    });
    effect(() => {
  ...
  save(level) {
    ...
    (settings[level].id ? Zc(settings[level].id, details) : Jc(details)).then((new_settings) => {
      this.saving.update((s) => {
        s[level] = false;
        return s;
      });
  ...
  saveAll() {
    ...
    const promises = [];
    for (let i = 0; i < De.NeverDisplay + 1; i++) {
      const settings = this.settings();
      if (settings[i] && !this.saving()[i]) {
        ...
        promises.push(settings[i].id ? Zc(settings[i].id, details) : Jc(details));
      }
    }
    if (promises.length) {
  ...
  _processSetting(setting) {
    if (setting.encryption_level === De.Admin && !this.is_admin || setting.encryption_level === De.Support && !this.is_support || setting.encryption_level === De.NeverDisplay) {
      const obj = {};
...
//# sourceMappingURL=chunk-OHBSUPRB.js.map
