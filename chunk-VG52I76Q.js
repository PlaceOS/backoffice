...
function toQueryString(map) {
  ...
  if (map) {
    for (const key in map) {
      if (Object.prototype.hasOwnProperty.call(map, key) && map[key] !== void 0 && map[key] !== null) {
        const value = map[key];
        str += `${str ? "&" : ""}${key}=${encodeURIComponent(typeof value === "object" && value !== null ? JSON.stringify(value) : `${value}`)}`;
      }
    }
  }
  return str;
...
function extensionsForItem(item, type) {
  const authority_config = Ut()?.config?.backoffice;
...
//# sourceMappingURL=chunk-VG52I76Q.js.map
