...
var BindingDirective = class _BindingDirective extends AsyncHandler {
  ...
  _old_model = null;
  ngOnInit() {
    waitForClientSignalValue(wr(), (_) => _).then(() => this.bindVariable());
  }
  ngOnChanges(changes) {
  ...
  bindVariable() {
    if (Pt() && this.bind() && this.sys() && this.mod() && !this._binding) {
      this.timeout("bind", () => {
        const module = Dl(this.sys(), this.mod(), this.index());
  ...
  execute() {
    if (Pt() && this.exec() && this.sys() && this.mod() && !this._timers["execute"]) {
...
//# sourceMappingURL=chunk-QX3UWUPL.js.map
