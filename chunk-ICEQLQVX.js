...
var GroupStateService = class _GroupStateService {
  ...
  _counts = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_counts" } : (
    ...
    loader: async ({ params }) => {
      ...
      const [users, zones] = await Promise.all([
        Lu({ group_id: item.id, limit: 1 }).then((response) => response.total).catch(() => 0),
        Qu({ group_id: item.id, limit: 1 }).then((response) => response.total).catch(() => 0)
      ]);
      return { users, zones };
    }
  }));
  ...
  async bulkAddUsers(existing_users = []) {
    ...
    this._loading.set(true);
    const results = await Promise.allSettled(users.map((user) => Gu({
      group_id: this.active_item.id,
      user_id: user.id,
      permissions
    })));
  ...
  async bulkAddZones(existing_zones = []) {
    ...
    if (!zones?.length)
      return;
    this._loading.set(true);
    const results = await Promise.allSettled(zones.map((zone) => Zu({
      group_id: this.active_item.id,
      zone_id: zone.id
    })));
    this._loading.set(false);
...
//# sourceMappingURL=chunk-ICEQLQVX.js.map
