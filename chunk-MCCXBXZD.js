...
var AdminSchemasComponent = class _AdminSchemasComponent {
  ...
  async saveSchema() {
    ...
    const new_schema = await lastValueFrom(schema.id ? R(__spreadProps(__spreadValues({}, details), {
      id: schema.id,
      method: "patch"
    })) : M(__spreadValues({}, details)));
    schema_list = [
  ...
  async loadSchemas() {
    const schema_list = await lastValueFrom(A({
      query_params: {},
      fn: (_) => _,
      path: "schema"
    }).pipe(map((_) => _.data)));
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _AdminSchemasComponent, selectors: [["admin-schemas"]], decls: 22, vars: 15, consts: [[1, "flex", "h-full", "w-full", "flex-col"], [1, "my-4", "flex", "items-center", "justify-between", "space-x-2"], [1, "text-2xl"], [1, "flex", "items-center", "space-x-2"], ["appearance", "outline", 1, "no-subscript", "w-56"], ["name", "type", 3, "ngModelChange", "ngModel", "placeholder"], [3, "value"], [3, "click"], [1, "font-sans", "italic"], ["btn", "", "matRipple", "", 1, "h-12", "w-40", 3, "click"], [1, "mb-4", "flex", "items-center", "space-x-2"], [1, "relative", "h-1/2", "flex-1"], ["lang", "json", 3, "ngModel", "readonly"], [1, "absolute", "inset-x-2", "bottom-5", "top-2", "flex", "items-center", "justify-center", "rounded-xl", "bg-base-200"], [1, "flex", "w-1/2", "flex-1", "flex-col"], ["for", "type"], ["appearance", "outline", 1, "no-subscript", "w-full"], ["matInput", "", 3, "ngModelChange", "ngModel"], ["btn", "", "matRipple", "", 1, "mt-6", "h-12", "w-40", 3, "click"], ["lang", "json", 3, "ngModelChange", "ngModel", "readonly"], [1, "p-8", "opacity-30"]], template: function AdminSchemasComponent_Template(rf, ctx) {
    if (rf & 1) {
...
//# sourceMappingURL=chunk-MCCXBXZD.js.map
