...
var ExtensionOutletComponent = class _ExtensionOutletComponent extends AsyncHandler {
  ...
  _online = signalFromClient(Er(), false);
  ...
  async loadMetadata(item, message, parent = false) {
    const metadata = await Du(parent ? item.parent_id : item.id, message.name);
    if (metadata) {
      this._postMessage({
        id: message.id,
        type: "backoffice",
  ...
  async loadResource(item, message) {
    ...
    const tkn = V();
    document.cookie = `${tkn === "x-api-key" ? "api-key=" + encodeURIComponent(Qe()) : "bearer_token=" + encodeURIComponent(tkn)};max-age=60;path=/api/;samesite=strict;${location.protocol === "https:" ? "secure;" : ""}`;
...
//# sourceMappingURL=chunk-SZ5F6S6Q.js.map
