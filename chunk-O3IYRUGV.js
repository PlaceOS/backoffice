...
function generateBrokerFormModel(broker = {}) {
  const auth_type = broker.auth_type ?? ji.NoAuth;
  return {
    name: broker.name || "",
...
var applyBrokerFormSchema = (path) => {
  ...
  required(path.username, {
    when({ valueOf }) {
      return valueOf(path.auth_type) === ji.UserPassword;
    }
  });
  ...
  required(path.certificate, {
    when({ valueOf }) {
      return valueOf(path.auth_type) === ji.Certificate;
    }
  });
};
...
var BrokerFormComponent = class _BrokerFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const result = await (form_item.id ? Ro(form_item.id, form_item) : Uo(form_item)).catch(async (err) => {
...
var AdminBrokersComponent = class _AdminBrokersComponent extends AsyncHandler {
  ...
  async deleteBroker(item) {
    if (item) {
      ...
      const err = await Io(item.id).catch((_) => _);
      details.close();
      if (err)
  ...
  async loadBrokers() {
    ...
    const brokers = await Po().then((r) => r.data);
...
//# sourceMappingURL=chunk-O3IYRUGV.js.map
