...
var BulkItemModalComponent = class _BulkItemModalComponent {
  ...
  generateTemplate() {
    switch (this._data.constr) {
      ...
      case On:
        return [new On(USER_TEMPLATE).toJSON()];
      case Xt:
  ...
  findDuplicates() {
    ...
    const dedup_field = this._data.constr === On ? "email" : "name";
    this.item_list.forEach((item, index) => {
...
async function zoneSubtreeIds(zone_id) {
  ...
  while (level.length) {
    ...
    const children = await collectPages(ah({ parent_id: level.join(","), limit: PAGE_SIZE }));
    level = [];
...
async function splitZoneSystems(zone_ids) {
  ...
  const pages = await mapLimit(zone_ids, READ_CONCURRENCY, (zone_id) => collectPages($a({ zone_id, limit: PAGE_SIZE })));
  ...
  const confirmed = await mapLimit(candidates, READ_CONCURRENCY, (system) => va(system.id).catch(() => null));
  const orphaned = [];
...
async function planDomainCascade(domain) {
  ...
  const org_zone = await lh(org_zone_id).catch(() => null);
  if (!org_zone) {
...
var domains = {
  ...
  itemConstructor: Yi,
  loadModal: () => import("./chunk-4OXYRYBF.js").then((m) => m.DomainFormComponent),
  delete_message: ``,
  cascade: {
...
var drivers = {
  ...
  itemConstructor: As,
  loadModal: () => import("./chunk-LAGFSD5P.js").then((m) => m.DriverFormComponent),
  delete_message: ``,
  delete_extra: async (_) => {
    ...
    const count = await dc(query).then(({ total }) => total).catch((_err) => 0);
    return count ? [
      "error",
...
var groups = {
  ...
  show: (_) => Ru(_),
  save: (item) => item.id ? Mu(item.id, item) : Eu(item),
  remove: (item) => Cu(item.id),
  itemConstructor: wn,
  loadModal: () => import("./chunk-TFPYIC7I.js").then((m) => m.GroupFormComponent),
  delete_message: ``,
  name: "GROUPS"
};
var modules = {
  ...
  show: (_) => pc(_),
  save: (item) => item.id ? fc(item.id, item) : _c(item),
  remove: (item) => mc(item.id),
  itemConstructor: Is,
  loadModal: () => import("./chunk-IJ4I7GGV.js").then((m) => m.ModuleFormComponent),
  delete_message: ``,
  name: "MODULES"
};
var repositories = {
  ...
  show: (_) => Dc(_),
  save: (item) => item.id ? Hc(item.id, item) : zc(item),
  remove: (item) => Fc(item.id),
  itemConstructor: ur,
  loadModal: () => import("./chunk-4RNMDWF4.js").then((m) => m.RepositoryFormComponent),
  delete_message: `'`,
  name: "REPOS"
};
var systems = {
  ...
  itemConstructor: Rs,
  loadModal: () => import("./chunk-PH6BLAEC.js").then((m) => m.SystemFormComponent),
  delete_message: ``,
  name: "SYSTEMS"
};
...
var triggers = {
  ...
  show: (_) => Ga(_),
  save: (item) => item.id ? Ba(item.id, item) : Wa(item),
  remove: (item) => Qa(item.id),
  itemConstructor: pe,
  loadModal: () => import("./chunk-YVPEBFCN.js").then((m) => m.TriggerFormComponent),
  delete_message: ``,
  name: "TRIGGERS"
};
var users = {
  query: (_, options) => Ja(__spreadProps(__spreadValues({}, options), {
    q: _,
    fields: [
      "id",
      "name",
  ...
  show: (_) => Va(_),
  save: (item) => item.id ? Xa(item.id, item) : eh(item),
  remove: (item) => th(item.id),
  itemConstructor: On,
  loadModal: () => import("./chunk-D7OC6IGH.js").then((m) => m.UserFormComponent),
  delete_message: ``,
  name: "USERS"
};
var zones = {
  ...
  show: (_) => lh(_),
  save: (item) => item.id ? dh(item.id, item) : ph(item),
  remove: (item) => fh(item.id),
  itemConstructor: Xt,
  loadModal: () => import("./chunk-APAAZ2ON.js").then((m) => m.ZoneFormComponent),
  delete_message: ``,
  cascade: {
...
var ActiveItemService = class _ActiveItemService extends AsyncHandler {
  ...
  async delete(force_removal = false) {
    ...
    waitForEvent(ref.componentInstance.event, (e) => e.reason === "done").then(async (event) => {
      ...
      await (force_removal ? th(item.id, { force_removal: true }) : actions.remove(item)).then(() => {
        this._active_item.set(null);
  ...
  async updateSettings() {
    ...
    if (item && item.settings) {
      let settings = (await pa({ parent_id: item.id })).data;
...
//# sourceMappingURL=chunk-WKKSFDT2.js.map
