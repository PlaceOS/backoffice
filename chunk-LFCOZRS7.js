...
var PlaceDebugService = class _PlaceDebugService extends AsyncHandler {
  ...
  terminal_string = computed(() => {
    const list = this.events().map((event) => `${TERMINAL_COLOURS[event.level] || TERMINAL_COLOURS.debug}${format(Date.now(), "h:mm a")}, ${this._module_names[event.mod_id] || event.mod_id || "<UNKNOWN>"}, [${event.level.toUpperCase()}]\x1B[0m ${event.message.split("\n").reverse().join("\n")}`);
    return list.join("\n");
  }, ...ngDevMode ? [{ debugName: "terminal_string" }] : []);
  ...
  bind(module, module_name) {
    if (module) {
      ...
      Ba(options).then(() => {
        this.subscription(`debug_${module.id}`, () => Va(options));
...
//# sourceMappingURL=chunk-LFCOZRS7.js.map
