...
var PlaceClusterTaskListComponent = class _PlaceClusterTaskListComponent extends AsyncHandler {
  ...
  killProcess(process) {
    return lastValueFrom(nu(this.cluster().id, process.id));
  }
  async loadCluster(id) {
    const clusters = await lastValueFrom(Xs({ q: id }).pipe(map((_) => _.data)));
  ...
  async updateProcessList() {
    ...
    const list = await lastValueFrom(tu(this.cluster().id, {
      include_status: true
    })).catch((_) => []);
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _PlaceClusterTaskListComponent, selectors: [["engine-cluster-task-list"]], features: [\u0275\u0275InheritDefinitionFeature], decls: 30, vars: 44, consts: [["name_template", ""], ["cpu_template", ""], ["actions_template", ""], [1, "h-full", "w-full", "overflow-auto"], [1, "sticky", "left-0", "top-0", "z-20", "m-4", "mb-4", "flex", "w-[calc(100%-2rem)]", "items-center", "rounded-sm", "border", "border-base-300", "bg-base-200", "p-2"], ["icon", "", "matRipple", "", 3, "routerLink"], [1, "text-lg", "font-medium"], [1, "flex-1"], ["appearance", "outline", 1, "no-subscript", "rounded-sm", "bg-base-100"], ["matPrefix", "", 1, "prefix"], [1, "relative", "-left-0.5", "text-2xl"], ["matInput", "", 3, "ngModelChange", "ngModel", "placeholder"], [1, "px-4"], ["mode", "indeterminate", 1, "w-full"], [1, "mb-4", "w-full", "overflow-auto", "px-4"], [1, "block", "min-w-184", "text-sm", 3, "data", "columns", "sortable", "empty_message"], [1, "flex", "flex-col", "px-4", "py-2", "font-mono"], [1, "mb-1"], [1, "flex", "items-center", "space-x-2", "text-[0.625rem]"], [1, "rounded-sm", "bg-info", "px-2", "text-info-content"], [1, "rounded-sm", "bg-base-200", "px-2"], [1, "w-full", "p-4", "text-right"], [1, "mx-auto", "flex", "items-center", "space-x-2", "p-2"], ["icon", "", "matRipple", "", 1, "text-error", 3, "click", "matTooltip"]], template: function PlaceClusterTaskListComponent_Template(rf, ctx) {
    if (rf & 1) {
...
//# sourceMappingURL=chunk-PUTWSJOH.js.map
