...
var BookingLimitsModalComponent = class _BookingLimitsModalComponent {
  ...
  async save() {
    ...
    const call = G(`/api/staff/v1/tenants/${this.tenant.id}/limits`, booking_limits);
...
var StaffTenantModalComponent = class _StaffTenantModalComponent {
  ...
  async save() {
    ...
    const data = as(__spreadProps(__spreadValues(__spreadValues({}, this.tenant || {}), value), {
      booking_limits
    }), ["", null, void 0]);
    const call = this.tenant?.id ? Ne(`/api/staff/v1/tenants/${this.tenant.id}`, data) : G("/api/staff/v1/tenants", data);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(StaffTenantModalComponent, { className: "StaffTenantModalComponent", filePath: "src/app/admin/staff-tenant-modal.component.ts", lineNumber: 456 });
})();
...
var PlaceStaffAPIComponent = class _PlaceStaffAPIComponent {
  ...
  tenants = this.domain.pipe(switchMap(() => {
    ...
    return N("/api/staff/v1/tenants");
  }), catchError((__) => []), map((tenants) => {
    this.loading.set("");
  ...
  async ngOnInit() {
    ...
    this.domain_list = await xu().pipe(map((r) => r.data)).toPromise();
    const domain = Ut();
  ...
  editTenant(tenant) {
    ...
    ref.afterClosed().subscribe((__) => this.domain.next(this.domain.getValue()));
  }
  editLimits(tenant) {
  ...
  async removeTenant(tenant) {
    ...
    const system = await Oe(`/api/staff/v1/tenants/${tenant.id}`).toPromise().catch((err) => {
...
//# sourceMappingURL=chunk-SWCDMKQF.js.map
