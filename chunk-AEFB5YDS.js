...
var ZonesStateService = class _ZonesStateService {
    ...
    loader: async ({ params }) => {
      ...
      try {
        const details = await Promise.all([
          ...
          (isSubsystemUser() ? Promise.resolve({ data: [], total: 0 }) : ph(item.id)).then((d) => d.total).catch(() => 0),
          nc(item.id).then((d) => d.length).catch(() => 0),
          querySupportZones({ parent_id: item.id, limit: 1 }).then((d) => d.total).catch(() => 0),
          Wu({ zone_id: item.id, limit: 1 }).then((d) => d.total).catch(() => 0)
        ]);
  ...
  _triggers = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_triggers" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Xt))
        return [];
      const response = await (isSubsystemUser() ? Promise.resolve({ data: [], total: 0 }) : ph(item.id)).catch(() => ({
        data: []
      }));
      return response.data;
    }
  ...
  async addTrigger(trigger) {
    ...
    return hh(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
  ...
  async removeTrigger(trigger) {
    ...
    if (!details?.reason)
      return;
    const zone = await hh(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
  ...
  async bulkAddGroups(existing_groups = []) {
    ...
    if (!groups?.length)
      return;
    this._loading.set(true);
    const results = await Promise.allSettled(groups.map((group) => Ku({
      group_id: group.id,
      zone_id: this.active_item.id
    })));
    this._loading.set(false);
...
//# sourceMappingURL=chunk-AEFB5YDS.js.map
