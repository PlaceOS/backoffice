    ...
    template: `
        <div class="flex items-center space-x-2 my-4">
            ...
            <mat-form-field class="h-12" appearance="outline">
                ...
                >
                    <mat-option *ngFor="let schema of schema_list | async" [value]="schema">
                        {{ schema.name }}
                    </mat-option>
                </mat-select>
            </mat-form-field>
            </div>
            <button mat-button class="mt-6" (click)="newSchema()">Add Schema</button>
        </div>
        <div class="flex items-center space-x-2 mb-4" *ngIf="schema_copy">
            ...
            <button mat-button class="mt-6" (click)="saveSchema()">Save Schema</button>
        </div>
...
export class AdminSchemasComponent {
    ...
    public readonly schema_list = this._state.schemas.pipe(
        map((_) => [
            ..._,
            {
    ...
    public async newSchema() {
        const list = await this.schema_list.pipe(take(1)).toPromise();
        this.active_schema = list[list.length - 1];
        this.copySchema();
    }
