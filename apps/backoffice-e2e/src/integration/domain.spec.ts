...
describe('Domain test', () => {
    ...
    it('Can flag incorrect domain settings', () => {
        ...
        cy.wait(1000);
        cy.get(
            '*[class^="ng-untouched ng-star-inserted ng-dirty ng-invalid"]',
        ).should('exist');
        cy.get('*[class^="view-lines monaco-mouse-cursor-text"]')
            .click()
    ...
    it('Can add a new application to a Domain', () => {
        ...
        cy.get('*[class^="mat-checkbox-label"]')
            .contains('Skip Authorization')
            .click();
        cy.get('input[name="redirect-uri"]').type(
            'http://localhost:' + getRandomInt(1000) + '/oauth-resp.html',
        );
        cy.wait(1000);
    ...
    it('Can get the secret for an Application', () => {
        ...
        cy.get('*[class^="name"]')
            .contains('Applications')
            .click({ force: true });
        cy.get('*[class^="w-48 p-2 truncate underline text-center text-xs"]')
            .contains('Show')
            .click();
    ...
    it('Can export a domain', () => {
        ...
        cy.get('*[class^="heading select-text"]').then(($hd) => {
            ...
            cy.readFile(
                path.join(downloadsFolder, selected_domain + '.domains.tsv'),
            ).should('exist');
        });
    });
});
