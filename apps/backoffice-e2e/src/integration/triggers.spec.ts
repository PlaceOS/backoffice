...
describe('Triggers test', () => {
    ...
    it('Can add a new trigger condition', () => {
        ...
        cy.get('*[class^="mat-select"]')
            .contains('Select status variable')
            .click();
        cy.get('*[class^="mat-option-text"]').last().click({ force: true });
        cy.get('[placeholder="Status variable subkeys..."]')
            .click()
            .type('orange');
        cy.get('[placeholder="true/false, \'string\', 123.456"]')
            .click()
            .type('4');
    ...
    it('Can add a new trigger action', () => {
        ...
        cy.get('[placeholder="Email body contents..."]')
            .click()
            .type('This is the email body...');
        cy.get('*[class^="mat-button-wrapper"]').contains('Add').click();
        cy.get('*[class^="mat-simple-snackbar ng-star-inserted"]').contains(
            'Successfully',
        );
    ...
    it('Can see which systems the selected trigger is used in', () => {
        ...
        cy.get(
            '*[class^="mat-tab-link mat-focus-indicator tab ng-star-inserted mat-tab-label-active"]',
        ).contains('Instances');
    });
    ...
    it('Can export a trigger', () => {
        ...
        cy.get('*[class^="heading select-text"]').then(($hd) => {
            ...
            cy.readFile(
                path.join(downloadsFolder, user + '.triggers.tsv'),
            ).should('exist');
        });
    });
