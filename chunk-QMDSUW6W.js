...
var UploadLibraryComponent = class _UploadLibraryComponent extends AsyncHandler {
  ...
  domain_list = toSignal(Tu({ limit: 100 }).pipe(map((r) => r.data), shareReplay(1)), { initialValue: [] });
  _uploads_list = combineLatest([
    toObservable(this.domain),
    toObservable(this.search_term),
    toObservable(this.refresh)
  ]).pipe(filter(([domain]) => !!domain), debounceTime(300), switchMap(([domain, search]) => A({
    path: "uploads",
    query_params: {
      limit: 1e3,
      file_search: search,
  ...
  async ngOnInit() {
    const domain = Ut();
    const domain_list = await nextValueFrom(Tu({ limit: 100 }).pipe(map((r) => r.data)));
  ...
  async downloadUpload(upload) {
    const tkn = X();
    document.cookie = `${tkn === "x-api-key" ? "api-key=" + encodeURIComponent(zt()) : "bearer_token=" + encodeURIComponent(tkn)};max-age=60;path=/api/engine/v2/uploads;samesite=strict;${location.protocol === "https:" ? "secure;" : ""}`;
...
//# sourceMappingURL=chunk-QMDSUW6W.js.map
