import {
  UserPipe
} from "./chunk-WY6SX2BG.js";
import {
  SettingsFieldComponent
} from "./chunk-J3DDD2YE.js";
import {
  DiffViewerComponent
} from "./chunk-7PIF43BP.js";
import {
  validateJSONString
...
var MetadataHistoryModalComponent = class _MetadataHistoryModalComponent {
  ...
  async ngOnInit() {
    const history = await zu(this._data.id, {
      name: this._data.name,
      limit: 5e3
    });
...
var MetadataDisplayComponent = class _MetadataDisplayComponent extends AsyncHandler {
  ...
  async deleteMetadata(field) {
    ...
    if (result.reason !== "done")
      return;
    await Gu(this.item().id, { name: field }).catch((err) => {
  ...
  saveMetadata(field) {
    ...
    Lu(this.item().id, __spreadProps(__spreadValues({}, value), {
      ...
      if (field.name !== value.name) {
        Gu(this.item().id, { name: field.name }).catch((err) => notifyError(`Error removing old "${field.name}" metadata. Error: ${JSON.stringify(err.response || err.message || err)}`));
      }
      if (index >= 0) {
        this.metadata.update((l) => {
  ...
  async loadMetadata() {
    ...
    await Hu(this.item().id).then((map) => {
...
//# sourceMappingURL=chunk-TUCDW3JN.js.map
