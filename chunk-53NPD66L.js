...
var ZonesStateService = class _ZonesStateService {
    ...
    loader: async ({ params }) => {
      ...
      try {
        const details = await Promise.all([
          Kc({ zone_id: item.id, limit: 1 }).then((d) => d.total).catch(() => 0),
          Ba(item.id).then((d) => d.total).catch(() => 0),
          Ou(item.id).then((d) => d.length).catch(() => 0),
          wa({ parent_id: item.id, limit: 1 }).then((d) => d.total).catch(() => 0),
          xu({ zone_id: item.id, limit: 1 }).then((d) => d.total).catch(() => 0)
        ]);
  ...
  _systems = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_systems" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Ft))
        return [];
      const response = await Kc({ zone_id: item.id }).catch(() => ({ data: [] }));
  ...
  _triggers = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_triggers" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Ft))
        return [];
      const response = await Ba(item.id).catch(() => ({
        data: []
      }));
      return response.data;
    }
  ...
  _children = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_children" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Ft))
        return [];
      const response = await wa({ parent_id: item.id }).catch(() => ({ data: [] }));
      return response.data;
    }
  }));
  children = computed(
  ...
  get authority_id() {
    return this.active_item?.authority_id || St()?.id || void 0;
  }
  constructor() {
  ...
  async removeTrigger(trigger) {
    ...
    if (!details?.reason)
      return;
    const zone = await Fa(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
...
//# sourceMappingURL=chunk-53NPD66L.js.map
