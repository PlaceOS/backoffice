...
var AuthSourceModalComponent = class _AuthSourceModalComponent extends AsyncHandler {
  ...
  type = computed(() => this.item() instanceof so ? "saml" : this.item() instanceof eo ? "ldap" : "oauth", ...ngDevMode ? [{ debugName: "type" }] : []);
  ...
  setType(type) {
    ...
    switch (type) {
      case "saml":
        this.item.set(new so(data));
        break;
      case "ldap":
        this.item.set(new eo(data));
        break;
      default:
  ...
  updateMethod(item) {
    switch (this.type()) {
      case "saml":
        return item.id ? Wc(item.id, item) : Lc(item);
      case "ldap":
        return item.id ? Ku(item.id, item) : Zu(item);
    }
    return item.id ? kc(item.id, item) : $c(item);
  }
  /**
...
var DomainStateService = class _DomainStateService {
  ...
  users = combineLatest([
    this._changed,
    this.item
  ]).pipe(filter(([, item]) => item instanceof Zs), switchMap(([, item]) => Oa({ authority_id: item.id, limit: 1e3 })), map((response) => response.data.sort((a, b) => a.name.localeCompare(b.name))), catchError(() => []), shareReplay(1));
  auth_sources = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof Zs), switchMap(([, item]) => {
    const q = { authority_id: item.id };
    return combineLatest([
      zc(q).pipe(map((response) => response.data)),
      vc(q).pipe(map((response) => response.data)),
      Gu(q).pipe(map((response) => response.data))
    ]);
  }), map((sources) => {
    let list = [];
  ...
  applications = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof Zs), switchMap(([, item]) => hu({ authority_id: item.id })), map((response) => response.data.sort((a, b) => a.name.localeCompare(b.name))), catchError(() => []), shareReplay(1));
  counts = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof Zs), switchMap(async ([, item]) => {
    const q = { authority_id: item?.id };
    const details = await Promise.all([
      lastValueFrom(hu(q).pipe(map((response) => response.total))),
      lastValueFrom(combineLatest([
        zc(q),
        vc(q),
        Gu(q)
      ]).pipe(map(([saml, oauth, ldap]) => saml.total + oauth.total + ldap.total))),
      lastValueFrom(Oa(q).pipe(map((response) => response.total)))
    ]);
  ...
  async update(domain) {
    const item = await Iu(domain.id, domain).toPromise();
    this._state.replaceItem(item);
  }
  async performAzureIntegration() {
    const item = this.active_item;
    if (!(item instanceof Zs))
      return;
  ...
  async editApplication(item) {
    item = item || new Vs({ owner_id: this.active_item.id });
  ...
  async deleteApplication(item) {
    ...
    const err = await _u(item.id).toPromise().catch((_) => _);
    details.close();
    if (err)
  ...
  async deleteAuthSource(item) {
    ...
    const method = item instanceof so ? Bc : item instanceof ro ? xc : Xu;
...
//# sourceMappingURL=chunk-4K24ZJTE.js.map
