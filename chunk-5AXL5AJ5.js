...
var PlaceClusterTaskListComponent = class _PlaceClusterTaskListComponent extends AsyncHandler {
  ...
  _cluster_id = toSignal(this._route.paramMap.pipe(map((params) => params.get("id"))), { initialValue: null });
  ...
  killProcess(process) {
    return lastValueFrom(cc(this.cluster().id, { driver: process.id }));
  }
  async loadCluster(id) {
    const clusters = await lastValueFrom(sc({ q: id }).pipe(map((_) => _.data)));
  ...
  async updateProcessList() {
    ...
    const list = await lastValueFrom(uc(this.cluster().id, {
      include_status: true
    })).catch(() => []);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(PlaceClusterTaskListComponent, { className: "PlaceClusterTaskListComponent", filePath: "src/app/admin/cluster-details/cluster-task-list.component.ts", lineNumber: 182 });
})();
export {
  PlaceClusterTaskListComponent
};
//# sourceMappingURL=chunk-5AXL5AJ5.js.map
