...
var SettingsFormComponent = class _SettingsFormComponent extends AsyncHandler {
  ...
  constructor() {
    super();
    effect(() => {
      this.encryption_level.set(this.merge() ? Fe.NeverDisplay + 1 : Fe.None);
    });
    effect(() => {
  ...
  save(level) {
    ...
    (settings[level].id ? pa(settings[level].id, details) : fa(details)).then((new_settings) => {
      this.saving.update((s) => {
        s[level] = false;
        return s;
      });
  ...
  saveAll() {
    ...
    const promises = [];
    for (let i = 0; i < Fe.NeverDisplay + 1; i++) {
      const settings = this.settings();
      if (settings[i] && !this.saving()[i]) {
        ...
        promises.push(settings[i].id ? pa(settings[i].id, details) : fa(details));
      }
    }
    if (promises.length) {
  ...
  _processSettings(settings) {
    ...
    for (let i = 0; i < Fe.NeverDisplay + 1; i++) {
      ...
      if (!setting) {
        processed_settings.push(this._processSetting(new Pe({ encryption_level: i })));
        continue;
      }
  ...
  _processSetting(setting) {
    if (setting.encryption_level === Fe.Admin && !this.is_admin || setting.encryption_level === Fe.Support && !this.is_support || setting.encryption_level === Fe.NeverDisplay) {
      const obj = {};
      for (const key of setting.keys) {
        obj[key] = `<${i18n("COMMON.SETTINGS_MASKED")}>`;
      }
      const settings_string = (setting.keys || []).length ? dump(obj) : "";
      return new Pe(__spreadProps(__spreadValues({}, setting), {
        parent_id: this.id(),
        settings_string
      }));
    }
    return new Pe(__spreadProps(__spreadValues({}, setting), { parent_id: this.id() }));
  }
...
//# sourceMappingURL=chunk-DCN5RGCX.js.map
