...
var APIKeyModalComponent = class _APIKeyModalComponent extends AsyncHandler {
  ...
  form = form(this.formModel, (p2) => {
    required(p2.name);
    required(p2.user_id);
    required(p2.scopes);
    minLength(p2.scopes, 1);
  });
  loading = signal(
    "",
  ...
  async loadSelectedUser() {
    ...
    if (!user_id || selected_user)
      return;
    const user = await La(user_id).catch(() => null);
    if (!user)
      return;
  ...
  loadUsers() {
    this.timeout("load_users", async () => {
      ...
      try {
        const users = this.domain() ? (await Fa({
...
var APIKeyService = class _APIKeyService {
  ...
  _available_scopes = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_available_scopes" } : (
    /* istanbul ignore next */
    {}
  )), { loader: async () => await p("/api/engine/v2/scopes") }));
  ...
  _users = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_users" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!domain)
        return [];
      return (await Fa({ authority_id: domain.id, q: q2 })).data;
    }
  }));
  users = computed(
    () => this._users.value() || [],
  ...
  async _currentUser() {
    ...
    if (cached_user?.id)
      return cached_user;
    return La("current").catch(() => null);
  }
  async _scopes() {
    const scopes = this.available_scopes();
    if (scopes.length)
      return scopes;
    return p("/api/engine/v2/scopes").catch(() => []);
  }
  _userLabel(user) {
...
//# sourceMappingURL=chunk-ZSXN76N3.js.map
