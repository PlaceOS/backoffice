...
var AdminClusterNodeComponent = class _AdminClusterNodeComponent {
  ...
  used_memory = computed(
    () => {
      return Tt((this.node()?.memory_usage || 0) * 1024).replace("GB", "").replace("MB", "").trim();
    },
  ...
  total_memory = computed(
    () => {
      return Tt((this.node()?.memory_total || 0) * 1024);
    },
...
var PlaceClusterDetailsComponent = class _PlaceClusterDetailsComponent extends AsyncHandler {
  ...
  async updateClusters() {
    ...
    const response = await Oo({ include_status: false }).catch(() => ({ data: [] }));
...
//# sourceMappingURL=chunk-6XSWTU5T.js.map
