...
var EdgeModalComponent = class _EdgeModalComponent {
  ...
  async save() {
    ...
    const method = edge.id ? Gu(edge.id, edge) : Yu(edge);
...
var PlaceEdgeComponent = class _PlaceEdgeComponent {
  ...
  async remove(i) {
    ...
    const err = await lastValueFrom(Ju(i.id)).catch((_) => _);
    details.close();
    if (err)
  ...
  async loadEdges() {
    ...
    const { data } = await lastValueFrom(Vu());
...
//# sourceMappingURL=chunk-FTZDK2OD.js.map
