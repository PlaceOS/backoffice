...
function generateBrokerFormFields(broker) {
  const auth_type = broker.auth_type ?? Ys.NoAuth;
  const fields = {
    ...
    username: new FormControl(broker.username, auth_type === Ys.UserPassword ? [Validators.required] : []),
    password: new FormControl(broker.password, auth_type === Ys.UserPassword ? [Validators.required] : []),
    certificate: new FormControl(broker.certificate, auth_type === Ys.Certificate ? [Validators.required] : []),
...
function generateRepositoryFormFields(repository) {
  const fields = {
    ...
    repo_type: new FormControl(repository?.repo_type || Vr.Driver),
...
function generateTriggerConditionForm(condition = {}) {
  ...
  const fields = {
    ...
    operator: new FormControl(condition.operator || oo.EQ),
...
var BrokerFormComponent = class _BrokerFormComponent {
  ...
  ngOnInit() {
    this.auth_types = [
      ...
      { id: Ys.NoAuth, name: i18n("ADMIN.BROKERS_AUTH_TYPE_NONE") },
      {
        id: Ys.UserPassword,
...
var DriverFormComponent = class _DriverFormComponent extends AsyncHandler {
  ...
  repo_list = Ac({ limit: 1e3 }).pipe(map(({ data }) => data.filter((repo) => repo.type === Vr.Driver)), shareReplay(1));
  driver_list = this.repo.pipe(filter((item) => !!item?.id), distinctUntilKeyChanged("id"), tap(() => this.loading_type.update((types) => [...types, "drivers"])), switchMap(({ id }) => Rc(id, { limit: 1e3 }).pipe(catchError(() => of([])))), map((list) => list.map((_) => ({
    id: _,
    name: _.replace(/\//g, " > ")
  }))), tap(() => this.loading_type.update((types) => types.filter((_) => _ !== "drivers"))), shareReplay(1));
  commit_list = combineLatest([this.repo, this.driver]).pipe(filter(([repo, driver]) => !!repo?.id && !!driver?.id), distinctUntilChanged(([prev_repo, prev_driver], [curr_repo, curr_driver]) => prev_repo?.id === curr_repo?.id && prev_driver?.id === curr_driver?.id), tap(() => this.loading_type.update((types) => [...types, "commits"])), switchMap(([{ id }, driver]) => Mc(id, { driver: driver.id, limit: 1e3 }).pipe(catchError(() => of([])))), map((list) => list.map((item) => ({
    id: item.commit,
    name: `${item.subject}`,
  ...
  async applyDriverCommit(commit) {
    ...
    this.subscription("driver_details", Nc(repo.id, {
      driver: `${driver.id}`,
      commit: `${commit.id}`
  ...
  async _loadDetailsFromForm() {
    ...
    if (!id)
      return;
    this.loading.set("DRIVERS.DETAILS_LOADING");
    const repo = await lastValueFrom(wc(repository_id));
    const driver = {
      id: file_name,
...
var ModuleFormComponent = class _ModuleFormComponent extends AsyncHandler {
  ...
  driver_query_fn = (_) => Uu({ q: _ }).pipe(map((resp) => resp.data));
  system_query_fn = (_) => Zc({ q: _ }).pipe(map((resp) => resp.data));
  edge_query_fn = (_) => Hu({ q: _ }).pipe(map((resp) => resp.data));
...
var RepositoryFormComponent = class _RepositoryFormComponent extends AsyncHandler {
  ...
  get is_interface() {
    return this.form()?.value?.repo_type === Vr.Interface;
  }
  ngOnInit() {
    this.repo_types = [
      { id: Vr.Driver, name: i18n("REPOS.TYPE_DRIVER") },
      {
        id: Vr.Interface,
  ...
  ngOnChanges(changes) {
    ...
    if (changes.form && form) {
      ...
      this.branch_list = credentials_trigger$.pipe(debounceTime(300), tap(() => this.loading_branches.set(true)), switchMap(() => {
        ...
        return (id ? Cc(id) : isValidUrl(uri) && uri.startsWith("http") ? qc({
          repository_url: uri,
          username,
          password
      ...
      const default_branch = this.branch_list.pipe(debounceTime(300), switchMap(() => {
        const { id, uri, username, password } = this.form().value;
        return (id ? Dc(id) : isValidUrl(uri) && uri.startsWith("http") ? Tc({
          repository_url: uri,
          username,
          password
        }) : of("")).pipe(catchError(() => of("")));
      ...
      this.commit_list = merge(timer(300), this.credentials_blur$, uri_valid$, form.get("branch").valueChanges).pipe(debounceTime(300), tap(() => this.loading_commits.set(true)), switchMap(() => {
        ...
        return (id ? Mc(id, { branch }) : isValidUrl(uri) && uri.startsWith("http") && branch ? Uc({
          repository_url: uri,
          username,
          password,
          branch
...
var UserFormComponent = class _UserFormComponent {
  ...
  domain_list = Ou().pipe(map(({ data }) => data), shareReplay(1));
...
var ZoneFormComponent = class _ZoneFormComponent extends AsyncHandler {
  ...
  async updateZone() {
    ...
    if (parent_id) {
      const zone = await Ha(parent_id).toPromise();
...
var ItemCreateUpdateModalComponent = class _ItemCreateUpdateModalComponent extends AsyncHandler {
  ...
  get item_type() {
    if (this.item instanceof Lr) {
      return "system";
    } else if (this.item instanceof Br) {
      return "module";
    } else if (this.item instanceof Mn) {
      return "zone";
    } else if (this.item instanceof Hr) {
      return "driver";
    } else if (this.item instanceof ao) {
      return "user";
    } else if (this.item instanceof Zs) {
      return "domain";
    } else if (this.item instanceof Vs) {
      return "application";
    } else if (this.item instanceof me && this._data.external_save) {
      return "system-trigger";
    } else if (this.item instanceof me) {
      return "trigger";
    } else if (this.item instanceof io) {
      return "repository";
    } else if (this.item instanceof Gs) {
      return "broker";
    }
  }
  /**
  ...
  submit() {
    ...
    const form_item = this.item.id ? hs(__spreadValues(__spreadValues({}, item_json), this.form.value), this.item_type === "user" ? [void 0, null, ""] : [void 0, null]) : __spreadValues(__spreadValues({}, item_json), this.form.value);
  ...
  async newSettings(item, settings_string) {
    const new_settings = new Ee({
      parent_id: item.id,
      settings_string,
      encryption_level: He.Support
    });
    const settings = await Gc(new_settings).toPromise().catch((err) => {
      this.loading = null;
...
*/
//# sourceMappingURL=chunk-M3PW45TH.js.map
