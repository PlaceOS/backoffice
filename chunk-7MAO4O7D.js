...
var ModuleStateService = class _ModuleStateService {
  ...
  _driver = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_driver" } : (
    ...
    loader: async ({ params: item }) => {
      if (!(item instanceof Ps))
        return null;
      return Qo(item.driver_id).catch(() => null);
    }
  }));
  ...
  _system = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_system" } : (
    ...
    params: () => this.item(),
    loader: async ({ params: item }) => {
      if (!(item instanceof Ps) || !item.system_id)
        return null;
      return da(item.system_id).catch(() => null);
    }
  }));
  ...
  _edge = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_edge" } : (
    ...
    params: () => this.item(),
    loader: async ({ params: item }) => {
      if (!(item instanceof Ps) || !item.edge_id)
        return null;
      return nu(item.edge_id).catch(() => null);
    }
  }));
  edge = computed(
  ...
  async toggleModuleState() {
    const method = this.active_item.running ? hc : ac;
...
//# sourceMappingURL=chunk-7MAO4O7D.js.map
