...
var SystemStateService = class _SystemStateService extends AsyncHandler {
    ...
    loader: async ({ params }) => {
      ...
      try {
        const [triggers, metadata] = await Promise.all([
          qa(item.id).then((response) => response.total).catch(() => 0),
          Ju(item.id).then((response) => response.length).catch(() => 0)
        ]);
        return {
  ...
  _triggers = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_triggers" } : (
    ...
    loader: async ({ params }) => {
      ...
      try {
        const response = await qa(item.id).catch(() => ({
          data: []
        }));
  ...
  async startSystem() {
    ...
    const error = await ya(this.active_item.id).then(() => null).catch((err) => {
  ...
  async stopSystem() {
    ...
    const error = await ba(this.active_item.id).then(() => null).catch((err) => {
  ...
  async newModule() {
    const mod = await this._state.edit(new Ps({
  ...
  async addModuleToSystem(device) {
    ...
    const ref = this._dialog.open(SelectItemModalComponent, {
      data: {
        ...
        query_fn: (_) => ha({ q: _ }).then((resp) => resp.data.filter((_2) => _2.id !== item.id))
      }
    });
    ...
    if (!system)
      return ref.close();
    await ma(system.id, device.id).catch((_e) => {
      ref.close();
  ...
  async selectTrigger() {
    const ref = this._dialog.open(SelectItemModalComponent, {
      data: {
        ...
        query_fn: (_) => Oa({ q: _ }).then((resp) => resp.data)
      }
    });
    const details = await Promise.race([
      waitForEvent(ref.componentInstance.event, (_) => _.reason === "action"),
  ...
  async addTrigger(trigger) {
    const t = await Pa(this.active_item.id, {
      control_system_id: this.active_item.id,
      enabled: true,
      important: false,
  ...
  async editTrigger(trigger) {
    if (this.item() && trigger) {
      ...
      const trig = await ae(url, details.metadata).catch((err) => {
  ...
  async removeTrigger(trigger) {
    ...
    if (!details?.reason)
      return;
    await Ta(this.active_item.id, trigger.id).catch((err) => {
      details.close();
  ...
  async reorderModules(fst, snd) {
    ...
    const resp = await pa(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      modules: list
    })).catch((err) => {
  ...
  async addZones(zone_list) {
    ...
    const system = await pa(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      zones
    })).catch((err) => {
  ...
  async toggleModulePower(device) {
    const method = device.running ? hc : ac;
...
//# sourceMappingURL=chunk-XB3VKKB2.js.map
