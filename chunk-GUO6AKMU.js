import {
  UserPipe
} from "./chunk-3KKYOQCW.js";
import {
  SettingsFieldComponent
} from "./chunk-HJG72ZZO.js";
import {
  DiffViewerComponent
} from "./chunk-TLAPEJ5U.js";
import {
  openConfirmModal
...
var MetadataHistoryModalComponent = class _MetadataHistoryModalComponent {
  ...
  async ngOnInit() {
    const history = await ic(this._data.id, {
      name: this._data.name,
      limit: 5e3
    });
...
var MetadataDisplayComponent = class _MetadataDisplayComponent extends AsyncHandler {
  ...
  async deleteMetadata(field) {
    ...
    if (result.reason !== "done")
      return;
    await cc(this.item().id, { name: field }).catch((err) => {
  ...
  saveMetadata(field) {
    ...
    oc(this.item().id, __spreadProps(__spreadValues({}, value), {
      ...
      if (field.name !== value.name) {
        cc(this.item().id, { name: field.name }).catch((err) => notifyError(`Error removing old "${field.name}" metadata. Error: ${JSON.stringify(err.response || err.message || err)}`));
      }
      if (index >= 0) {
        this.metadata.update((l) => {
  ...
  async loadMetadata() {
    ...
    await sc(this.item().id).then((map) => {
...
//# sourceMappingURL=chunk-GUO6AKMU.js.map
