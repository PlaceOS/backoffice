...
var BrokerFormComponent = class _BrokerFormComponent extends AsyncHandler {
  ...
  submit() {
    ...
    (form_item.id ? ku(form_item.id, form_item) : $u(form_item)).subscribe((_item) => {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(BrokerFormComponent, { className: "BrokerFormComponent", filePath: "src/app/admin/broker-form.component.ts", lineNumber: 356 });
})();
...
var AdminBrokersComponent = class _AdminBrokersComponent extends AsyncHandler {
  ...
  async deleteBroker(item) {
    if (item) {
      ...
      const err = await xu(item.id).toPromise().catch((_) => _);
      details.close();
      if (err)
  ...
  async loadBrokers() {
    ...
    const brokers = await lastValueFrom(vu().pipe(map((r) => r.data)));
...
//# sourceMappingURL=chunk-VCI6DJHX.js.map
