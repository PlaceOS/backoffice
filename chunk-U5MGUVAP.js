...
var AuthSourceModalComponent = class _AuthSourceModalComponent extends AsyncHandler {
  ...
  updateMethod(item) {
    switch (this.type()) {
      case "saml":
        return item.id ? ha(item.id, item) : la(item);
      case "ldap":
        return item.id ? ec(item.id, item) : tc(item);
    }
    return item.id ? Pc(item.id, item) : Tc(item);
  }
  /**
...
var DomainStateService = class _DomainStateService {
  ...
  async deleteAuthSource(item) {
    ...
    const method = item instanceof cr ? da : item instanceof or ? Rc : nc;
...
//# sourceMappingURL=chunk-U5MGUVAP.js.map
