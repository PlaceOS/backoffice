  ...
  _auth_sources = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_auth_sources" } : (
    ...
    loader: async ({ params }) => {
      ...
      const [saml, oauth, ldap] = await Promise.all([
        Xc(q).then((response) => response.data),
        gc(q).then((response) => response.data),
        Bu(q).then((response) => response.data)
      ]).catch(() => [[], [], []]);
  ...
  _counts = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_counts" } : (
    ...
    loader: async ({ params }) => {
      ...
      const details = await Promise.all([
        mo(q).then((response) => response.total).catch(() => 0),
        Promise.all([
          Xc(q),
          gc(q),
          Bu(q)
        ]).then(([saml, oauth, ldap]) => saml.total + oauth.total + ldap.total).catch(() => 0),
        Fa(q).then((response) => response.total).catch(() => 0)
      ]);
...
//# sourceMappingURL=chunk-URRI5RY2.js.map
