  ...
  async bulkAddGroups(existing_groups = []) {
    const groups = await lastValueFrom(this._dialog.open(GroupBulkAddModalComponent, {
      data: {
        ...
        query_fn: (query) => Vc({ q: query, limit: 20 }).pipe(map((response) => response.data)),
        exclude: (group) => !!existing_groups.find((_) => _.group_id === group.id)
      },
      height: "auto",
      width: "auto",
...
//# sourceMappingURL=chunk-7LUAVN7K.js.map
