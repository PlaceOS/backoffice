import {
  UserPipe
} from "./chunk-ZBXSBFIL.js";
import {
  SettingsFieldComponent
} from "./chunk-PAQXHCJU.js";
import {
  DiffViewerComponent
} from "./chunk-DWSZDWML.js";
import {
  validateJSONString
...
var MetadataHistoryModalComponent = class _MetadataHistoryModalComponent {
  ...
  second_details = "";
  async ngOnInit() {
    const history = await Sa(this._data.id, {
      name: this._data.name,
      limit: 5e3
    }).toPromise();
...
var MetadataDisplayComponent = class _MetadataDisplayComponent extends AsyncHandler {
  ...
  async deleteMetadata(field) {
    ...
    if (result.reason !== "done")
      return;
    await lastValueFrom(Ia(this.item().id, { name: field })).catch((err) => {
  ...
  saveMetadata(field) {
    ...
    xa(this.item().id, __spreadProps(__spreadValues({}, value), {
      ...
      next: (item) => {
        ...
        if (field.name !== value.name) {
          lastValueFrom(Ia(this.item().id, { name: field.name })).catch((err) => notifyError(`Error removing old "${field.name}" metadata. Error: ${JSON.stringify(err.response || err.message || err)}`));
        }
        if (index >= 0) {
          this.metadata.update((l) => {
...
//# sourceMappingURL=chunk-WNZOOUXC.js.map
