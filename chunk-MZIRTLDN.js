...
var AuthSourceModalComponent = class _AuthSourceModalComponent extends AsyncHandler {
  ...
  type = computed(() => this.item() instanceof Co ? "saml" : this.item() instanceof To ? "ldap" : "oauth", ...ngDevMode ? [{ debugName: "type" }] : []);
  ...
  updateMethod(item) {
    switch (this.type()) {
      case "saml":
        return item.id ? wl(item.id, item) : Il(item);
      case "ldap":
        return item.id ? ga(item.id, item) : va(item);
    }
    return item.id ? La(item.id, item) : Ba(item);
  }
  /**
...
function ApplicationFormComponent_Conditional_1_Conditional_4_For_8_Template(rf, ctx) {
  if (rf & 1) {
    const _r3 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "mat-chip-row", 17);
...
function ApplicationFormComponent_Conditional_1_Conditional_4_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(3, "translate");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(4, "mat-form-field", 14)(5, "mat-chip-grid", 15, 0);
...
function ApplicationFormComponent_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275conditionalCreate(2, ApplicationFormComponent_Conditional_1_Conditional_2_Template, 12, 8, "div", 4);
    \u0275\u0275conditionalCreate(3, ApplicationFormComponent_Conditional_1_Conditional_3_Template, 7, 6, "div", 4);
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(4, ApplicationFormComponent_Conditional_1_Conditional_4_Template, 11, 9, "div", 4);
    \u0275\u0275conditionalCreate(5, ApplicationFormComponent_Conditional_1_Conditional_5_Template, 9, 6, "div", 4);
    \u0275\u0275elementStart(6, "div", 5);
    \u0275\u0275element(7, "settings-toggle", 6);
    \u0275\u0275pipe(8, "translate");
    \u0275\u0275element(9, "settings-toggle", 7);
    \u0275\u0275pipe(10, "translate");
    \u0275\u0275elementEnd();
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.form.controls.subsystems ? 4 : -1);
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.form.controls.redirect_uri ? 5 : -1);
    \u0275\u0275advance(2);
    \u0275\u0275property("name", \u0275\u0275pipeBind1(8, 8, "DOMAINS.APP_SKIP"));
    \u0275\u0275advance(2);
    \u0275\u0275property("name", \u0275\u0275pipeBind1(10, 10, "DOMAINS.APP_PRESERVE_ID"));
    \u0275\u0275advance(2);
    \u0275\u0275conditional(ctx_r0.form.controls.redirect_uri ? 11 : -1);
  }
}
var ApplicationFormComponent = class _ApplicationFormComponent extends AsyncHandler {
  ...
  client_id = signal("", ...ngDevMode ? [{ debugName: "client_id" }] : []);
  separators = [ENTER, COMMA, SPACE];
  subsystem_list = signal([], ...ngDevMode ? [{ debugName: "subsystem_list" }] : []);
  addSubsystem = (e) => addChipItem(this.form.controls.subsystems, e);
  removeSubsystem = (i) => removeChipItem(this.form.controls.subsystems, i);
  ngOnInit() {
    const item = this._data.item;
    const edit = !!item.id;
    this.heading = i18n(`DOMAINS.APPLICATION_${edit ? "EDIT" : "NEW"}`);
    this.form = generateApplicationFormFields(item);
    this.subsystem_list = toSignal(this.form.controls.subsystems.valueChanges, {
    ...
    const redirect_uri_signal = toSignal(this.form.get("redirect_uri").valueChanges.pipe(startWith(redirect_uri || "")), {
      initialValue: redirect_uri || "",
      injector: this._injector
    });
    const preserve_client_id_signal = toSignal(this.form.get("preserve_client_id").valueChanges.pipe(startWith(!!this.form.value.preserve_client_id)), {
    ...
    effect(() => {
      ...
      if (redirect_value !== trimmed_value) {
        this.form.patchValue({ redirect_uri: trimmed_value }, { emitEvent: false });
      }
  ...
  submit() {
    ...
    (save_item.id ? Wu(save_item.id, save_item) : Lu(save_item)).subscribe((_item) => {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _ApplicationFormComponent, selectors: [["application-form"]], outputs: { event: "event" }, features: [\u0275\u0275InheritDefinitionFeature], decls: 2, vars: 3, consts: [["chipList", ""], [3, "save", "heading", "loading"], ["application", "", 1, "flex", "flex-col", 3, "formGroup"], [1, "fieldset"], [1, "field"], [1, "fieldset", "mb-4"], ["formControlName", "skip_authorization", 1, "flex-1", 3, "name"], ["formControlName", "preserve_client_id", 1, "flex-1", 3, "name"], ["for", "application-name"], ["appearance", "outline"], ["matInput", "", "name", "application-name", "placeholder", "Application Name", "formControlName", "name", "required", ""], ["for", "scopes"], ["matInput", "", "name", "scopes", "formControlName", "scopes", 3, "placeholder"], ["for", "subsystems-input"], ["appearance", "outline", 1, "w-full"], ["aria-label", "Subsystem List"], ["id", "subsystems-input", 3, "matChipInputTokenEnd", "placeholder", "matChipInputFor", "matChipInputSeparatorKeyCodes", "matChipInputAddOnBlur"], [3, "removed"], [1, "max-w-md", "truncate"], ["matChipRemove", ""], ["for", "redirect-uri"], ["matInput", "", "name", "redirect-uri", "placeholder", "Redirect URI e.g. http://localhost:4200/oauth-resp.html", "formControlName", "redirect_uri"], ["for", "client-id"], ["matInput", "", "name", "client-id", 3, "placeholder", "disabled", "ngModel", "ngModelOptions"]], template: function ApplicationFormComponent_Template(rf, ctx) {
    if (rf & 1) {
      ...
      \u0275\u0275conditionalCreate(1, ApplicationFormComponent_Conditional_1_Template, 12, 12, "form", 2);
      \u0275\u0275elementEnd();
    }
    if (rf & 2) {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ApplicationFormComponent, { className: "ApplicationFormComponent", filePath: "src/app/domains/application-form.component.ts", lineNumber: 230 });
})();
...
var DomainStateService = class _DomainStateService {
  ...
  users = combineLatest([
    this._changed,
    this.item
  ]).pipe(filter(([, item]) => item instanceof Ao), switchMap(([, item]) => fh({ authority_id: item.id, limit: 1e3 })), map((response) => response.data.sort((a, b) => a.name.localeCompare(b.name))), catchError(() => []), shareReplay(1));
  auth_sources = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof Ao), switchMap(([, item]) => {
    const q = { authority_id: item.id };
    return combineLatest([
      xl(q).pipe(map((response) => response.data)),
      ja(q).pipe(map((response) => response.data)),
      ma(q).pipe(map((response) => response.data))
    ]);
  }), map((sources) => {
    let list = [];
  ...
  applications = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof Ao), switchMap(([, item]) => zu({ authority_id: item.id })), map((response) => response.data.sort((a, b) => a.name.localeCompare(b.name))), catchError(() => []), shareReplay(1));
  counts = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof Ao), switchMap(async ([, item]) => {
    const q = { authority_id: item?.id };
    const details = await Promise.all([
      lastValueFrom(zu(q).pipe(map((response) => response.total))),
      lastValueFrom(combineLatest([
        xl(q),
        ja(q),
        ma(q)
      ]).pipe(map(([saml, oauth, ldap]) => saml.total + oauth.total + ldap.total))),
      lastValueFrom(fh(q).pipe(map((response) => response.total)))
    ]);
  ...
  async update(domain) {
    const item = await pc(domain.id, domain).toPromise();
  ...
  async performAzureIntegration() {
    ...
    if (!(item instanceof Ao))
      return;
    const result = await lastValueFrom(v(`/api/engine/v2/admin_consent/${encodeURIComponent(item.id)}`)).catch((error) => {
  ...
  async editApplication(item) {
    item = item || new go({ owner_id: this.active_item.id });
    const ref = this._dialog.open(ApplicationFormComponent, {
      data: {
        ...
        save: (i) => {
          delete i.client_id;
          return i.id ? Wu(i.id, i) : Lu(i);
        }
      }
    });
    const instance = ref.componentInstance;
    const details = await Promise.race([
      lastValueFrom(instance.event.pipe(first((_) => _.reason === "done"))),
  ...
  async deleteApplication(item) {
    ...
    const err = await Bu(item.id).toPromise().catch((_) => _);
    details.close();
    if (err)
  ...
  async deleteAuthSource(item) {
    ...
    const method = item instanceof Co ? Pl : item instanceof Ro ? Ga : ba;
...
//# sourceMappingURL=chunk-MZIRTLDN.js.map
