...
var AuthSourceModalComponent = class _AuthSourceModalComponent extends AsyncHandler {
  ...
  type = computed(() => this.item() instanceof co ? "saml" : this.item() instanceof ro ? "ldap" : "oauth", ...ngDevMode ? [{ debugName: "type" }] : []);
  ...
  updateMethod(item) {
    switch (this.type()) {
      case "saml":
        return item.id ? Qc(item.id, item) : Gc(item);
      case "ldap":
        return item.id ? tc(item.id, item) : nc(item);
    }
    return item.id ? wc(item.id, item) : Pc(item);
  }
  /**
...
function ApplicationFormComponent_Conditional_1_Conditional_10_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(3);
    \u0275\u0275property("placeholder", \u0275\u0275pipeBind1(6, 7, "DOMAINS.APP_CLIENT_PLACEHOLDER"))("disabled", true)("ngModel", ctx_r0.client_id())("ngModelOptions", \u0275\u0275pureFunction0(9, _c03));
  }
}
function ApplicationFormComponent_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "form", 1)(1, "div", 2);
    \u0275\u0275conditionalCreate(2, ApplicationFormComponent_Conditional_1_Conditional_2_Template, 12, 8, "div", 3);
    \u0275\u0275conditionalCreate(3, ApplicationFormComponent_Conditional_1_Conditional_3_Template, 7, 6, "div", 3);
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(4, ApplicationFormComponent_Conditional_1_Conditional_4_Template, 9, 6, "div", 3);
    \u0275\u0275elementStart(5, "div", 4);
    \u0275\u0275element(6, "settings-toggle", 5);
    \u0275\u0275pipe(7, "translate");
    \u0275\u0275element(8, "settings-toggle", 6);
    \u0275\u0275pipe(9, "translate");
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(10, ApplicationFormComponent_Conditional_1_Conditional_10_Template, 7, 10, "div", 3);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
...
var ApplicationFormComponent = class _ApplicationFormComponent extends AsyncHandler {
  ...
  submit() {
    ...
    (save_item.id ? yu(save_item.id, save_item) : gu(save_item)).subscribe((_item) => {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ApplicationFormComponent, { className: "ApplicationFormComponent", filePath: "src/app/domains/application-form.component.ts", lineNumber: 172 });
})();
...
var DomainStateService = class _DomainStateService {
  ...
  users = combineLatest([
    this._changed,
    this.item
  ]).pipe(filter(([, item]) => item instanceof to), switchMap(([, item]) => Ta({ authority_id: item.id, limit: 1e3 })), map((response) => response.data.sort((a, b) => a.name.localeCompare(b.name))), catchError(() => []), shareReplay(1));
  auth_sources = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof to), switchMap(([, item]) => {
    const q = { authority_id: item.id };
    return combineLatest([
      Bc(q).pipe(map((response) => response.data)),
      xc(q).pipe(map((response) => response.data)),
      Xu(q).pipe(map((response) => response.data))
    ]);
  }), map((sources) => {
    let list = [];
  ...
  applications = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof to), switchMap(([, item]) => _u({ authority_id: item.id })), map((response) => response.data.sort((a, b) => a.name.localeCompare(b.name))), catchError(() => []), shareReplay(1));
  counts = combineLatest([this._changed, this.item]).pipe(filter(([, item]) => item instanceof to), switchMap(async ([, item]) => {
    const q = { authority_id: item?.id };
    const details = await Promise.all([
      lastValueFrom(_u(q).pipe(map((response) => response.total))),
      lastValueFrom(combineLatest([
        Bc(q),
        xc(q),
        Xu(q)
      ]).pipe(map(([saml, oauth, ldap]) => saml.total + oauth.total + ldap.total))),
      lastValueFrom(Ta(q).pipe(map((response) => response.total)))
    ]);
  ...
  async update(domain) {
    const item = await Uu(domain.id, domain).toPromise();
  ...
  async editApplication(item) {
    item = item || new Ys({ owner_id: this.active_item.id });
  ...
  async deleteAuthSource(item) {
    ...
    const method = item instanceof co ? Yc : item instanceof oo ? Oc : rc;
...
//# sourceMappingURL=chunk-FBNBK5KU.js.map
