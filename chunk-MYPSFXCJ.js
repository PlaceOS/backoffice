...
var ZonesStateService = class _ZonesStateService {
    ...
    loader: async ({ params }) => {
      ...
      try {
        const details = await Promise.all([
          ha({ zone_id: item.id, limit: 1 }).then((d) => d.total).catch(() => 0),
          oh(item.id).then((d) => d.total).catch(() => 0),
          Ju(item.id).then((d) => d.length).catch(() => 0),
          eh({ parent_id: item.id, limit: 1 }).then((d) => d.total).catch(() => 0),
          zu({ zone_id: item.id, limit: 1 }).then((d) => d.total).catch(() => 0)
        ]);
  ...
  _systems = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_systems" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Yt))
        return [];
      const response = await ha({ zone_id: item.id }).catch(() => ({ data: [] }));
  ...
  _triggers = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_triggers" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Yt))
        return [];
      const response = await oh(item.id).catch(() => ({
        data: []
      }));
      return response.data;
    }
  ...
  _children = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_children" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Yt))
        return [];
      const response = await eh({ parent_id: item.id }).catch(() => ({ data: [] }));
      return response.data;
    }
  }));
  children = computed(
  ...
  get authority_id() {
    return this.active_item?.authority_id || It()?.id || void 0;
  }
  constructor() {
  ...
  async addTrigger(trigger) {
    ...
    return sh(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
  ...
  async removeTrigger(trigger) {
    ...
    if (!details?.reason)
      return;
    const zone = await sh(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
  ...
  async bulkAddGroups(existing_groups = []) {
    ...
    if (!groups?.length)
      return;
    this._loading.set(true);
    const results = await Promise.allSettled(groups.map((group) => Lu({
      group_id: group.id,
      zone_id: this.active_item.id
    })));
    this._loading.set(false);
  ...
  async removeGroup(item) {
    ...
    await Gu(item.group_id, item.zone_id).catch((error) => {
      details.close();
...
//# sourceMappingURL=chunk-MYPSFXCJ.js.map
