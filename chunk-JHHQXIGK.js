...
var RepositoriesStateService = class _RepositoriesStateService {
  ...
  _driver_list = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_driver_list" } : (
    ...
    loader: async ({ params: item }) => {
      if (!(item instanceof Zi) || item.repo_type === As.Interface)
        return [];
      this._loading.set(true);
      this._driver_list_error.set("");
      try {
        return await Pc(item.id, { limit: 2e3 });
      } catch (err) {
  ...
  async pullLatestCommit() {
    const commit = await Ec(this.active_item.id).catch((err) => {
    ...
    if (!commit)
      return;
    const repo = await $c(this.active_item.id);
    if (repo)
...
//# sourceMappingURL=chunk-JHHQXIGK.js.map
