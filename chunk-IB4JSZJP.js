...
var AdminBrokersComponent = class _AdminBrokersComponent extends AsyncHandler {
  ...
  async deleteBroker(item) {
    if (item) {
      ...
      const err = await yu(item.id).toPromise().catch((_) => _);
      details.close();
      if (err)
  ...
  async loadBrokers() {
    ...
    const brokers = await lastValueFrom(du().pipe(map((r) => r.data)));
...
//# sourceMappingURL=chunk-IB4JSZJP.js.map
