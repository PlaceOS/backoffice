...
import {
  ...
  addChipItem,
  removeChipItem
} from "./chunk-XOD6WJGY.js";
import {
  AuthenticatedImageDirective
} from "./chunk-AIHAF754.js";
import {
  MatChipGrid,
  MatChipInput,
  MatChipRemove,
  MatChipRow,
  MatChipsModule
} from "./chunk-ND6UXKI3.js";
import {
  DateFromPipe
} from "./chunk-OWJOYUIE.js";
import {
  Clipboard
} from "./chunk-BFHNQYWR.js";
import {
  HotkeysService
} from "./chunk-RKP6PC35.js";
import {
  toObservable,
  toSignal
...
} from "./chunk-JIQ6FGOU.js";
import {
  SimpleTableComponent
} from "./chunk-WHWJVSMC.js";
import {
  FullscreenModalShellComponent
} from "./chunk-E3PNC77F.js";
import {
  MatProgressSpinner,
  MatProgressSpinnerModule
} from "./chunk-YBTJMEJ7.js";
import {
  UploadsService
} from "./chunk-44XOPICB.js";
import {
  SettingsToggleComponent
} from "./chunk-TOOU46W3.js";
import {
  MatInput,
  MatInputModule
...
} from "./chunk-RCORFESO.js";
import {
  SettingsService,
  format
} from "./chunk-DOZGI6ZZ.js";
import {
  toDate
...
var BulkItemModalComponent = class _BulkItemModalComponent {
  ...
  generateTemplate() {
    switch (this._data.constr) {
      case Qr:
        return [new Qr(SYSTEM_TEMPLATE).toJSON()];
      case Gr:
        return [new Gr(MODULE_TEMPLATE).toJSON()];
      case Br:
        return [new Br(DRIVER_TEMPLATE).toJSON()];
      case fo:
        return [new fo(USER_TEMPLATE).toJSON()];
      case Dn:
        return [new Dn(ZONE_TEMPLATE).toJSON()];
    }
  }
  getRequiredFields() {
    switch (this._data.constr) {
  ...
  findDuplicates() {
    ...
    const dedup_field = this._data.constr === fo ? "email" : "name";
    this.item_list.forEach((item, index) => {
...
var DomainFormComponent = class _DomainFormComponent extends AsyncHandler {
  ...
  submit() {
    ...
    const form_item = item.id ? ds(__spreadValues(__spreadValues({}, item_json), this.form.value), [void 0]) : __spreadValues(__spreadValues({}, item_json), this.form.value);
    (form_item.id ? Uu(form_item.id, form_item) : Ru(form_item)).subscribe((_item) => {
  ...
  async newSettings(item, settings_string) {
    const new_settings = new Oe({
      parent_id: item.id,
      settings_string,
      encryption_level: He.Support
    });
    await Xc(new_settings).toPromise().catch((err) => {
      this.loading = null;
...
function generateDriverFormFields(driver) {
  const fields = {
    ...
    role: new FormControl(driver?.role || Mt.Logic),
...
var DriverFormComponent = class _DriverFormComponent extends AsyncHandler {
  ...
  role_types = [
    { id: Mt.SSH, name: "DRIVERS.SSH" },
    { id: Mt.Device, name: "DRIVERS.DEVICE" },
    { id: Mt.Service, name: "DRIVERS.SERVICE" },
    { id: Mt.Websocket, name: "DRIVERS.WEBSOCKET" },
    { id: Mt.Logic, name: "DRIVERS.LOGIC" }
  ];
  alert_levels = [
  ...
  repo_list = Ec({ limit: 1e3 }).pipe(map(({ data }) => data.filter((repo) => repo.type === Yr.Driver)), shareReplay(1));
  driver_list = toObservable(this.repo).pipe(filter((item) => !!item?.id), distinctUntilKeyChanged("id"), tap(() => this.loading_type.update((types) => [...types, "drivers"])), switchMap(({ id }) => Nc(id, { limit: 1e3 }).pipe(catchError(() => of([])))), map((list) => list.map((_) => ({
    id: _,
    name: _.replace(/\//g, " > ")
  ...
  ]).pipe(filter(([repo, driver]) => !!repo?.id && !!driver?.id), distinctUntilChanged(([prev_repo, prev_driver], [curr_repo, curr_driver]) => prev_repo?.id === curr_repo?.id && prev_driver?.id === curr_driver?.id), tap(() => this.loading_type.update((types) => [...types, "commits"])), switchMap(([{ id }, driver]) => Fc(id, {
    driver: driver.id,
    limit: 1e3
  ...
  async applyDriverCommit(commit) {
    ...
    const details = await lastValueFrom(Hc(repo.id, {
      driver: `${driver.id}`,
      commit: `${commit.id}`
  ...
  submit() {
    ...
    const item_json = item.toJSON ? item.toJSON() : item;
    const form_item = item.id ? ds(__spreadValues(__spreadValues({}, item_json), this.form.value), [void 0]) : __spreadValues(__spreadValues({}, item_json), this.form.value);
    (form_item.id ? Fu(form_item.id, form_item) : ju(form_item)).subscribe((_item) => {
      this._dialog_ref.disableClose = false;
      this.event.emit({
        reason: "done",
        metadata: { item: _item }
  ...
  _applyDriverDetails(details) {
    ...
    this.form.patchValue({
      ...
      role: port_number ? port_number === 22 ? Mt.SSH : Mt.Device : details_any.uri_base ? details_any.uri_base.startsWith("ws") ? Mt.Websocket : Mt.Service : Mt.Logic,
  ...
  async _loadDetailsFromForm() {
    ...
    if (!id)
      return;
    this.loading.set("DRIVERS.DETAILS_LOADING");
    const repo = await lastValueFrom(Ic(repository_id));
    const driver = {
      id: file_name,
...
function generateModuleFormFields(mod) {
  const fields = {
    ...
    role: new FormControl(mod?.role || Mt.Logic),
  ...
  if (!mod?.id) {
    ...
    fields.driver.valueChanges.subscribe((value) => {
      ...
      fields.role.setValue(value.role || Mt.Logic);
...
var ModuleFormComponent = class _ModuleFormComponent extends AsyncHandler {
  ...
  driver_query_fn = (_) => Du({ q: _ }).pipe(map((resp) => resp.data));
  system_query_fn = (_) => na({ q: _ }).pipe(map((resp) => resp.data));
  edge_query_fn = (_) => Vu({ q: _ }).pipe(map((resp) => resp.data));
...
function generateRepositoryFormFields(repository) {
  const fields = {
    ...
    repo_type: new FormControl(repository?.repo_type || Yr.Driver),
...
var RepositoryFormComponent = class _RepositoryFormComponent extends AsyncHandler {
  ...
  is_interface = computed(() => this.form?.value?.repo_type === Yr.Interface, ...ngDevMode ? [{ debugName: "is_interface" }] : []);
  get hide_uri() {
  ...
  ngOnInit() {
    ...
    this.repo_types = [
      { id: Yr.Driver, name: i18n("REPOS.TYPE_DRIVER") },
      {
        id: Yr.Interface,
  ...
  submit() {
    ...
    const form_item = item.id ? ds(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
    (form_item.id ? Tc(form_item.id, form_item) : qc(form_item)).subscribe((_item) => {
      this._dialog_ref.disableClose = false;
      this.event.emit({ reason: "done", metadata: { item: _item } });
      notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
      if (!this.form.value.id && this.form.controls.settings) {
  ...
  _setupBranchAndCommitStreams() {
    ...
    this.branch_list = credentials_trigger$.pipe(debounceTime(300), tap(() => this.loading_branches.set(true)), switchMap(() => {
      ...
      return (id ? jc(id) : isValidUrl(uri) && uri.startsWith("http") ? Cc({
        repository_url: uri,
        username,
        password
    ...
    const default_branch = this.branch_list.pipe(debounceTime(300), switchMap(() => {
      const { id, uri, username, password } = this.form.value;
      return (id ? zc(id) : isValidUrl(uri) && uri.startsWith("http") ? Mc({
        repository_url: uri,
        username,
        password
      }) : of("")).pipe(catchError(() => of("")));
    ...
    this.commit_list = merge(timer(300), this._cred_blur.pipe(skip(1)), uri_valid$, this.form.get("branch").valueChanges).pipe(debounceTime(300), tap(() => this.loading_commits.set(true)), switchMap(() => {
      ...
      return (id ? Fc(id, { branch }) : isValidUrl(uri) && uri.startsWith("http") && branch ? Dc({
        repository_url: uri,
        username,
        password,
        branch
...
function generateSystemsFormFields(system) {
  const fields = {
    ...
    camera_snapshot_urls: new FormControl(system.camera_snapshot_urls ? system.camera_snapshot_urls : [system.camera_snapshot_url].filter(Boolean), [validateURLArray]),
...
function SystemFormComponent_Conditional_1_Conditional_40_For_8_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275attribute("aria-label", \u0275\u0275pipeBind2(4, 2, "SYSTEMS.REMOVE_ITEM", \u0275\u0275pureFunction1(5, _c07, item_r9)));
  }
}
...
var SystemFormComponent = class _SystemFormComponent extends AsyncHandler {
  ...
  submit() {
    ...
    (processed_item.id ? sa(processed_item.id, processed_item) : oa(processed_item)).subscribe((_item) => {
      this._dialog_ref.disableClose = false;
      this.event.emit({ reason: "done", metadata: { item: _item } });
      notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
      if (!this.form.value.id && this.form.controls.settings) {
...
function generateTriggerConditionForm(condition = {}) {
  ...
  const fields = {
    ...
    operator: new FormControl(condition.operator || ao.EQ),
...
var TriggerFormComponent = class _TriggerFormComponent extends AsyncHandler {
  ...
  submit() {
    ...
    const item_json = item.toJSON ? item.toJSON() : item;
    const form_item = item.id ? ds(__spreadValues(__spreadValues({}, item_json), this.form.value), [void 0]) : __spreadValues(__spreadValues({}, item_json), this.form.value);
    (form_item.id ? Pa(form_item.id, form_item) : Oa(form_item)).subscribe((_item) => {
      this._dialog_ref.disableClose = false;
      this.event.emit({ reason: "done", metadata: { item: _item } });
      notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
      if (!this.form.value.id && this.form.controls.settings) {
...
var UserFormComponent = class _UserFormComponent extends AsyncHandler {
  ...
  domain_list = Tu().pipe(map(({ data }) => data), shareReplay(1));
  ...
  submit() {
    ...
    this._dialog_ref.disableClose = true;
    const item_json = item.toJSON ? item.toJSON() : item;
    const form_item = item.id ? ds(__spreadValues(__spreadValues({}, item_json), this.form.value), [
      void 0,
      null,
      ""
    ]) : __spreadValues(__spreadValues({}, item_json), this.form.value);
    (form_item.id ? Ra(form_item.id, form_item) : Ma(form_item)).subscribe((_item) => {
      this._dialog_ref.disableClose = false;
      this.event.emit({ reason: "done", metadata: { item: _item } });
      notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
      this._dialog_ref.close();
...
var ZoneFormComponent = class _ZoneFormComponent extends AsyncHandler {
  ...
  submit() {
    ...
    const item_json = item.toJSON ? item.toJSON() : item;
    const form_item = item.id ? ds(__spreadValues(__spreadValues({}, item_json), this.form.value), [void 0]) : __spreadValues(__spreadValues({}, item_json), this.form.value);
    (form_item.id ? Qa(form_item.id, form_item) : Ga(form_item)).subscribe((_item) => {
      this._dialog_ref.disableClose = false;
      this.event.emit({ reason: "done", metadata: { item: _item } });
      notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
      if (!this.form.value.id && this.form.controls.settings) {
  ...
  async updateZone() {
    ...
    if (parent_id) {
      const zone = await Va(parent_id).toPromise();
...
var domains = {
  ...
  show: (_) => qu(_),
  save: (item) => item.id ? Uu(item.id, item) : Ru(item),
...
var drivers = {
  ...
  show: (_) => Nu(_),
  save: (item) => item.id ? Fu(item.id, item) : ju(item),
  remove: (item) => zu(item.id),
  itemConstructor: Br,
  ...
  delete_extra: async (_) => {
    ...
    const count = await hc(query).pipe(map(({ total }) => total)).toPromise().catch((_err) => 0);
    return count ? [
      "error",
...
var repositories = {
  ...
  show: (_) => Ic(_),
  save: (item) => item.id ? Tc(item.id, item) : qc(item),
  remove: (item) => Uc(item.id),
  itemConstructor: uo,
...
var systems = {
  ...
  show: (_) => ia(_),
  save: (item) => item.id ? sa(item.id, __spreadProps(__spreadValues({}, item), {
    support_url: processURL(item, item.support_url)
  })) : oa(__spreadProps(__spreadValues({}, item), {
    support_url: processURL(item, item.support_url)
  })),
  remove: (item) => ua(item.id),
  itemConstructor: Qr,
...
var triggers = {
  ...
  show: (_) => wa(_),
  save: (item) => item.id ? Pa(item.id, item) : Oa(item),
  remove: (item) => Ea(item.id),
  itemConstructor: me,
...
var users = {
  ...
  show: (_) => qa(_),
  save: (item) => item.id ? Ra(item.id, item) : Ma(item),
  remove: (item) => Ca(item.id),
  itemConstructor: fo,
...
var zones = {
  ...
  show: (_) => Va(_),
  save: (item) => item.id ? Qa(item.id, item) : Ga(item),
  remove: (item) => Ya(item.id),
  itemConstructor: Dn,
...
var ActiveItemService = class _ActiveItemService extends AsyncHandler {
  ...
  async updateSettings() {
    ...
    if (item && item.settings) {
      let settings = await lastValueFrom(Jc({ parent_id: item.id }).pipe(map((resp) => resp.data)));
      settings = new Array(5).fill(0).map((_, idx) => settings.find((_2) => _2.encryption_level === idx) || new Oe({
...
*/
//# sourceMappingURL=chunk-XLTJ4T7V.js.map
