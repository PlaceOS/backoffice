import {
  subMinutes
} from "./chunk-G7PKMVDI.js";
import {
  AuthenticatedImageDirective
} from "./chunk-Q3UICFYO.js";
import {
...
import {
  ...
  MatChipRow,
  MatChipsModule
} from "./chunk-TY5J7Y43.js";
import {
  DateFromPipe
} from "./chunk-DINJ4675.js";
import {
  Clipboard
} from "./chunk-KQ433EDT.js";
import {
  HotkeysService
} from "./chunk-YXSE7SMW.js";
import {
  addSignalChipItem,
...
import {
  ...
  submit,
  validate
} from "./chunk-E6MC4KZ6.js";
import {
  CONFIRM_METADATA,
  ConfirmModalComponent
} from "./chunk-XDJFKACP.js";
import {
  SimpleTableComponent
} from "./chunk-RZD44Q3D.js";
import {
  FullscreenModalShellComponent
} from "./chunk-WMLGKGFL.js";
import {
  MatProgressSpinner,
...
} from "./chunk-HD5UPPVV.js";
import {
  SettingsService,
  format
} from "./chunk-34SZHLRP.js";
import {
  toDate
...
var BulkItemModalComponent = class _BulkItemModalComponent {
  ...
  generateTemplate() {
    switch (this._data.constr) {
      case ms:
        return [new ms(SYSTEM_TEMPLATE).toJSON()];
      case gs:
        return [new gs(MODULE_TEMPLATE).toJSON()];
      case ls:
        return [new ls(DRIVER_TEMPLATE).toJSON()];
      case bn:
        return [new bn(USER_TEMPLATE).toJSON()];
      case Ft:
        return [new Ft(ZONE_TEMPLATE).toJSON()];
    }
  }
  getRequiredFields() {
    switch (this._data.constr) {
  ...
  findDuplicates() {
    ...
    const dedup_field = this._data.constr === bn ? "email" : "name";
    this.item_list.forEach((item, index) => {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(DuplicateModalComponent, { className: "DuplicateModalComponent", filePath: "src/app/overlays/duplicate-modal.component.ts", lineNumber: 163 });
})();
...
var DomainFormComponent = class _DomainFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Ws(__spreadValues(__spreadValues({}, item_json), this.formModel()), [void 0]) : __spreadValues(__spreadValues({}, item_json), this.formModel());
      try {
        const _item = await (form_item.id ? qo(form_item.id, form_item) : Po(form_item));
  ...
  async newSettings(item, settings_string) {
    const new_settings = new Ae({
      parent_id: item.id,
      settings_string,
      encryption_level: Me.Support
    });
    await Gc(new_settings).catch((err) => {
      this.loading = null;
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _DomainFormComponent, selectors: [["domain-form"]], outputs: { event: "event" }, features: [\u0275\u0275InheritDefinitionFeature], decls: 2, vars: 3, consts: [["chipList", ""], [3, "save", "heading", "loading"], ["domain", "", 1, "flex", "flex-col"], [1, "fieldset"], [1, "field"], ["for", "domain-name"], ["appearance", "outline"], ["matInput", "", 3, "placeholder", "formField"], ["for", "domain"], ["for", "login-url"], ["for", "logout-url"], ["for", "description"], ["for", "email-domains-input"], ["appearance", "outline", 1, "w-full"], ["aria-label", "Image List"], ["id", "email-domains-input", 3, "matChipInputTokenEnd", "placeholder", "matChipInputFor", "matChipInputSeparatorKeyCodes", "matChipInputAddOnBlur"], [3, "removed"], [1, "max-w-md", "truncate"], ["matChipRemove", ""]], template: function DomainFormComponent_Template(rf, ctx) {
    if (rf & 1) {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(DomainFormComponent, { className: "DomainFormComponent", filePath: "src/app/domains/domain-form.component.ts", lineNumber: 275 });
})();
...
function DriverFormComponent_Conditional_3_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275repeaterCreate(58, DriverFormComponent_Conditional_3_For_59_Template, 3, 4, "mat-option", 24, _forTrack04);
    \u0275\u0275elementEnd();
    \u0275\u0275controlCreate();
    \u0275\u0275elementEnd()();
  }
...
var DriverFormComponent = class _DriverFormComponent extends AsyncHandler {
  ...
  role_types = [
    { id: At.SSH, name: "DRIVERS.SSH" },
    { id: At.Device, name: "DRIVERS.DEVICE" },
    { id: At.Service, name: "DRIVERS.SERVICE" },
    { id: At.Websocket, name: "DRIVERS.WEBSOCKET" },
    { id: At.Logic, name: "DRIVERS.LOGIC" }
  ];
  alert_levels = [
  ...
  _repo_list = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_repo_list" } : (
    ...
    try {
      const { data } = await dc({ limit: 1e3 });
      return data.filter((repo) => repo.type === bs.Driver);
    } finally {
  ...
  async applyDriverCommit(commit) {
    ...
    const details = await qc(repo.id, {
      driver: `${driver.id}`,
      commit: `${commit.id}`
    }).catch(() => null);
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Ws(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? Co(form_item.id, form_item) : Eo(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({
          reason: "done",
          metadata: { item: _item }
  ...
  _applyDriverDetails(details) {
    ...
    this.formModel.update((value) => __spreadProps(__spreadValues({}, value), {
      ...
      role: port_number ? port_number === 22 ? At.SSH : At.Device : details_any.uri_base ? details_any.uri_base.startsWith("ws") ? At.Websocket : At.Service : At.Logic,
  ...
  async _loadDetailsFromForm() {
    ...
    if (!id)
      return;
    this.loading.set("DRIVERS.DETAILS_LOADING");
    const repo = await pc(repository_id);
    const driver = {
      id: file_name,
  ...
  _setLoadingType(type2, loading) {
    this.loading_type.update((types) => {
      const list = types.filter((_) => _ !== type2);
      return loading ? [...list, type2] : list;
    });
  }
...
function generateGroupFormModel(group) {
  return {
    ...
    authority_id: group?.authority_id || St()?.id || "",
...
function GroupFormComponent_For_45_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275attribute("aria-label", \u0275\u0275pipeBind2(4, 2, "COMMON.ITEM_REMOVE", \u0275\u0275pureFunction1(5, _c12, subsystem_r3)));
  }
}
var GroupFormComponent = class _GroupFormComponent extends AsyncHandler {
  ...
  _domain_list = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_domain_list" } : (
    /* istanbul ignore next */
    {}
  )), { loader: async () => (await xo({ limit: 1e3 })).data }));
  ...
  query_parent_groups = (_) => ou({ q: _, limit: 20 }).then(({ data }) => data);
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = Ws(__spreadProps(__spreadValues(__spreadValues({}, item), form_value), {
        subsystems: form_value.subsystems || []
      }), ["", void 0]);
      try {
        const _item = await (form_item.id ? hu(form_item.id, form_item) : au(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _GroupFormComponent, selectors: [["group-form"]], outputs: { event: "event" }, features: [\u0275\u0275InheritDefinitionFeature], decls: 48, vars: 48, consts: [["chipList", ""], [3, "save", "heading", "loading"], [1, "flex", "flex-col"], [1, "field"], ["for", "group-name"], ["appearance", "outline"], ["matInput", "", "id", "group-name", 3, "placeholder", "formField"], ["for", "group-description"], ["matInput", "", "id", "group-description", 3, "placeholder", "formField"], [1, "fieldset"], ["for", "group-parent"], [3, "ngModelChange", "placeholder", "query_fn", "exclude", "ngModel", "ngModelOptions"], ["for", "group-authority"], [3, "placeholder", "formField"], [3, "value"], ["for", "group-subsystems"], ["appearance", "outline", 1, "w-full"], ["aria-label", "Subsystem List"], ["id", "group-subsystems", "name", "group-subsystems", 3, "matChipInputTokenEnd", "placeholder", "matChipInputFor", "matChipInputSeparatorKeyCodes", "matChipInputAddOnBlur"], [3, "removed"], [1, "max-w-md", "truncate"], ["matChipRemove", ""]], template: function GroupFormComponent_Template(rf, ctx) {
    ...
    if (rf & 2) {
      ...
      \u0275\u0275advance(2);
      \u0275\u0275property("placeholder", \u0275\u0275pipeBind1(27, 37, "GROUPS.PARENT_SEARCH"))("query_fn", ctx.query_parent_groups)("exclude", ctx.exclude_parent_group)("ngModel", ctx.parent_group())("ngModelOptions", \u0275\u0275pureFunction0(47, _c04));
      \u0275\u0275control();
      \u0275\u0275advance(4);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(GroupFormComponent, { className: "GroupFormComponent", filePath: "src/app/groups/group-form.component.ts", lineNumber: 185 });
})();
...
var applyModuleFormSchema = (path) => {
  ...
  required(path.uri, {
    when({ valueOf }) {
      ...
      return role === At.Service || role === At.Websocket;
    }
  });
  required(path.ip, {
    when({ valueOf }) {
      const role = valueOf(path.role);
      return role === At.Device || role === At.SSH;
    }
  });
  required(path.port, {
    when({ valueOf }) {
  ...
  required(path.system, {
    when({ valueOf }) {
      return valueOf(path.role) === At.Logic;
    }
  });
};
...
function ModuleFormComponent_Conditional_1_Conditional_2_Conditional_3_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(5, "mat-form-field", 12);
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind1(3, 6, "MODULES.FIELD_IP"), " ");
    \u0275\u0275advance(2);
    ...
    \u0275\u0275control();
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.form.ip().invalid() ? 7 : -1);
  }
}
...
function ModuleFormComponent_Conditional_1_Conditional_2_Conditional_4_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind1(3, 7, "MODULES.PORT_NUMBER"), " ");
    \u0275\u0275advance(2);
    \u0275\u0275conditional(ctx_r0.role() === "ssh" || ctx_r0.role() === "device" ? 4 : -1);
    \u0275\u0275advance(2);
    \u0275\u0275property("placeholder", \u0275\u0275pipeBind1(7, 9, "MODULES.PORT_NUMBER"))("formField", ctx_r0.form.port);
    \u0275\u0275control();
    \u0275\u0275advance(2);
    \u0275\u0275conditional(ctx_r0.form.port().invalid() ? 8 : -1);
  }
}
...
function ModuleFormComponent_Conditional_1_Conditional_2_Conditional_10_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275repeaterCreate(6, ModuleFormComponent_Conditional_1_Conditional_2_Conditional_10_For_7_Template, 3, 4, "mat-option", 20, _forTrack06);
    \u0275\u0275elementEnd();
    \u0275\u0275controlCreate();
    \u0275\u0275elementEnd()();
  }
...
function ModuleFormComponent_Conditional_1_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275conditionalCreate(3, ModuleFormComponent_Conditional_1_Conditional_2_Conditional_3_Template, 8, 8, "div", 2);
    \u0275\u0275conditionalCreate(4, ModuleFormComponent_Conditional_1_Conditional_2_Conditional_4_Template, 9, 11, "div", 2);
    \u0275\u0275elementEnd();
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(2);
    \u0275\u0275conditional(ctx_r0.form.ip && !(ctx_r0.role() === "service" || ctx_r0.role() === "websocket") ? 3 : -1);
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.form.port && !(ctx_r0.role() === "service" || ctx_r0.role() === "websocket") ? 4 : -1);
    \u0275\u0275advance(2);
    \u0275\u0275conditional(ctx_r0.form.tls && !(ctx_r0.role() === "service" || ctx_r0.role() === "websocket") ? 6 : -1);
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.form.udp && !(ctx_r0.role() === "service" || ctx_r0.role() === "websocket") ? 7 : -1);
    \u0275\u0275advance();
...
function ModuleFormComponent_Conditional_1_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275conditional(!ctx_r0.form.driver || ctx_r0.formModel().driver ? 2 : -1);
  }
}
var ModuleFormComponent = class _ModuleFormComponent extends AsyncHandler {
  ...
  driver_query_fn = (_) => To({ q: _ }).then((resp) => resp.data);
  system_query_fn = (_) => Kc({ q: _ }).then((resp) => resp.data);
  edge_query_fn = (_) => Ho({ q: _ }).then((resp) => resp.data);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      this.formModel.update((value) => __spreadProps(__spreadValues({}, value), {
        ...
        alert_level: driver.alert_level || "medium",
        role: driver.role || At.Logic,
        udp: driver.role === At.Service || driver.role === At.Websocket ? false : value.udp,
        system: driver.role === At.Logic ? value.system : void 0
      }));
    });
  }
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Ws(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? Bu(form_item.id, form_item) : Gu(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        if (!this.formModel().id && form_item.settings) {
          await this.newSettings(_item, form_item.settings);
        }
        this._dialog_ref.close();
      } catch (err) {
        this.loading = null;
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ModuleFormComponent, { className: "ModuleFormComponent", filePath: "src/app/modules/module-form.component.ts", lineNumber: 367 });
})();
...
function generateRepositoryFormModel(repository) {
  return {
    ...
    uri: repository?.uri || "",
    repo_type: repository?.repo_type || bs.Driver,
...
function RepositoryFormComponent_Conditional_1_Conditional_5_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(5, "*");
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(6, "mat-form-field", 5)(7, "input", 12);
    \u0275\u0275pipe(8, "translate");
...
function RepositoryFormComponent_Conditional_1_Conditional_7_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(3, "translate");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(4, "mat-form-field", 5)(5, "input", 14);
    \u0275\u0275pipe(6, "translate");
...
function RepositoryFormComponent_Conditional_1_Conditional_8_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(3, "translate");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(4, "mat-form-field", 5)(5, "input", 16);
    \u0275\u0275pipe(6, "translate");
...
function RepositoryFormComponent_Conditional_1_Conditional_9_For_9_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "mat-option", 9);
    \u0275\u0275text(1);
    \u0275\u0275elementEnd();
...
function RepositoryFormComponent_Conditional_1_Conditional_9_Conditional_10_Template(rf, ctx) {
  if (rf & 1) {
...
function RepositoryFormComponent_Conditional_1_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(5, "*");
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(6, "mat-form-field", 5)(7, "mat-select", 18);
    \u0275\u0275repeaterCreate(8, RepositoryFormComponent_Conditional_1_Conditional_9_For_9_Template, 2, 2, "mat-option", 9, \u0275\u0275repeaterTrackByIdentity);
    \u0275\u0275elementEnd();
    \u0275\u0275controlCreate();
    \u0275\u0275conditionalCreate(10, RepositoryFormComponent_Conditional_1_Conditional_9_Conditional_10_Template, 2, 0, "div", 19);
    \u0275\u0275elementStart(11, "mat-error");
    \u0275\u0275text(12);
    \u0275\u0275pipe(13, "translate");
    \u0275\u0275elementEnd()()();
  ...
  if (rf & 2) {
    ...
    \u0275\u0275control();
    \u0275\u0275advance();
    \u0275\u0275repeater(ctx_r0.branch_list());
    \u0275\u0275advance(2);
    \u0275\u0275conditional(ctx_r0.loading_branches() ? 10 : -1);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate(\u0275\u0275pipeBind1(13, 9, "REPOS.BRANCH_REQUIRED"));
  }
}
function RepositoryFormComponent_Conditional_1_Conditional_10_For_14_Conditional_8_Template(rf, ctx) {
  if (rf & 1) {
...
function RepositoryFormComponent_Conditional_1_Conditional_10_For_14_Template(rf, ctx) {
  if (rf & 1) {
    const _r8 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "mat-option", 9)(1, "div", 28);
    \u0275\u0275listener("click", function RepositoryFormComponent_Conditional_1_Conditional_10_For_14_Template_div_click_1_listener() {
    ...
    \u0275\u0275pipe(7, "date");
    \u0275\u0275elementEnd()();
    \u0275\u0275conditionalCreate(8, RepositoryFormComponent_Conditional_1_Conditional_10_For_14_Conditional_8_Template, 2, 1, "code", 32);
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275classProp("w-full!", ctx_r0.formModel().commit_hash === commit_r9.hash);
    \u0275\u0275advance(3);
...
function RepositoryFormComponent_Conditional_1_Conditional_10_Conditional_15_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 19);
    \u0275\u0275element(1, "mat-spinner", 20);
    \u0275\u0275elementEnd();
...
function RepositoryFormComponent_Conditional_1_Conditional_10_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(3, "translate");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(4, "mat-form-field", 5)(5, "mat-select", 23)(6, "mat-select-trigger")(7, "div", 24)(8, "div", 25);
    \u0275\u0275text(9);
    \u0275\u0275elementEnd();
    ...
    \u0275\u0275pipe(12, "slice");
    \u0275\u0275elementEnd()()();
    \u0275\u0275repeaterCreate(13, RepositoryFormComponent_Conditional_1_Conditional_10_For_14_Template, 12, 14, "mat-option", 9, \u0275\u0275repeaterTrackByIdentity);
    \u0275\u0275elementEnd();
    \u0275\u0275controlCreate();
    \u0275\u0275conditionalCreate(15, RepositoryFormComponent_Conditional_1_Conditional_10_Conditional_15_Template, 2, 0, "div", 19);
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(16, "div", 2)(17, "settings-toggle", 27);
    \u0275\u0275listener("ngModelChange", function RepositoryFormComponent_Conditional_1_Conditional_10_Template_settings_toggle_ngModelChange_17_listener($event) {
      \u0275\u0275restoreView(_r7);
      const ctx_r0 = \u0275\u0275nextContext(2);
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind3(12, 9, ctx_r0.formModel().commit_hash || "HEAD", 0, 8), " ");
    \u0275\u0275advance(2);
    \u0275\u0275repeater(ctx_r0.commit_list());
    \u0275\u0275advance(2);
    \u0275\u0275conditional(ctx_r0.loading_commits() ? 15 : -1);
    \u0275\u0275advance(2);
    \u0275\u0275property("ngModel", ctx_r0.follow_latest())("ngModelOptions", \u0275\u0275pureFunction0(13, _c05));
    \u0275\u0275control();
  }
}
...
function RepositoryFormComponent_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "form", 1);
    \u0275\u0275conditionalCreate(1, RepositoryFormComponent_Conditional_1_Conditional_1_Template, 12, 12, "div", 2);
    \u0275\u0275elementStart(2, "div", 3);
    \u0275\u0275conditionalCreate(3, RepositoryFormComponent_Conditional_1_Conditional_3_Template, 8, 4, "div", 2);
    \u0275\u0275conditionalCreate(4, RepositoryFormComponent_Conditional_1_Conditional_4_Template, 12, 12, "div", 2);
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(5, RepositoryFormComponent_Conditional_1_Conditional_5_Template, 12, 12, "div", 2);
    \u0275\u0275elementStart(6, "div", 3);
    \u0275\u0275conditionalCreate(7, RepositoryFormComponent_Conditional_1_Conditional_7_Template, 7, 7, "div", 2);
    \u0275\u0275conditionalCreate(8, RepositoryFormComponent_Conditional_1_Conditional_8_Template, 10, 8, "div", 2);
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(9, RepositoryFormComponent_Conditional_1_Conditional_9_Template, 14, 11, "div", 2);
    \u0275\u0275conditionalCreate(10, RepositoryFormComponent_Conditional_1_Conditional_10_Template, 18, 14);
    \u0275\u0275conditionalCreate(11, RepositoryFormComponent_Conditional_1_Conditional_11_Template, 7, 7, "div", 2);
    \u0275\u0275conditionalCreate(12, RepositoryFormComponent_Conditional_1_Conditional_12_Template, 7, 7, "div", 2);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
...
var RepositoryFormComponent = class _RepositoryFormComponent extends AsyncHandler {
  ...
  is_interface = computed(
    () => this.formModel().repo_type === bs.Interface,
  ...
  ngOnInit() {
    ...
    this.heading = i18n(`${this._name}.${edit ? "EDIT" : "NEW"}`);
    this.repo_types = [
      { id: bs.Driver, name: i18n("REPOS.TYPE_DRIVER") },
      {
        id: bs.Interface,
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Ws(__spreadValues(__spreadValues({}, item_json), form_value), [void 0]) : __spreadValues(__spreadValues({}, item_json), form_value);
      try {
        const _item = await (form_item.id ? _c(form_item.id, form_item) : mc(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
  ...
  async _loadBranches() {
    ...
    try {
      ...
      if (branch && (!this.formModel().branch || !list.includes(this.formModel().branch))) {
        this.formModel.update((model2) => __spreadProps(__spreadValues({}, model2), { branch }));
      }
    } finally {
  ...
  async _loadDefaultBranch() {
    const { id, uri, username, password } = this.formModel();
    return (id ? Ac(id) : isValidUrl(uri) && uri.startsWith("http") ? $c({
      repository_url: uri,
      username,
      password
  ...
  async _loadCommits() {
    ...
    try {
      const list = await (id ? Sc(id, { branch }) : vc({
        repository_url: uri,
        username,
        password,
        branch
      ...
      this.base_commit = commit_list.find((commit) => commit.hash === this.formModel().commit_hash) || commit_list[0];
    } finally {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _RepositoryFormComponent, selectors: [["repository-form"]], outputs: { event: "event" }, features: [\u0275\u0275InheritDefinitionFeature], decls: 2, vars: 3, consts: [[3, "save", "heading", "loading"], ["repository", "", 1, "flex", "flex-col"], [1, "field"], [1, "fieldset"], ["for", "repository-name"], ["appearance", "outline"], ["matInput", "", 3, "placeholder", "formField"], ["for", "type"], [3, "formField"], [3, "value"], ["for", "folder-name"], ["for", "uri"], ["matInput", "", 3, "blur", "placeholder", "formField"], ["for", "repo-u"], ["matInput", "", "autocomplete", "off", 3, "blur", "placeholder", "formField"], ["for", "repo-p"], ["matInput", "", "autocomplete", "new-password", 3, "blur", "type", "placeholder", "formField"], ["matSuffix", "", 3, "click"], [3, "formField", "placeholder"], ["matSuffix", "", 1, "suffix", "ml-2"], ["diameter", "24"], [1, "field", "commit"], ["for", "commit"], ["placeholder", "Select commit", 3, "formField"], [1, "flex", "items-center", "space-x-4"], [1, "flex-1", "truncate"], [1, "bg-base-200", "mr-4!", "rounded-sm", "px-1.5", "font-mono", "text-[0.625rem]"], ["label", "Follow latest commit", 1, "mb-4", 3, "ngModelChange", "ngModel", "ngModelOptions"], [1, "flex", "w-[calc(100%-2.20rem)]", "flex-1", "items-center", "space-x-2", 3, "click"], [1, "flex", "w-1/2", "flex-1", "flex-col", "truncate", "leading-tight"], [1, "truncate"], [1, "text-base-content", "truncate", "font-mono", "text-[0.625rem]", "opacity-30"], [1, "bg-base-200", "rounded-sm", "p-1", "text-xs"], ["for", "description"], ["for", "root-path"]], template: function RepositoryFormComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "fullscreen-modal-shell", 0);
      \u0275\u0275listener("save", function RepositoryFormComponent_Template_fullscreen_modal_shell_save_0_listener() {
        return ctx.submit();
      });
      \u0275\u0275conditionalCreate(1, RepositoryFormComponent_Conditional_1_Template, 13, 10, "form", 1);
      \u0275\u0275elementEnd();
    }
    if (rf & 2) {
      \u0275\u0275property("heading", ctx.heading)("loading", ctx.saving);
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(RepositoryFormComponent, { className: "RepositoryFormComponent", filePath: "src/app/repositories/repository-form.component.ts", lineNumber: 404 });
})();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(CounterComponent, { className: "CounterComponent", filePath: "src/app/ui/counter.component.ts", lineNumber: 74 });
})();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ImageListFieldComponent, { className: "ImageListFieldComponent", filePath: "src/app/ui/custom-fields/image-list-field.component.ts", lineNumber: 216 });
})();
...
function SystemFormComponent_Conditional_1_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.form.zone && !ctx_r0.formModel().zones.length ? 1 : -1);
    \u0275\u0275advance(2);
...
var SystemFormComponent = class _SystemFormComponent extends AsyncHandler {
  ...
  query_fn = (_) => wa({ q: _ }).then((resp) => resp.data);
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (processed_item.id ? Yc(processed_item.id, processed_item) : Vc(processed_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SystemFormComponent, { className: "SystemFormComponent", filePath: "src/app/systems/system-form.component.ts", lineNumber: 605 });
})();
...
function generateTriggerConditionFormModel(condition = {}) {
  ...
  return {
    ...
    operator: condition.operator || Qi.EQ,
    right: right || void 0,
...
function validateEmailList(control) {
  if (control.value && control.value instanceof Array) {
    const value = control.value;
    return value.reduce((valid, email2) => valid && !Validators.email({ value: email2 }), true) ? null : { email: true };
  }
  return null;
}
...
var TriggerFormComponent = class _TriggerFormComponent extends AsyncHandler {
  ...
  async submit() {
    ...
    try {
      const _item = await (form_item.id ? ba(form_item.id, form_item) : va(form_item));
      this._dialog_ref.disableClose = false;
      this.event.emit({ reason: "done", metadata: { item: _item } });
      notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
      this._dialog_ref.close();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(TriggerFormComponent, { className: "TriggerFormComponent", filePath: "src/app/triggers/trigger-form.component.ts", lineNumber: 173 });
})();
...
var UserFormComponent = class _UserFormComponent extends AsyncHandler {
  ...
  constructor() {
    super();
    effect(() => {
      const domains2 = this.domain_list();
      if (!this.formModel().authority_id && domains2[0]) {
        this.formModel.update((value) => __spreadProps(__spreadValues({}, value), {
          authority_id: domains2[0].id
        }));
      }
    });
  }
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      this._dialog_ref.disableClose = true;
      const item_json = item.toJSON ? item.toJSON() : item;
      const form_item = item.id ? Ws(__spreadValues(__spreadValues({}, item_json), this.formModel()), [void 0, null, ""]) : __spreadValues(__spreadValues({}, item_json), this.formModel());
      try {
        const _item = await (form_item.id ? Ra(form_item.id, form_item) : Ua(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(UserFormComponent, { className: "UserFormComponent", filePath: "src/app/users/user-form.component.ts", lineNumber: 386 });
})();
...
var ZoneFormComponent = class _ZoneFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const form_item = item.id ? Ws(__spreadValues(__spreadValues({}, item_json), this.formModel()), [void 0]) : __spreadValues(__spreadValues({}, item_json), this.formModel());
      try {
        const _item = await (form_item.id ? Fa(form_item.id, form_item) : La(form_item));
        this._dialog_ref.disableClose = false;
        this.event.emit({ reason: "done", metadata: { item: _item } });
        notifySuccess(i18n(`${this._name}.SAVE_SUCCESS`));
        this._dialog_ref.close();
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ZoneFormComponent, { className: "ZoneFormComponent", filePath: "src/app/zones/zone-form.component.ts", lineNumber: 338 });
})();
...
var domains = {
  ...
  show: (_) => Ao(_),
  save: (item) => item.id ? qo(item.id, item) : Po(item),
  remove: (item) => Ro(item.id),
  itemConstructor: Di,
...
var drivers = {
  ...
  show: (_) => Io(_),
  save: (item) => item.id ? Co(item.id, item) : Eo(item),
  remove: (item) => Oo(item.id),
  itemConstructor: ls,
  ...
  delete_extra: async (_) => {
    ...
    const count = await Lu(query).then(({ total }) => total).catch((_err) => 0);
    return count ? [
      "error",
...
var groups = {
  ...
  show: (_) => uu(_),
  save: (item) => item.id ? hu(item.id, item) : au(item),
  remove: (item) => lu(item.id),
  itemConstructor: vn,
...
var modules = {
  ...
  show: (_) => ju(_),
  save: (item) => item.id ? Bu(item.id, item) : Gu(item),
  remove: (item) => Wu(item.id),
  itemConstructor: gs,
...
var repositories = {
  ...
  show: (_) => pc(_),
  save: (item) => item.id ? _c(item.id, item) : mc(item),
  remove: (item) => gc(item.id),
  itemConstructor: Gi,
...
async function saveSystem(item) {
  ...
  if (item.id)
    return Yc(item.id, form_data);
  const system = await Vc(form_data);
...
async function addLogicDriverModules(system, driver_ids) {
  ...
  await Promise.all([...new Set(driver_ids)].map(async (driver_id) => {
    const driver = await Io(driver_id);
    if (driver.role !== At.Logic)
      return;
    await Gu({
      driver_id: driver.id,
      control_system_id: system.id,
...
var triggers = {
  ...
  show: (_) => $a(_),
  save: (item) => item.id ? ba(item.id, item) : va(item),
...
var users = {
  ...
  show: (_) => qa(_),
  save: (item) => item.id ? Ra(item.id, item) : Ua(item),
  remove: (item) => Ta(item.id),
  itemConstructor: bn,
...
var zones = {
  ...
  show: (_) => za(_),
  save: (item) => item.id ? Fa(item.id, item) : La(item),
  remove: (item) => ja(item.id),
  itemConstructor: Ft,
...
var ActiveItemService = class _ActiveItemService extends AsyncHandler {
  ...
  async delete() {
    ...
    if (item) {
      ...
      waitForEvent(ref.componentInstance.event, (e) => e.reason === "done").then(async () => {
        ref.componentInstance.loading.set(i18n(`${this.actions.name}.DELETE_LOADING`));
        await this.actions.remove(item).then(() => {
  ...
  async updateSettings() {
    ...
    if (item && item.settings) {
      let settings = (await Lc({ parent_id: item.id })).data;
      settings = new Array(5).fill(0).map((_, idx) => settings.find((_2) => _2.encryption_level === idx) || new Ae({
...
//# sourceMappingURL=chunk-TN2OSL4Q.js.map
