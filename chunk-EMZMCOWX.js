import {
  UserAvatarComponent
} from "./chunk-OUBLER7A.js";
import {
  PlaceDebugService
} from "./chunk-33FSCMNX.js";
import {
  MatMenu,
  MatMenuItem,
  MatMenuModule,
...
var ApplicationPickerTooltipComponent = class _ApplicationPickerTooltipComponent {
  ...
  async ngOnInit() {
    const active_authority = It();
    if (!active_authority?.id) {
      this.loading.set(false);
      return;
    }
    const response = await mo({
...
var SidebarMenuComponent = class _SidebarMenuComponent extends AsyncHandler {
  ...
  get alerts_url() {
    return It()?.config?.backoffice?.alerts_url;
  }
  get metrics_url() {
    return It()?.config?.backoffice?.metrics_url;
  }
  close = () => this._tooltip?.close();
  switchGroup() {
...
//# sourceMappingURL=chunk-EMZMCOWX.js.map
