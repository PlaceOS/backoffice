...
var UserPipe = class _UserPipe {
  async transform(id) {
    ...
    if (!user) {
      user = await lastValueFrom(Aa(id));
      USERS.push(user);
    }
    return user;
  }
...
var SettingsFormComponent = class _SettingsFormComponent extends AsyncHandler {
  ...
  save(level) {
    ...
    lastValueFrom(settings[level].id ? Bc(settings[level].id, details) : Vc(details)).then((new_settings) => {
      this.saving.update((s) => {
        s[level] = false;
        return s;
      });
  ...
  saveAll() {
    ...
    for (let i = 0; i < He.NeverDisplay + 1; i++) {
      ...
      if (settings[i] && !this.saving()[i]) {
        ...
        promises.push(settings[i].id ? Bc(settings[i].id, details) : Vc(details));
      }
    }
    if (promises.length) {
  ...
  _processSettings(settings) {
    ...
    for (let i = 0; i < He.NeverDisplay + 1; i++) {
      ...
      if (!setting) {
        processed_settings.push(this._processSetting(new Ie({ encryption_level: i })));
        continue;
      }
  ...
  _processSetting(setting) {
    if (setting.encryption_level === He.Admin && !this.is_admin || setting.encryption_level === He.Support && !this.is_support || setting.encryption_level === He.NeverDisplay) {
      ...
      return new Ie(__spreadProps(__spreadValues({}, setting), {
        parent_id: this.id(),
        settings_string
      }));
    }
    return new Ie(__spreadProps(__spreadValues({}, setting), { parent_id: this.id() }));
  }
...
//# sourceMappingURL=chunk-2JC2XAFE.js.map
