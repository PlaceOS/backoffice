...
var AdminSchemasComponent = class _AdminSchemasComponent {
  ...
  async saveSchema() {
    ...
    const new_schema = await (schema.id ? U(__spreadProps(__spreadValues({}, details), {
      id: schema.id,
      method: "patch"
    })) : R(__spreadValues({}, details)));
    schema_list = [
      ...schema_list.filter((_2) => schema.id !== _2.id),
      new_schema
    ];
  ...
  getSchema(id) {
    ...
    const schema = schema_list.find((_2) => _2.id === id);
    if (!schema)
      return null;
    return JSON.parse(schema.schema || "{}");
  }
  async loadSchemas() {
    const schema_list = await _({
      query_params: {},
      fn: (_2) => __spreadValues({
        name: _2.name || "",
        schema: _2.schema || "{}"
      }, _2),
      path: "schema"
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminSchemasComponent, { className: "AdminSchemasComponent", filePath: "src/app/admin/schemas.component.ts", lineNumber: 124 });
})();
export {
  AdminSchemasComponent
};
//# sourceMappingURL=chunk-XCWGF74N.js.map
