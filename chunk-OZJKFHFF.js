...
var CsvUploadComponent = class _CsvUploadComponent {
    ...
    if (event.target) {
      ...
      if (file) {
        ...
        reader.addEventListener("load", (evt) => {
          this.processCSVData(evt.srcElement.result, file.name.endsWith(".csv") ? "," : "	");
          element.value = "";
        });
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _CsvUploadComponent, selectors: [["bulk-item-csv-upload"]], inputs: { template: [1, "template"] }, outputs: { list: "list" }, decls: 3, vars: 2, consts: [["matRipple", "", 1, "relative", "mx-4", "flex", "h-[24rem]", "w-[24rem]", "flex-col", "items-center", "justify-center", "space-y-4", "rounded-xl", "border-4", "border-dashed", "border-base-300", "hover:bg-base-200", 3, "hover"], [1, "flex", "h-[24rem]", "w-[24rem]", "flex-col", "items-center", "justify-center", "space-y-4"], [1, "p-4"], ["matRipple", "", 1, "relative", "mx-4", "flex", "h-[24rem]", "w-[24rem]", "flex-col", "items-center", "justify-center", "space-y-4", "rounded-xl", "border-4", "border-dashed", "border-base-300", "hover:bg-base-200", 3, "dragenter", "dragleave", "dragend"], [1, "text-6xl"], [1, "text"], ["type", "file", 1, "absolute", "inset-0", "opacity-0", 3, "change"], ["diameter", "32"], ["btn", "", "matRipple", "", 1, "w-full", 3, "click"]], template: function CsvUploadComponent_Template(rf, ctx) {
    if (rf & 1) {
...
(() => {
  ...
  }], () => [], { template: [{ type: Input, args: [{ isSignal: true, alias: "template", required: false }] }], list: [{ type: Output, args: ["list"] }] });
})();
(() => {
...
var StatusListComponent = class _StatusListComponent {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _StatusListComponent, selectors: [["bulk-item-status-list"]], inputs: { list: [1, "list"], save: [1, "save"] }, outputs: { done: "done" }, features: [\u0275\u0275NgOnChangesFeature], decls: 4, vars: 1, consts: [[1, "flex", "flex-col", "items-center", "px-4", "pb-4"], [1, "info"], [1, "flex", "w-[24rem]", "items-center", "rounded", "border", "border-base-200", "p-2"], [1, "flex", "flex-1", "flex-col", "justify-center", "px-2"], [1, "name", "flex-1"], [1, "text-xs", "text-error"], [1, "status"], [1, "flex", "h-8", "w-8", "items-center", "justify-center", "rounded-full", "text-2xl", "shadow", 3, "bg-error", "text-error-content", "bg-success", "text-success-content", "matTooltip"], ["diameter", "24"], [1, "flex", "h-8", "w-8", "items-center", "justify-center", "rounded-full", "text-2xl", "shadow", 3, "matTooltip"]], template: function StatusListComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "div", 0);
...
var BulkItemModalComponent = class _BulkItemModalComponent {
  ...
  getAvailableFields() {
    ...
    return unique(list.map((i) => ({ id: i, name: i.split("_").join(" ") })), "id").filter((field) => field.id !== "id" && field.id[0] !== "_" && !IGNORE_FIELDS.includes(field.id));
  }
  generateTemplate() {
    switch (this._data.constr) {
      case Tr:
        return [new Tr(SYSTEM_TEMPLATE).toJSON()];
      case Mr:
        return [new Mr(MODULE_TEMPLATE).toJSON()];
      case Ir:
        return [new Ir(DRIVER_TEMPLATE).toJSON()];
      case Jo:
        return [new Jo(USER_TEMPLATE)];
      case vn:
        return [new vn(ZONE_TEMPLATE)];
    }
  }
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _BulkItemModalComponent, selectors: [["app-bulk-item-modal"]], decls: 10, vars: 8, consts: [[1, "m-4", "flex", "items-center", "justify-between", "rounded", "bg-base-200", "px-4", "py-2"], [1, "text-xl", "font-medium"], ["icon", "", "matRipple", "", "mat-dialog-close", ""], [3, "save", "list"], [3, "list", "fields"], [3, "list", "field_list", "mappings"], [3, "template"], [3, "done", "save", "list"], [3, "listChange", "next", "previous", "list", "fields"], [3, "new_mappings", "mapping_done", "previous", "list", "field_list", "mappings"], [3, "list", "template"]], template: function BulkItemModalComponent_Template(rf, ctx) {
    if (rf & 1) {
...
var domains = {
  ...
  show: (_) => iu(_),
  save: (item) => item.id ? ou(item.id, item) : su(item),
  remove: (item) => uu(item.id),
  itemConstructor: Co,
  delete_message: ``,
  name: "DOMAINS"
};
var drivers = {
  ...
  show: (_) => au(_),
  save: (item) => item.id ? lu(item.id, item) : fu(item),
  remove: (item) => hu(item.id),
  itemConstructor: Ir,
  delete_message: ``,
  delete_extra: async (_) => {
    const query = { offset: 0, limit: 1, driver_id: _.id };
    const count = await Uu(query).pipe(map(({ total }) => total)).toPromise().catch((_2) => 0);
    return count ? [
      "error",
...
var modules = {
  ...
  show: (_) => qu(_),
  save: (item) => item.id ? Nu(item.id, item) : Cu(item),
  remove: (item) => ju(item.id),
  itemConstructor: Mr,
  delete_message: ``,
  name: "MODULES"
};
var repositories = {
  ...
  show: (_) => Xu(_),
  save: (item) => item.id ? ec(item.id, item) : tc(item),
  remove: (item) => nc(item.id),
  itemConstructor: Wo,
  delete_message: `'`,
  name: "REPOS"
};
var systems = {
  ...
  show: (_) => wc(_),
  save: (item) => item.id ? Oc(item.id, __spreadProps(__spreadValues({}, item), {
    support_url: processURL(item, item.support_url)
  })) : Ec(__spreadProps(__spreadValues({}, item), {
    support_url: processURL(item, item.support_url)
  })),
...
var users = {
  ...
  show: (_) => Kc(_),
  save: (item) => item.id ? Gc(item.id, item) : Xc(item),
...
var zones = {
  ...
  show: (_) => ra(_),
  save: (item) => item.id ? ia(item.id, item) : oa(item),
  remove: (item) => sa(item.id),
  itemConstructor: vn,
  delete_message: ``,
  name: "ZONES"
};
...
var ActiveItemService = class _ActiveItemService extends AsyncHandler {
  ...
  duplicate() {
    ...
    if (item) {
      const ref = this._dialog.open(DuplicateModalComponent, {
        data: { item, save: this.actions.save }
      });
  ...
  async updateSettings() {
    ...
    if (item && item.settings) {
      let settings = await lastValueFrom(gc({ parent_id: item.id }).pipe(map((resp) => resp.data)));
      settings = new Array(5).fill(0).map((_, idx) => settings.find((_2) => _2.encryption_level === idx) || new Se({
        encryption_level: idx
      }));
      settings.sort((a, b) => a.encryption_level - b.encryption_level);
      this._active_item.next(new this.actions.itemConstructor(__spreadProps(__spreadValues({}, item), { settings })));
    }
  }
...
//# sourceMappingURL=chunk-OZJKFHFF.js.map
