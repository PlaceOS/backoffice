...
var SystemTriggerFormComponent = class _SystemTriggerFormComponent extends AsyncHandler {
  ...
  query_fn = (q) => Wh({ q, limit: 20 }).then((resp) => resp.data);
  ...
  async _loadPlaylistNames() {
    const playlists = await Promise.all(this.formModel().playlists.map((id) => Qh(id).catch(() => null)));
    const names = {};
...
var SystemStateService = class _SystemStateService extends AsyncHandler {
  ...
  _associated_settings = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_associated_settings" } : (
    ...
    loader: async ({ params }) => {
      ...
      if (!(item instanceof Rs))
        return [];
      return isSubsystemUser() ? pa({ parent_id: item.id }).then((response) => response.data).catch(() => []) : Da(item.id).catch(() => []);
    }
  }));
    ...
    loader: async ({ params }) => {
      ...
      try {
        const [triggers, metadata] = await Promise.all([
          Oa(item.id).then((response) => response.total).catch(() => 0),
          sc(item.id).then((response) => response.length).catch(() => 0)
        ]);
        return {
    ...
    loader: async ({ params }) => {
      ...
      try {
        const response = isSubsystemUser() ? {
          data: await Promise.all(item.zones.map((id) => lh(id)))
        } : await Ca(item.id).catch(() => ({
          data: []
        }));
  ...
  _triggers = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_triggers" } : (
    ...
    loader: async ({ params }) => {
      ...
      try {
        const response = await Oa(item.id).catch(() => ({
          data: []
        }));
  ...
  async startSystem() {
    ...
    const error = await Pa(this.active_item.id).then(() => null).catch((err) => {
  ...
  async stopSystem() {
    ...
    const error = await Ta(this.active_item.id).then(() => null).catch((err) => {
  ...
  async addModuleToSystem(device) {
    ...
    if (!system)
      return ref.close();
    await xa(system.id, device.id).catch((_e) => {
      ref.close();
  ...
  async addTrigger(trigger) {
    const t = await wa(this.active_item.id, {
      control_system_id: this.active_item.id,
      enabled: true,
      important: false,
  ...
  async removeTrigger(trigger) {
    ...
    if (!details?.reason)
      return;
    await Na(this.active_item.id, trigger.id).catch((err) => {
      details.close();
  ...
  async reorderModules(fst, snd) {
    ...
    const resp = await ka(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      modules: list
    })).catch((err) => {
  ...
  async removeModule(device) {
    ...
    if (!details?.reason)
      return;
    const system = await qa(this.active_item.id, device.id).catch((err) => {
  ...
  async addZones(zone_list) {
    ...
    const system = await ka(this.active_item.id, __spreadProps(__spreadValues({}, this.active_item), {
      zones
    })).catch((err) => {
  ...
  async toggleModulePower(device) {
    const method = device.running ? yc : gc;
...
//# sourceMappingURL=chunk-IWJNTKZE.js.map
