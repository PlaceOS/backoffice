...
var RepositoryFormComponent = class _RepositoryFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (form_item.id ? Hc(form_item.id, form_item) : zc(form_item));
  ...
  async _loadDefaultBranch() {
    const { id, uri, username, password } = this.formModel();
    return (id ? Zc(id) : isValidUrl(uri) && uri.startsWith("http") ? jc({
      repository_url: uri,
      username,
      password
  ...
  async _loadCommits() {
    ...
    try {
      const list = await (id ? Qc(id, { branch }) : Bc({
        repository_url: uri,
        username,
        password,
        branch
  ...
  async newSettings(item, settings_string) {
    ...
    await ma(new_settings).catch((err) => {
      this.saving.set(null);
...
//# sourceMappingURL=chunk-4RNMDWF4.js.map
