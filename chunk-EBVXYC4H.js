...
var PlaceClusterTaskListComponent = class _PlaceClusterTaskListComponent extends AsyncHandler {
  ...
  killProcess(process) {
    return lastValueFrom(Su(this.cluster().id, { driver: process.id }));
  }
  async loadCluster(id) {
    const clusters = await lastValueFrom(gu({ q: id }).pipe(map((_) => _.data)));
  ...
  async updateProcessList() {
    ...
    const list = await lastValueFrom(vu(this.cluster().id, {
      include_status: true
    })).catch(() => []);
...
//# sourceMappingURL=chunk-EBVXYC4H.js.map
