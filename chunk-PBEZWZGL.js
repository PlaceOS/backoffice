...
function generateBrokerFormModel(broker = {}) {
  const auth_type = broker.auth_type ?? Ni.NoAuth;
  return {
    name: broker.name || "",
...
var applyBrokerFormSchema = (path) => {
  ...
  required(path.username, {
    when({ valueOf }) {
      return valueOf(path.auth_type) === Ni.UserPassword;
    }
  });
  ...
  required(path.certificate, {
    when({ valueOf }) {
      return valueOf(path.auth_type) === Ni.Certificate;
    }
  });
};
...
var BrokerFormComponent = class _BrokerFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      const result = await (form_item.id ? yo(form_item.id, form_item) : $o(form_item)).catch(async (err) => {
...
var AdminBrokersComponent = class _AdminBrokersComponent extends AsyncHandler {
  ...
  async deleteBroker(item) {
    if (item) {
      ...
      const err = await bo(item.id).catch((_) => _);
      details.close();
      if (err)
...
//# sourceMappingURL=chunk-PBEZWZGL.js.map
