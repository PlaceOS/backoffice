...
var OauthSourceFormComponent = class _OauthSourceFormComponent extends AsyncHandler {
  ...
  ngOnChanges(changes) {
    ...
    if (changes.form && form2) {
      ...
      if (form2.info_mappings) {
        const map2 = model.info_mappings || {};
        this.info_mapping_list.set(Object.keys(map2).map((key) => {
          return { PlaceOS: key, Remote: map2[key] };
        }));
      }
      if (form2.authorize_params) {
        const map2 = model.authorize_params || {};
        this.auth_params_list.set(Object.keys(map2).map((key) => {
          return { Parameter: key, Value: map2[key] };
        }));
      }
      if (form2.ensure_matching) {
        const map2 = model.ensure_matching || {};
        this.ensure_matching_list.set(Object.keys(map2).map((key) => {
  ...
  updateMappings(mappings, key, split = false, fields = ["PlaceOS", "Remote"]) {
    ...
    this.formModel().update((model) => __spreadProps(__spreadValues({}, model), { [key]: map2 }));
  }
...
var SamlSourceFormComponent = class _SamlSourceFormComponent extends AsyncHandler {
  ...
  ngOnChanges(changes) {
    ...
    if (changes.form && form2) {
      ...
      if (form2.attribute_statements) {
        const map2 = model.attribute_statements || {};
        this.attribute_statement_mappings.set(Object.keys(map2).map((key) => {
          return { name: key, mappings: map2[key].join(",") };
        }));
      }
      if (form2.idp_sso_target_url_runtime_params) {
        const map2 = model.idp_sso_target_url_runtime_params || {};
        this.runtime_param_list.set(Object.keys(map2).map((key) => {
          return { name: key, mapping: map2[key] };
        }));
      }
    }
  }
  ...
  updateRuntimeParams(mappings) {
    this.timeout("mappings", () => {
      ...
      this.formModel().update((model) => __spreadProps(__spreadValues({}, model), {
        idp_sso_target_url_runtime_params: map2
      }));
    }, 200);
  }
  static \u0275fac = /* @__PURE__ */ (() => {
...
var AuthSourceModalComponent = class _AuthSourceModalComponent extends AsyncHandler {
  ...
  samlFormModel = signal(
    generateSAMLSourceForm(new cr({ authority_id: this._data.domain.id })),
  ...
  ldapFormModel = signal(
    generateLDAPSourceForm(new sr({ authority_id: this._data.domain.id })),
  ...
  type = computed(
    () => this.item() instanceof cr ? "saml" : this.item() instanceof sr ? "ldap" : "oauth",
  ...
  setType(type) {
    ...
    switch (type) {
      case "saml":
        this.item.set(new cr(data));
        break;
      case "ldap":
        this.item.set(new sr(data));
        break;
      default:
        this.item.set(new or(data));
        break;
    }
    this.initialiseForm();
  }
  updateMethod(item) {
    switch (this.type()) {
      case "saml":
        return item.id ? ca(item.id, item) : aa(item);
      case "ldap":
        return item.id ? Xu(item.id, item) : ec(item);
    }
    return item.id ? qc(item.id, item) : Pc(item);
  }
  /**
  ...
  initialiseForm() {
    ...
    } else if (item instanceof cr) {
      this.samlFormModel.set(generateSAMLSourceForm(item));
    } else if (item instanceof sr) {
...
var ApplicationFormComponent = class _ApplicationFormComponent extends AsyncHandler {
  ...
  async submit() {
    await submit(this.form, async () => {
      ...
      try {
        const _item = await (save_item.id ? Ao(save_item.id, save_item) : xo(save_item));
...
var DomainStateService = class _DomainStateService {
  ...
  _url = toSignal(this._router.events.pipe(filter((event) => event instanceof NavigationEnd), map((event) => event.urlAfterRedirects)), { initialValue: this._router.url });
  _changed = signal(
    0,
  ...
  _domain_id = computed(
    () => {
      const item = this.item();
      return item instanceof Yi ? item.id : void 0;
    },
    ...ngDevMode ? [{ debugName: "_domain_id" }] : (
      /* istanbul ignore next */
      []
    )
  );
  _tab = computed(
    ...
    ...ngDevMode ? [{ debugName: "_tab" }] : (
      /* istanbul ignore next */
      []
    )
  );
  // Once opened, retain a tab's data until the user selects another domain.
  _opened_tabs = linkedSignal(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_opened_tabs" } : (
    /* istanbul ignore next */
    {}
  )), {
    ...
    computation: ({ id, tab }, previous) => ({
      ...
      tabs: /* @__PURE__ */ new Set([
        ...previous && previous.value.id === id ? previous.value.tabs : [],
        tab
      ])
    })
  }));
  _load_users = computed(
    () => this._opened_tabs().tabs.has("users"),
    ...ngDevMode ? [{ debugName: "_load_users" }] : (
      /* istanbul ignore next */
      []
    )
  );
  _load_auth = computed(
    ...
    ...ngDevMode ? [{ debugName: "_load_auth" }] : (
      /* istanbul ignore next */
      []
    )
  );
  _load_applications = computed(
    () => this._opened_tabs().tabs.has("applications"),
    ...ngDevMode ? [{ debugName: "_load_applications" }] : (
      /* istanbul ignore next */
      []
    )
  );
  _users = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_users" } : (
    ...
    params: () => this._domain_id() ? {
      id: this._domain_id(),
      changed: this._changed(),
      full: this._load_users()
    } : void 0,
    loader: async ({ params }) => {
      const response = await Ka({
        authority_id: params.id,
        limit: params.full ? 1e3 : 1
      }).catch(() => ({ data: [], total: 0 }));
  ...
  _auth_sources = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_auth_sources" } : (
    ...
    loader: async ({ params }) => {
      ...
      return {
        ...
        total: responses.map((response) => response.total).reduce((total, count) => total + count, 0)
      };
    }
  }));
  ...
  _applications = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_applications" } : (
    ...
    loader: async ({ params }) => {
      const response = await ko(__spreadValues({
        authority_id: params.id
      }, params.full ? {} : { limit: 1 })).catch(() => ({ data: [], total: 0 }));
      return __spreadProps(__spreadValues({}, response), {
        data: [...response.data].sort((a, b) => a.name.localeCompare(b.name))
      });
    }
  ...
  loading = computed(
    () => this._users.isLoading() || this._auth_sources.isLoading() || this._applications.isLoading(),
    ...ngDevMode ? [{ debugName: "loading" }] : (
      /* istanbul ignore next */
      []
    )
  ...
  async update(domain) {
    const item = await Ko(domain.id, domain);
  ...
  async performAzureIntegration() {
    ...
    if (!(item instanceof Yi))
      return;
    const result = await f(`/api/engine/v2/admin_consent/${encodeURIComponent(item.id)}`).catch((error) => {
  ...
  async editApplication(item) {
    item = item || new Gi({ owner_id: this.active_item.id });
  ...
  async deleteApplication(item) {
    ...
    const err = await qo(item.id).catch((_) => _);
    details.close();
    if (err)
  ...
  async deleteAuthSource(item) {
    ...
    const method = item instanceof cr ? ha : item instanceof or ? Tc : tc;
...
//# sourceMappingURL=chunk-BLTUFZOW.js.map
