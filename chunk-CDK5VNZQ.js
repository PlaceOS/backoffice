...
var AdminBrokersComponent = class _AdminBrokersComponent extends AsyncHandler {
  ...
  async deleteBroker(item) {
    if (item) {
      ...
      const err = await vu(item.id).toPromise().catch((_) => _);
      details.close();
      if (err)
  ...
  async loadBrokers() {
    ...
    const brokers = await lastValueFrom(mu().pipe(map((r) => r.data)));
...
//# sourceMappingURL=chunk-CDK5VNZQ.js.map
