...
var ExtensionOutletComponent = class _ExtensionOutletComponent extends AsyncHandler {
  ...
  ngOnInit() {
    Wo().pipe(first((_) => _)).subscribe(() => this.timeout("init", () => this.app_loaded = true));
  ...
  async updateMetadata(item, message) {
    await Xu(item.id, message.name).toPromise();
    await ec(item.id, {
      id: item.id,
      name: message.name,
  ...
  async loadMetadata(item, message, parent = false) {
    const metadata = await Xu(parent ? item.parent_id : item.id, message.name).toPromise();
    if (metadata) {
      this._postMessage({
        id: message.id,
        type: "backoffice",
...
//# sourceMappingURL=chunk-JUS2H4AW.js.map
