...
var UsersStateService = class _UsersStateService {
  ...
  _counts = resource(__spreadProps(__spreadValues({}, ngDevMode ? { debugName: "_counts" } : (
    ...
    loader: async ({ params }) => {
      ...
      const details = await Promise.all([
        nc(item.id).then((d) => d.length).catch((_err) => 0),
        Fu({ user_id: item.id, limit: 1 }).then((response) => response.total).catch(() => 0)
      ]);
  ...
  async bulkAddGroups(existing_groups = []) {
    ...
    if (!groups?.length)
      return;
    this._loading.set(true);
    const results = await Promise.allSettled(groups.map((group) => ju({
      user_id: this.active_item.id,
      group_id: group.id
    })));
    this._loading.set(false);
  ...
  async revive() {
    ...
    try {
      ...
      if (this._service.active_item === item) {
        this._service.replaceItem(new Cn(__spreadProps(__spreadValues({}, item), { deleted: false })));
      }
      this.changed();
...
//# sourceMappingURL=chunk-XK6CIBSI.js.map
