...
function generateBasicHandlers(endpoint, data, filter = (_, _q) => true) {
  ...
  Hr(action);
  Hr(__spreadProps(__spreadValues({}, action), { method: "PUT" }));
  Hr({
    path: `${endpoint}/:id`,
    metadata: data,
    method: "DELETE",
...
Hr({
  ...
  metadata: REPO_DATA,
  method: "GET",
  callback: (_) => REPO_DATA.filter((repo) => repo.type === bs.Interface).map((repo) => repo.name)
});
...
//# sourceMappingURL=chunk-VNPYAALR.js.map
