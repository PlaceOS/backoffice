...
var ResourceImportsComponent = class _ResourceImportsComponent {
  ...
  async ngOnInit() {
    const domain = Ut();
    const domain_list = await lastValueFrom(Ou({ limit: 100 }).pipe(map((_) => _.data)));
  ...
  async importResource(resource, notify = true) {
    ...
    if (!domain)
      return;
    const system = await lastValueFrom(na({
  ...
  async loadResourceList() {
    ...
    const { data } = await lastValueFrom(Xc({
...
//# sourceMappingURL=chunk-JTTNRHNH.js.map
