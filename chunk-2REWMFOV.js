...
var EdgeModalComponent = class _EdgeModalComponent {
  ...
  async save() {
    ...
    const method = edge.id ? zu(edge.id, edge) : Hu(edge);
...
var PlaceEdgeComponent = class _PlaceEdgeComponent {
  ...
  async token(edge) {
    const details = await lastValueFrom(Lu(edge.id));
  ...
  async remove(i) {
    ...
    const err = await lastValueFrom(Wu(i.id)).catch((_) => _);
    details.close();
    if (err)
  ...
  async loadEdges() {
    ...
    const { data } = await lastValueFrom(ju());
...
//# sourceMappingURL=chunk-2REWMFOV.js.map
