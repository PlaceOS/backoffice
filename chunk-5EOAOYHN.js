...
var APIKeyModalComponent = class _APIKeyModalComponent extends AsyncHandler {
  ...
  loadUsers() {
    this.timeout("load_users", async () => {
      const users = await lastValueFrom(this.domain() ? Oa({
...
var APIKeyService = class _APIKeyService {
  ...
  available_domains = Ou({ limit: 500 }).pipe(map((_) => _.data), shareReplay(1));
  ...
  users = combineLatest([
    ...
    return domain ? Oa({ authority_id: domain.id, q }).pipe(map((_) => _.data)) : of([]);
  }), shareReplay(1));
  setDomain(domain) {
  ...
  async editKey(key) {
    ...
    const domain = this._domain.getValue();
    const updated_key = await lastValueFrom(T({
      id: key.id,
      query_params: {},
      fn: (d) => new PlaceAPIKeyDetails(d),
      path: "api_keys",
...
//# sourceMappingURL=chunk-5EOAOYHN.js.map
