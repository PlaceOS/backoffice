...
var DriverUpdateListModalComponent = class _DriverUpdateListModalComponent {
  ...
  async updateDrivers() {
    ...
    await Promise.all(selected.map((driver) => driver.commit !== driver.update_info.commit ? Ko(driver.id, __spreadProps(__spreadValues({}, driver), {
...
var DriverStateService = class _DriverStateService {
  ...
  async updateDriver() {
    ...
    const success = await Ko(item.id, __spreadProps(__spreadValues({}, item), {
  ...
  async recompileDriver() {
    ...
    await Yo(item.id).catch(async (e) => {
      console.log("Error:", e);
  ...
  async reloadDriver() {
    ...
    const success = await Vo(item.id).catch(() => false);
    if (success === false) {
  ...
  async removeModule(device) {
    ...
    if (!details?.reason)
      return;
    const system = await ga(this.active_item.id, device.id).catch((err) => {
...
//# sourceMappingURL=chunk-JPGXFJWW.js.map
