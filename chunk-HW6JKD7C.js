...
var ENDPOINT_SIGNALS = {};
function createMockChangeSignal(initial_value) {
  const state = signal(initial_value);
...
function endpointData(endpoint) {
  return (ENDPOINT_SIGNALS[endpoint] ? ENDPOINT_SIGNALS[endpoint]() : null) || [];
}
...
function generateBasicHandlers(endpoint, data, filter = (_, _q) => true) {
  ...
  ENDPOINT_SIGNALS[endpoint] = createMockChangeSignal(initial_data);
  Gr({
    path: `${endpoint}`,
    metadata: data,
    method: "GET",
  ...
  Gr(action);
  Gr(__spreadProps(__spreadValues({}, action), { method: "PUT" }));
  Gr({
    path: `${endpoint}/:id`,
    metadata: data,
    method: "DELETE",
...
var FILTER_FN3 = (item, q) => {
  ...
  let match = true;
  if (q.q) {
    match = match && `${item.name || ""} ${item.custom_name || ""}`.toLowerCase().indexOf((q.q || "").toLowerCase()) >= 0;
  }
...
Gr({
  ...
  metadata: REPO_DATA,
  method: "GET",
  callback: (_) => REPO_DATA.filter((repo) => repo.type === As.Interface).map((repo) => repo.name)
});
...
var FILTER_FN8 = (item, q) => {
  ...
  if (q.parent_id) {
    ...
    match = match && parents.some((parent) => parent === "root" ? !parent_id : parent === parent_id);
  }
  if (q.control_system_id) {
    const system = endpointData(`${API}/systems`).find((sys) => sys.id === q.control_system_id);
...
function delayedMetadata(parent_id) {
  const delay = Number(sessionStorage.getItem("PLACEOS.mocks.metadata_delay") || 0);
  if (!delay)
    return getMetadata(parent_id);
  return new Promise((resolve) => setTimeout(() => resolve(getMetadata(parent_id)), delay));
}
Gr({
...
//# sourceMappingURL=chunk-HW6JKD7C.js.map
