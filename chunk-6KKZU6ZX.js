...
import {
  ...
  addChipItem,
  removeChipItem
} from "./chunk-ZBOVLJ32.js";
import {
  AuthenticatedImageDirective
} from "./chunk-UTQB3OKR.js";
import {
  DateFromPipe
...
} from "./chunk-VPDNCESF.js";
import {
  UploadsService
} from "./chunk-LWBVO5TV.js";
import {
  SettingsToggleComponent
} from "./chunk-Z7NA6H3I.js";
import {
  MatInput,
  MatInputModule
} from "./chunk-G5APHUZQ.js";
import {
  MatSelect,
  MatSelectModule,
  MatSelectTrigger
} from "./chunk-ZZFHGPV7.js";
import {
  MatError,
  MatFormField,
  MatFormFieldModule,
...
function generateBrokerFormFields(broker) {
  const auth_type = broker.auth_type ?? Vs.NoAuth;
  const fields = {
    ...
    username: new FormControl(broker.username, auth_type === Vs.UserPassword ? [Validators.required] : []),
    password: new FormControl(broker.password, auth_type === Vs.UserPassword ? [Validators.required] : []),
    certificate: new FormControl(broker.certificate, auth_type === Vs.Certificate ? [Validators.required] : []),
...
function generateTriggerConditionForm(condition = {}) {
  ...
  const fields = {
    ...
    operator: new FormControl(condition.operator || io.EQ),
...
var BrokerFormComponent = class _BrokerFormComponent {
  ...
  ngOnInit() {
    this.auth_types = [
      ...
      { id: Vs.NoAuth, name: i18n("ADMIN.BROKERS_AUTH_TYPE_NONE") },
      {
        id: Vs.UserPassword,
...
var DriverFormComponent = class _DriverFormComponent extends AsyncHandler {
  ...
  repo_list = kc({ limit: 1e3 }).pipe(map(({ data }) => data.filter((repo) => repo.type === Vr.Driver)), shareReplay(1));
  driver_list = this.repo.pipe(filter((item) => !!item?.id), distinctUntilKeyChanged("id"), tap(() => this.loading_type.update((types) => [...types, "drivers"])), switchMap(({ id }) => Ec(id, { limit: 1e3 }).pipe(catchError(() => of([])))), map((list) => list.map((_) => ({
    id: _,
    name: _.replace(/\//g, " > ")
  }))), tap(() => this.loading_type.update((types) => types.filter((_) => _ !== "drivers"))), shareReplay(1));
  commit_list = combineLatest([this.repo, this.driver]).pipe(filter(([repo, driver]) => !!repo?.id && !!driver?.id), distinctUntilChanged(([prev_repo, prev_driver], [curr_repo, curr_driver]) => prev_repo?.id === curr_repo?.id && prev_driver?.id === curr_driver?.id), tap(() => this.loading_type.update((types) => [...types, "commits"])), switchMap(([{ id }, driver]) => qc(id, { driver: driver.id, limit: 1e3 }).pipe(catchError(() => of([])))), map((list) => list.map((item) => ({
    id: item.commit,
    name: `${item.subject}`,
  ...
  async applyDriverCommit(commit) {
    ...
    this.subscription("driver_details", Mc(repo.id, {
      driver: `${driver.id}`,
      commit: `${commit.id}`
  ...
  async _loadDetailsFromForm() {
    ...
    if (!id)
      return;
    this.loading.set("DRIVERS.DETAILS_LOADING");
    const repo = await lastValueFrom($c(repository_id));
    const driver = {
      id: file_name,
...
var ModuleFormComponent = class _ModuleFormComponent extends AsyncHandler {
  ...
  driver_query_fn = (_) => Tu({ q: _ }).pipe(map((resp) => resp.data));
  system_query_fn = (_) => Gc({ q: _ }).pipe(map((resp) => resp.data));
  edge_query_fn = (_) => ju({ q: _ }).pipe(map((resp) => resp.data));
  ...
  ngOnChanges(changes) {
    ...
    if (changes.form && form) {
      ...
      this.branch_list = merge(timer(300), form.get("uri").valueChanges, form.get("username").valueChanges, form.get("password").valueChanges).pipe(debounceTime(300), switchMap(() => {
        ...
        return (id ? Uc(id) : isValidUrl(uri) && uri.startsWith("http") ? Pc({
          repository_url: uri,
          username,
          password
      ...
      const default_branch = this.branch_list.pipe(debounceTime(300), switchMap(() => {
        const { id, uri, username, password } = this.form().value;
        return (id ? Rc(id) : isValidUrl(uri) && uri.startsWith("http") ? Ic({
          repository_url: uri,
          username,
          password
        }) : of("")).pipe(catchError(() => of("")));
      ...
      this.commit_list = merge(timer(300), form.get("uri").valueChanges, form.get("branch").valueChanges, form.get("username").valueChanges, form.get("password").valueChanges).pipe(debounceTime(300), switchMap(() => {
        ...
        return (id ? qc(id, { branch }) : isValidUrl(uri) && uri.startsWith("http") && branch ? Tc({
          repository_url: uri,
          username,
          password,
          branch
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(RepositoryFormComponent, { className: "RepositoryFormComponent", filePath: "src/app/ui/forms/repository-form.component.ts", lineNumber: 364 });
})();
...
var SystemFormComponent = class _SystemFormComponent extends AsyncHandler {
  ...
  query_fn = (_) => Da({ q: _ }).pipe(map((resp) => resp.data));
...
var UserFormComponent = class _UserFormComponent {
  ...
  domain_list = xu().pipe(map(({ data }) => data), shareReplay(1));
...
var ZoneFormComponent = class _ZoneFormComponent extends AsyncHandler {
  ...
  async updateZone() {
    ...
    if (parent_id) {
      const zone = await ja(parent_id).toPromise();
...
var ItemCreateUpdateModalComponent = class _ItemCreateUpdateModalComponent extends AsyncHandler {
  ...
  submit() {
    ...
    const item = this.item.id ? as(__spreadValues(__spreadValues({}, item_json), this.form.value), this.item_type === "user" ? [void 0, null, ""] : [void 0, null]) : __spreadValues(__spreadValues({}, item_json), this.form.value);
  ...
  async newSettings(item, settings_string) {
    const new_settings = new Ie({
      parent_id: item.id,
      settings_string,
      encryption_level: He.Support
    });
    const settings = await Vc(new_settings).toPromise().catch((err) => {
      this.loading = null;
...
*/
//# sourceMappingURL=chunk-6KKZU6ZX.js.map
