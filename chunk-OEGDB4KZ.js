...
var DriverUpdateListModalComponent = class _DriverUpdateListModalComponent {
  ...
  async updateDrivers() {
    ...
    await Promise.all(selected.map((driver) => driver.commit !== driver.update_info.commit ? vc(driver.id, __spreadProps(__spreadValues({}, driver), {
...
var DriverStateService = class _DriverStateService {
  ...
  updates_available = this._poll.pipe(switchMap(() => yc({ update_available: true, limit: 1 }).pipe(catchError(() => of({ data: [], total: 0 })))), map((d) => d.total > 1), shareReplay(1));
  modules = this.item.pipe(switchMap(async (item) => {
    if (!item)
      return { data: [] };
    this._loading.next(true);
    const details = await Ea({
      driver_id: item.id
    }).toPromise();
  ...
  docs = this.item.pipe(debounceTime(100), switchMap(async (item) => {
    if (!item)
      return "";
    this._loading.next(true);
    const docs = await lastValueFrom(Ac(item.id)).catch(() => "");
    this._loading.next(false);
    return docs;
  ...
  async updateDriver() {
    ...
    const success = await vc(item.id, __spreadProps(__spreadValues({}, item), {
  ...
  async recompileDriver() {
    ...
    await Sc(item.id).toPromise().catch(async (e) => {
      console.log("Error:", e);
  ...
  async reloadDriver() {
    ...
    const success = await kc(item.id).toPromise().catch(() => false);
    if (success === false) {
  ...
  async removeModule(device) {
    ...
    if (!details || !details.reason)
      return;
    const system = await jl(this.active_item.id, device.id).toPromise().catch((err) => {
...
//# sourceMappingURL=chunk-OEGDB4KZ.js.map
