...
gulp.task('update:version', (next) => {
    ...
    return gulp.src(['./src/app/services/settings.service.ts']) // Any file globs are supported
        .pipe(replace(/get version\(\) { return '[0-9a-zA-Z.-]*'/g, `get version() { return '${v}'`, { logs: { enabled: true } }))
...
gulp.task('clean:version', (next) => {
    return gulp.src(['./src/app/services/settings.service.ts']) // Any file globs are supported
        .pipe(replace(/get version\(\) { return '[0-9a-zA-Z.-]*'/g, `get version() { return 'local-dev'`, { logs: { enabled: true } }))
