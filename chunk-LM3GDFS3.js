...
var RepositoryAboutComponent = class _RepositoryAboutComponent extends AsyncHandler {
  ...
  local_url = computed(() => this.item()?.type === Vr.Interface ? `${location.origin}/${this.item()?.folder_name}/` : `${location.hash}`, ...ngDevMode ? [{ debugName: "local_url" }] : []);
  repo_uri = computed(() => this.item()?.uri.replace(/\/[a-zA-Z0-9\-.:]*@/, "/...@"), ...ngDevMode ? [{ debugName: "repo_uri" }] : []);
  is_interface = computed(() => this.item()?.type === Vr.Interface, ...ngDevMode ? [{ debugName: "is_interface" }] : []);
...
//# sourceMappingURL=chunk-LM3GDFS3.js.map
